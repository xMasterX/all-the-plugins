...
typedef struct {
    FujitsuState* state;
    uint8_t focus;
} FujitsuSetupViewModel;

struct FujitsuSetupView {
    View* view;
};

static uint8_t row_count(void) {
    return fujitsu_ir_get_option_count() > 0 ? 2 : 1;
}
...
static void fujitsu_setup_view_draw(Canvas* canvas, void* model) {
    FujitsuSetupViewModel* m = model;
    if(!m->state) return;
    ...
    bool has_opt = fujitsu_ir_get_option_count() > 0;
    ...
    if(has_opt) {
        ...
        canvas_set_font(canvas, FontSecondary);
        canvas_draw_str_aligned(
            canvas, 32, oy + 3, AlignCenter, AlignTop, fujitsu_ir_get_option_label());
        ...
        canvas_draw_str_aligned(
            ...
            AlignCenter,
            AlignTop,
            fujitsu_ir_get_option_name(m->state->option));
        ...
        snprintf(
            ...
            (unsigned)fujitsu_ir_get_option_count());
...
static bool fujitsu_setup_view_input(InputEvent* event, void* context) {
    FujitsuSetupView* view = context;
    bool consumed = false;

    with_view_model(
        view->view,
        FujitsuSetupViewModel * m,
        {
            if(event->type != InputTypeShort && event->type != InputTypeRepeat) {
                // nothing
            } else {
                uint8_t rows = row_count();
                uint8_t opts = fujitsu_ir_get_option_count();
                switch(event->key) {
                case InputKeyUp:
...
FujitsuSetupView* fujitsu_setup_view_alloc(void) {
    FujitsuSetupView* view = malloc(sizeof(FujitsuSetupView));
    view->view = view_alloc();

    view_allocate_model(view->view, ViewModelTypeLocking, sizeof(FujitsuSetupViewModel));
    view_set_context(view->view, view);
    view_set_draw_callback(view->view, fujitsu_setup_view_draw);
    view_set_input_callback(view->view, fujitsu_setup_view_input);
...
void fujitsu_setup_view_set_state(FujitsuSetupView* view, FujitsuState* state) {
    with_view_model(view->view, FujitsuSetupViewModel * m, { m->state = state; }, true);
}
