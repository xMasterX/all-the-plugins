...
static inline uint8_t eeprom_read_byte(const uint8_t* address) {
    return EEPROM.read((int)(uintptr_t)address);
}

static inline void eeprom_update_byte(uint8_t* address, uint8_t value) {
    EEPROM.update((int)(uintptr_t)address, value);
}

static inline void eeprom_write_block(const void* src, void* dst, size_t len) {
    const uint8_t* in = (const uint8_t*)src;
    int addr = (int)(uintptr_t)dst;
    for(size_t i = 0; i < len; i++) {
        EEPROM.write(addr + (int)i, in[i]);
    }
}

static inline void eeprom_read_block(void* dst, const void* src, size_t len) {
    uint8_t* out = (uint8_t*)dst;
    int addr = (int)(uintptr_t)src;
    for(size_t i = 0; i < len; i++) {
        out[i] = EEPROM.read(addr + (int)i);
    }
}
