...
typedef struct {
    DelonghiState* state;
    uint8_t focus;
} DelonghiSetupViewModel;

struct DelonghiSetupView {
    View* view;
};

static uint8_t row_count(void) {
    return delonghi_ir_get_option_count() > 0 ? 2 : 1;
}
...
static void delonghi_setup_view_draw(Canvas* canvas, void* model) {
    DelonghiSetupViewModel* m = model;
    if(!m->state) return;
    ...
    bool has_opt = delonghi_ir_get_option_count() > 0;
    ...
    if(has_opt) {
        ...
        canvas_set_font(canvas, FontSecondary);
        canvas_draw_str_aligned(
            canvas, 32, oy + 3, AlignCenter, AlignTop, delonghi_ir_get_option_label());
        ...
        canvas_draw_str_aligned(
            ...
            AlignCenter,
            AlignTop,
            delonghi_ir_get_option_name(m->state->option));
        ...
        snprintf(
            ...
            (unsigned)delonghi_ir_get_option_count());
...
static bool delonghi_setup_view_input(InputEvent* event, void* context) {
    DelonghiSetupView* view = context;
    bool consumed = false;

    with_view_model(
        view->view,
        DelonghiSetupViewModel * m,
        {
            if(event->type != InputTypeShort && event->type != InputTypeRepeat) {
                // nothing
            } else {
                uint8_t rows = row_count();
                uint8_t opts = delonghi_ir_get_option_count();
                switch(event->key) {
                case InputKeyUp:
...
DelonghiSetupView* delonghi_setup_view_alloc(void) {
    DelonghiSetupView* view = malloc(sizeof(DelonghiSetupView));
    view->view = view_alloc();

    view_allocate_model(view->view, ViewModelTypeLocking, sizeof(DelonghiSetupViewModel));
    view_set_context(view->view, view);
    view_set_draw_callback(view->view, delonghi_setup_view_draw);
    view_set_input_callback(view->view, delonghi_setup_view_input);
...
void delonghi_setup_view_set_state(DelonghiSetupView* view, DelonghiState* state) {
    with_view_model(view->view, DelonghiSetupViewModel * m, { m->state = state; }, true);
}
