...
} DelonghiExtraViewModel;

struct DelonghiExtraView {
    View* view;
    DelonghiExtraViewSendCallback send_callback;
    void* send_context;
};

static void delonghi_extra_view_draw(Canvas* canvas, void* model) {
    DelonghiExtraViewModel* m = model;
    if(!m->state) return;
    ...
    uint8_t total = (uint8_t)DelonghiExtraCount;
    ...
    for(uint8_t r = 0; r < shown; r++) {
        ...
        canvas_draw_str_aligned(
            canvas, 32, y + 2, AlignCenter, AlignTop, delonghi_ir_get_extra_name((DelonghiExtra)i));
    ...
    char buf[DELONGHI_CODE_STR_LEN];
    ...
    DelonghiRequest req = delonghi_state_request(m->state);
    delonghi_ir_format_extra(&req, (DelonghiExtra)m->selected, buf, sizeof(buf));
...
static bool delonghi_extra_view_input(InputEvent* event, void* context) {
    DelonghiExtraView* view = context;
    bool consumed = false;
    bool send = false;
    DelonghiExtra to_send = (DelonghiExtra)0;

    with_view_model(
        view->view,
        DelonghiExtraViewModel * m,
        {
            if(event->type == InputTypeShort || event->type == InputTypeRepeat) {
                ...
                case InputKeyUp:
                    ...
                    break;
                case InputKeyDown:
                    if(m->selected + 1 < (uint8_t)DelonghiExtraCount) m->selected++;
                ...
                case InputKeyOk:
                    if(event->type == InputTypeShort) {
                        to_send = (DelonghiExtra)m->selected;
                        send = true;
                    }
                    consumed = true;
                    break;
...
DelonghiExtraView* delonghi_extra_view_alloc(void) {
    DelonghiExtraView* view = malloc(sizeof(DelonghiExtraView));
    view->view = view_alloc();
    view->send_callback = NULL;
    view->send_context = NULL;

    view_allocate_model(view->view, ViewModelTypeLocking, sizeof(DelonghiExtraViewModel));
    view_set_context(view->view, view);
    view_set_draw_callback(view->view, delonghi_extra_view_draw);
    view_set_input_callback(view->view, delonghi_extra_view_input);
...
void delonghi_extra_view_set_state(DelonghiExtraView* view, DelonghiState* state) {
    with_view_model(view->view, DelonghiExtraViewModel * m, { m->state = state; }, true);
}

void delonghi_extra_view_set_last_sent(DelonghiExtraView* view, const char* last_sent) {
    with_view_model(view->view, DelonghiExtraViewModel * m, { m->last_sent = last_sent; }, true);
}
