#include "kelvinator_setup_view.h"
...
typedef struct {
    ...
    uint8_t focus;
} KelvinatorSetupViewModel;

struct KelvinatorSetupView {
    View* view;
};

static uint8_t row_count(void) {
    return kelvinator_ir_get_option_count() > 0 ? 2 : 1;
}
...
static void kelvinator_setup_view_draw(Canvas* canvas, void* model) {
    KelvinatorSetupViewModel* m = model;
    if(!m->state) return;
    ...
    bool has_opt = kelvinator_ir_get_option_count() > 0;
    ...
    if(has_opt) {
        ...
        canvas_set_font(canvas, FontSecondary);
        canvas_draw_str_aligned(
            canvas, 32, oy + 3, AlignCenter, AlignTop, kelvinator_ir_get_option_label());
        ...
        canvas_draw_str_aligned(
            ...
            AlignCenter,
            AlignTop,
            kelvinator_ir_get_option_name(m->state->option));
        ...
        snprintf(
            ...
            (unsigned)kelvinator_ir_get_option_count());
...
static bool kelvinator_setup_view_input(InputEvent* event, void* context) {
    KelvinatorSetupView* view = context;
    bool consumed = false;

    with_view_model(
        view->view,
        KelvinatorSetupViewModel * m,
        {
            if(event->type != InputTypeShort && event->type != InputTypeRepeat) {
                // nothing
            } else {
                uint8_t rows = row_count();
                uint8_t opts = kelvinator_ir_get_option_count();
                switch(event->key) {
                case InputKeyUp:
...
KelvinatorSetupView* kelvinator_setup_view_alloc(void) {
    KelvinatorSetupView* view = malloc(sizeof(KelvinatorSetupView));
    view->view = view_alloc();

    view_allocate_model(view->view, ViewModelTypeLocking, sizeof(KelvinatorSetupViewModel));
    view_set_context(view->view, view);
    view_set_draw_callback(view->view, kelvinator_setup_view_draw);
    view_set_input_callback(view->view, kelvinator_setup_view_input);
...
void kelvinator_setup_view_set_state(KelvinatorSetupView* view, KelvinatorState* state) {
    with_view_model(view->view, KelvinatorSetupViewModel * m, { m->state = state; }, true);
}
