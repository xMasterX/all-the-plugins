...
struct KelonExtraView {
    View* view;
    KelonExtraViewSendCallback send_callback;
    void* send_context;
};

static void kelon_extra_view_draw(Canvas* canvas, void* model) {
    KelonExtraViewModel* m = model;
    if(!m->state) return;
    ...
    uint8_t total = (uint8_t)KelonExtraCount;
    ...
    for(uint8_t r = 0; r < shown; r++) {
        ...
        canvas_draw_str_aligned(
            canvas, 32, y + 2, AlignCenter, AlignTop, kelon_ir_get_extra_name((KelonExtra)i));
    ...
    KelonRequest req = kelon_state_request(m->state);
    kelon_ir_format_extra(&req, (KelonExtra)m->selected, buf, sizeof(buf));
...
static bool kelon_extra_view_input(InputEvent* event, void* context) {
    KelonExtraView* view = context;
    bool consumed = false;
    bool send = false;
    KelonExtra to_send = (KelonExtra)0;

    with_view_model(
        view->view,
        KelonExtraViewModel * m,
        {
            if(event->type == InputTypeShort || event->type == InputTypeRepeat) {
                ...
                case InputKeyUp:
                    ...
                    break;
                case InputKeyDown:
                    if(m->selected + 1 < (uint8_t)KelonExtraCount) m->selected++;
...
KelonExtraView* kelon_extra_view_alloc(void) {
    KelonExtraView* view = malloc(sizeof(KelonExtraView));
    view->view = view_alloc();
    view->send_callback = NULL;
    view->send_context = NULL;

    view_allocate_model(view->view, ViewModelTypeLocking, sizeof(KelonExtraViewModel));
    view_set_context(view->view, view);
    view_set_draw_callback(view->view, kelon_extra_view_draw);
    view_set_input_callback(view->view, kelon_extra_view_input);
...
void kelon_extra_view_set_state(KelonExtraView* view, KelonState* state) {
    with_view_model(view->view, KelonExtraViewModel * m, { m->state = state; }, true);
}

void kelon_extra_view_set_last_sent(KelonExtraView* view, const char* last_sent) {
    with_view_model(view->view, KelonExtraViewModel * m, { m->last_sent = last_sent; }, true);
}
