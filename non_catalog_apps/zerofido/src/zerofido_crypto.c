...
static bool zf_unwrap_private_key(
    ...
    bool ok = furi_hal_crypto_decrypt(
        record->private_wrapped, private_key, sizeof(record->private_wrapped));
...
bool zf_crypto_hmac_sha256_parts_with_scratch(
    ...
    size_t second_size,
    uint8_t out[32]) {
    return zf_hmac_sha256_parts_with_scratch(
...
bool zf_crypto_generate_credential_keypair(ZfCredentialRecord* record) {
    ...
    do {
        ...
        if(!furi_hal_crypto_encrypt(
               private_key, record->private_wrapped, sizeof(record->private_wrapped))) {
            furi_hal_crypto_enclave_unload_key(ZF_UNIQUE_KEY_SLOT);
            break;
        }
        furi_hal_crypto_enclave_unload_key(ZF_UNIQUE_KEY_SLOT);
