...
_Static_assert(
    sizeof(ZfLocalAttestationScratch) <= 1536U,
...
static bool
    zf_der_write_length(uint8_t* out, size_t out_capacity, size_t value_len, size_t* written) {
    size_t len_size = zf_der_length_size(value_len);
...
static bool
    zf_der_append_tlv(ZfDerBuffer* buffer, uint8_t tag, const uint8_t* value, size_t value_len) {
...
static bool zf_der_append_bit_string(
    ZfDerBuffer* buffer,
    const uint8_t* value,
    size_t value_len,
    uint8_t* scratch,
...
static bool zf_der_parse_length(
    const uint8_t* input,
    size_t input_len,
    size_t* header_len,
    size_t* value_len) {
...
static bool zf_der_read_element(
    const uint8_t* input,
    size_t input_len,
...
static bool zf_local_attestation_ensure_directories(
    const ZfLocalAttestationProfile* profile,
    Storage* storage) {
...
static bool zf_local_attestation_remove_paths(
    const ZfLocalAttestationProfile* profile,
    Storage* storage,
    ...
    if(remove_committed) {
        return zf_storage_remove_atomic_file(
                   storage, profile->cert_file, profile->cert_temp_file) &&
...
static void
    zf_local_attestation_cleanup_temps(const ZfLocalAttestationProfile* profile, Storage* storage) {
...
static void
    zf_local_attestation_cleanup_pair(const ZfLocalAttestationProfile* profile, Storage* storage) {
...
bool zf_local_attestation_load_cert(
    const ZfLocalAttestationProfile* profile,
    uint8_t* out,
    size_t out_capacity,
    size_t* out_len) {
...
static void zf_local_attestation_cleanup_storage(
    const ZfLocalAttestationProfile* profile,
    bool remove_committed) {
    Storage* storage = NULL;
...
static bool zf_encrypt_private_key(
    const ZfLocalAttestationProfile* profile,
    Storage* storage,
    uint8_t* private_key) {
    bool ok = false;
    ...
    ok = zf_storage_write_encrypted_blob_atomic(
        storage, profile->key_file, profile->key_temp_file, &spec);
    if(!ok) {
...
bool zf_local_attestation_load_private_key(
    const ZfLocalAttestationProfile* profile,
...
bool zf_local_attestation_extract_cert_public_key(
    ...
    if(!cert || !public_key ||
       !zf_der_read_element(
           cert, cert_len, 0x30, &certificate_value, &certificate_value_len, &element_len) ||
       element_len != cert_len) {
        return false;
    }
    if(!zf_der_read_element(
           certificate_value,
           certificate_value_len,
           0x30,
           &tbs_value,
           &tbs_value_len,
           &element_len)) {
    ...
    offset += element_len;
    if(!zf_der_read_element(
           certificate_value + offset,
           certificate_value_len - offset,
           0x30,
           &element_value,
           &element_value_len,
           &element_len)) {
    ...
    if(tbs_offset < tbs_value_len && tbs_value[tbs_offset] == 0xA0) {
        if(!zf_der_read_element(
               tbs_value + tbs_offset,
               tbs_value_len - tbs_offset,
               0xA0,
               &element_value,
               &element_value_len,
               &element_len)) {
    ...
    offset = 0;
    if(!zf_der_read_element(
           spki_value, spki_value_len, 0x30, &element_value, &element_value_len, &element_len)) {
        return false;
    }
    offset += element_len;
    if(!zf_der_read_element(
           spki_value + offset,
           spki_value_len - offset,
           0x03,
           &bit_string_value,
           &bit_string_value_len,
           &element_len)) {
...
static bool zf_private_key_matches_cert(
    const ZfLocalAttestationProfile* profile,
    const uint8_t private_key[ZF_PRIVATE_KEY_LEN],
    uint8_t* cert,
...
static bool zf_generate_keypair(
    uint8_t private_key[ZF_PRIVATE_KEY_LEN],
    uint8_t public_key[ZF_LOCAL_ATTESTATION_EC_POINT_SIZE]) {
...
static bool zf_build_tbs(
    ...
    ZfDerBuffer spki = {
        .data = scratch ? scratch->spki : NULL, .capacity = scratch ? sizeof(scratch->spki) : 0U};
    ...
    if(!zf_der_append_tlv(&body, 0xA0U, version, sizeof(version)) ||
       ...
       !zf_der_append_bit_string(
           &spki,
           public_key,
           ZF_LOCAL_ATTESTATION_EC_POINT_SIZE,
           scratch->bit_string,
...
static bool zf_build_cert(
    ...
    if(!zf_crypto_sign_hash_with_private_key(
           private_key, hash, signature, sizeof(signature), &signature_len)) {
    ...
    bool ok =
        zf_der_append(&body, scratch->tbs, tbs_len) &&
        zf_der_append(&body, ecdsa_sha256_alg, sizeof(ecdsa_sha256_alg)) &&
        zf_der_append_bit_string(
            &body, signature, signature_len, scratch->bit_string, sizeof(scratch->bit_string)) &&
...
static bool zf_bootstrap_assets(
    const ZfLocalAttestationProfile* profile,
    const uint8_t* cert,
    size_t cert_len,
    const uint8_t private_key[ZF_PRIVATE_KEY_LEN]) {
    ...
    do {
        ...
        zf_local_attestation_cleanup_temps(profile, storage);
        if(!zf_storage_write_file_atomic(
               storage, profile->cert_file, profile->cert_temp_file, cert, cert_len)) {
