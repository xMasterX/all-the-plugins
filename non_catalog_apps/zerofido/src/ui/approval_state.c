...
static void zerofido_ui_prepare_interaction_locked(
    ZerofidoApp* app,
    ...
    strncpy(
        app->approval.target_id, target_id ? target_id : "", sizeof(app->approval.target_id) - 1);
    ...
    if(kind == ZfInteractionKindApproval) {
        ...
        zf_ui_format_approval_header(
            app->approval.details.approval.operation,
            sizeof(app->approval.details.approval.operation),
            protocol,
            operation);
...
static bool zerofido_ui_wait_for_interaction_result(
    ZerofidoApp* app,
...
bool zerofido_ui_request_approval(
    ...
    zerofido_ui_prepare_interaction_locked(
        app, ZfInteractionKindApproval, protocol, operation, target_id, user_text);
...
bool zerofido_ui_request_assertion_selection(
    ...
    zerofido_ui_prepare_interaction_locked(
        app,
        ZfInteractionKindAssertionSelection,
        ZfUiProtocolFido2,
        "Select account",
        rp_id,
        "Choose account");
...
void zerofido_ui_show_interaction(ZerofidoApp* app) {
    ...
    if(kind == ZfInteractionKindApproval) {
        zf_ui_format_approval_body(
            body,
            sizeof(body),
            app->approval.details.approval.protocol,
...
void zerofido_ui_hide_interaction(ZerofidoApp* app) {
    ...
    should_hide = app->approval.pending_hide_generation == generation &&
                  approval_state != ZfApprovalPending;
    if(should_hide) {
        app->approval.pending_hide_generation = 0;
    }
    furi_mutex_release(app->ui_mutex);
