...
bool zf_cbor_encode_p256_cose_key(
    ZfCborEncoder* enc,
    int64_t alg,
    const uint8_t* x,
    size_t x_size,
    const uint8_t* y,
    size_t y_size) {
    return zf_cbor_encode_map(enc, 5) && zf_cbor_encode_int(enc, 1) &&
           zf_cbor_encode_int(enc, 2) && zf_cbor_encode_int(enc, 3) &&
           zf_cbor_encode_int(enc, alg) && zf_cbor_encode_int(enc, -1) &&
           zf_cbor_encode_int(enc, 1) && zf_cbor_encode_int(enc, -2) &&
           zf_cbor_encode_bytes(enc, x, x_size) && zf_cbor_encode_int(enc, -3) &&
           zf_cbor_encode_bytes(enc, y, y_size);
}
