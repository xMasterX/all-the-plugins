...
bool zf_storage_build_child_path(
    const char* dir_path,
    const char* name,
    char* path,
    size_t path_size) {
...
bool zf_storage_for_each_dir_entry(
    Storage* storage,
    const char* dir_path,
    char* name_buffer,
    size_t name_buffer_size,
    ZfStorageDirEntryVisitor visitor,
    void* context) {
    File* dir = NULL;
    FileInfo info;
    bool ok = false;
...
static bool
    zf_storage_remove_suffix_visitor(const char* name, const FileInfo* info, void* context) {
    ...
    if(!zf_storage_build_child_path(
           remove_context->dir_path,
           name,
           remove_context->path_buffer,
...
bool zf_storage_remove_dir_entries_with_suffix(
    Storage* storage,
    const char* dir_path,
    const char* suffix,
    char* name_buffer,
    ...
    return zf_storage_for_each_dir_entry(
        storage,
        dir_path,
        name_buffer,
        name_buffer_size,
        zf_storage_remove_suffix_visitor,
        &context);
}

static bool zf_storage_build_suffixed_path(
    const char* path,
    const char* suffix,
    char* out,
    size_t out_size) {
...
bool zf_storage_recover_atomic_file(Storage* storage, const char* path, const char* temp_path) {
    ...
    if(!storage || !path || !temp_path ||
       !zf_storage_build_suffixed_path(
           path, ZF_STORAGE_BACKUP_SUFFIX, backup_path, sizeof(backup_path))) {
        return false;
    }
...
static bool zf_storage_prepare_atomic_backup(
    Storage* storage,
    const char* path,
...
static bool
    zf_storage_publish_atomic_temp(Storage* storage, const char* path, const char* temp_path) {
    char backup_path[ZF_STORAGE_PATH_BUFFER_SIZE];
    bool backup_active = false;
    bool ok = false;

    if(!zf_storage_build_suffixed_path(
           path, ZF_STORAGE_BACKUP_SUFFIX, backup_path, sizeof(backup_path))) {
        zf_storage_remove_optional(storage, temp_path);
        return false;
    }
...
bool zf_storage_write_file_atomic(
    Storage* storage,
    const char* path,
...
bool zf_storage_write_format_atomic(
    Storage* storage,
    const char* path,
    const char* temp_path,
    ZfStorageFormatWriter writer,
    void* context) {
    bool ok = false;
    bool opened = false;
    bool wrote = false;
...
static bool
    zf_storage_version_allowed(const ZfStorageEncryptedBlobReadSpec* spec, uint32_t version) {
...
bool zf_storage_write_encrypted_blob_atomic(
    Storage* storage,
    const char* path,
    const char* temp_path,
    const ZfStorageEncryptedBlobWriteSpec* spec) {
    ...
    ok = zf_storage_write_format_atomic(
        storage, path, temp_path, zf_storage_write_encrypted_blob_fields, &context);
...
bool zf_storage_read_encrypted_blob(
    Storage* storage,
    const char* path,
    ...
    memset(mac, 0, sizeof(mac));
    if(!zf_storage_build_suffixed_path(
           path, ZF_STORAGE_TEMP_SUFFIX, temp_path, sizeof(temp_path)) ||
       !zf_storage_recover_atomic_file(storage, path, temp_path)) {
        goto cleanup;
    }
    ...
    if(spec->verify_mac && !spec->verify_mac(
                               spec->plaintext,
                               spec->plaintext_len,
                               iv,
                               encrypted,
                               spec->encrypted_len,
                               spec->has_mac ? mac : NULL,
                               spec->has_mac ? spec->mac_len : 0U,
                               spec->callback_context)) {
        goto cleanup;
    }
    if(spec->out_version) {
