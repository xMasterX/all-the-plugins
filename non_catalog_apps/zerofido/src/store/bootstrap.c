...
static bool
    zf_store_bootstrap_index_visitor(const char* name, const FileInfo* info, void* context) {
    ...
    if(zf_store_record_format_load_index_with_buffer(
           index_context->storage,
           name,
           &index_context->store->records[index_context->store->count],
           index_context->buffer,
...
bool zf_store_bootstrap_init_with_buffer(
    ...
    if(!zf_storage_for_each_dir_entry(
           storage,
           ZF_APP_DATA_DIR,
           name,
           sizeof(name),
           zf_store_bootstrap_index_visitor,
           &context)) {
        goto cleanup;
    }
...
static bool
    zf_store_bootstrap_wipe_visitor(const char* name, const FileInfo* info, void* context) {
...
bool zf_store_bootstrap_wipe_app_data(Storage* storage) {
    ...
    if(!zf_storage_remove_optional_paths(
           storage, pin_paths, sizeof(pin_paths) / sizeof(pin_paths[0]))) {
        return false;
    }

    return zf_storage_for_each_dir_entry(
        storage, ZF_APP_DATA_DIR, name, sizeof(name), zf_store_bootstrap_wipe_visitor, &context);
}
