...
static inline void
    zf_store_build_record_path(const char* file_name, char* path, size_t path_size) {
...
static inline void
    zf_store_build_backup_path(const char* file_name, char* path, size_t path_size) {
    snprintf(path, path_size, ZF_APP_DATA_DIR "/%s.bak", file_name);
}

static inline void
    zf_store_build_counter_floor_path(const char* file_name, char* path, size_t path_size) {
    snprintf(path, path_size, ZF_APP_DATA_DIR "/%s.counter", file_name);
}

static inline void
    zf_store_build_counter_floor_temp_path(const char* file_name, char* path, size_t path_size) {
