...
static void zf_record_compute_counter_binding_fields(
...
static bool zf_counter_floor_encode_fields(
    const uint8_t credential_id[ZF_CREDENTIAL_ID_LEN],
    uint32_t created_at,
    uint32_t sign_count,
...
static bool zf_counter_floor_decode_fields(
    const uint8_t credential_id[ZF_CREDENTIAL_ID_LEN],
    uint32_t created_at,
    ...
    if(!furi_hal_crypto_enclave_load_key(
           FURI_HAL_CRYPTO_ENCLAVE_UNIQUE_KEY_SLOT, counter_file->iv)) {
        return false;
    }
...
static bool zf_store_counter_floor_write_value_fields(
    Storage* storage,
    ...
    ok = zf_storage_write_file_atomic(
        storage, path, temp_path, (const uint8_t*)&counter_file, sizeof(counter_file));
    zf_crypto_secure_zero(&counter_file, sizeof(counter_file));
    return ok;
}

static bool zf_store_counter_floor_write_value(
    Storage* storage,
...
static bool zf_store_counter_floor_validate_fields(
    ...
    if(!storage_file_exists(storage, path)) {
        if(has_embedded_counter_floor) {
            if(!zf_counter_floor_decode_fields(
                   credential_id, created_at, embedded_counter_floor, &stored_sign_count) ||
        ...
        if(!zf_store_counter_floor_write_value_fields(
               storage, file_name, credential_id, created_at, *sign_count)) {
            return false;
        }
        if(out_high_water) {
    ...
    if(!zf_counter_floor_decode_fields(
           credential_id, created_at, (const ZfCounterFloorFile*)buffer, &stored_sign_count)) {
        return false;
    }
...
static bool zf_store_counter_floor_validate(
    Storage* storage,
...
static bool zf_store_counter_floor_validate_index(
    Storage* storage,
    const char* file_name,
...
static bool zf_record_wrap_hmac_secret(
    const ZfCredentialRecord* record,
...
static bool zf_record_unwrap_hmac_secret(
    ZfCredentialRecord* record,
...
bool zf_store_record_format_reserve_counter_with_buffer(
    ...
    ok = zf_store_record_format_write_record_with_buffer(
        storage, &reserved_record, buffer, buffer_size);
...
bool zf_store_record_format_reserve_counter(
    ...
    bool ok = zf_store_record_format_reserve_counter_with_buffer(
        storage, record, buffer, sizeof(buffer), out_high_water);
...
bool zf_store_record_format_encode(
    ...
    if(!zf_counter_floor_encode_fields(
           record->credential_id, record->created_at, record->sign_count, &counter_floor)) {
        return false;
    }
...
static bool zf_record_id_matches_file_name(
...
static bool zf_record_decode_field(
    ZfCborCursor* cursor,
    uint64_t key,
    ZfCredentialRecord* record,
    uint32_t* version) {
    ...
    case ZfRecordKeyDisplayName:
        return zf_record_decode_text(
            record->user_display_name, sizeof(record->user_display_name), cursor);
    ...
    case ZfRecordKeyPrivateWrapped:
        return zf_record_decode_bytes(
            record->private_wrapped, sizeof(record->private_wrapped), cursor);
...
static bool zf_record_decode(
    ...
    for(size_t i = 0; i < pairs; ++i) {
        ...
        if(key == ZfRecordKeyHmacSecretWrapped) {
            ...
            if(!zf_record_decode_bytes(hmac_secret_wrapped, sizeof(hmac_secret_wrapped), &cursor)) {
                goto cleanup;
            }
    ...
    if(version != ZF_STORE_VERSION || out_record->rp_id[0] == '\0' ||
       !zf_record_id_matches_file_name(
           out_record->credential_id, out_record->credential_id_len, file_name)) {
        goto cleanup;
    }
    if(saw_hmac_secret_wrapped &&
...
static bool zf_store_record_format_load_record_internal_with_buffer(
    ...
    decoded = zf_record_decode(
        buffer, size, file_name, record, &embedded_counter_floor, &has_embedded_counter_floor);
    zf_crypto_secure_zero(buffer, buffer_size);
    if(!decoded) {
    ...
    bool ok = zf_store_counter_floor_validate(
        storage, record, &embedded_counter_floor, has_embedded_counter_floor, NULL);
    zf_crypto_secure_zero(&embedded_counter_floor, sizeof(embedded_counter_floor));
    return ok;
}
...
bool zf_store_record_format_load_record_with_buffer(
    ...
    uint8_t* buffer,
    size_t buffer_size) {
    return zf_store_record_format_load_record_internal_with_buffer(
        storage, file_name, record, buffer, buffer_size, true);
}
...
bool zf_store_record_format_load_record_for_display_with_buffer(
    ...
    size_t buffer_size) {
    return zf_store_record_format_load_record_internal_with_buffer(
        storage, file_name, record, buffer, buffer_size, false);
}
...
bool zf_store_record_format_load_index_with_buffer(
    ...
    if(!zf_store_counter_floor_validate_index(
           storage,
           file_name,
           entry,
           &embedded_counter_floor,
