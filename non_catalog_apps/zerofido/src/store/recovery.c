...
static bool zf_store_recovery_record_primary_is_valid(
    ...
    valid = zf_store_record_format_load_index_with_buffer(
        context->storage, file_name, &entry, context->buffer, context->buffer_size);
    zf_crypto_secure_zero(&entry, sizeof(entry));
    return valid;
}

static bool zf_store_recovery_recover_record_backup(
    ZfStoreRecoveryCleanupContext* context,
    const char* file_name,
    const char* backup_path) {
    char record_path[128];

    if(!zf_storage_build_child_path(ZF_APP_DATA_DIR, file_name, record_path, sizeof(record_path))) {
        return false;
    }
...
static bool zf_store_recovery_recover_counter_backup(Storage* storage, const char* file_name) {
    ...
    zf_store_build_counter_floor_temp_path(
        file_name, counter_temp_path, sizeof(counter_temp_path));
    return zf_storage_recover_atomic_file(storage, counter_path, counter_temp_path);
}

static bool
    zf_store_recovery_cleanup_visitor(const char* name, const FileInfo* info, void* context) {
    ...
    if(zf_store_has_suffix(name, ".bak")) {
        ...
        if(zf_store_record_format_is_record_name(file_name)) {
            return zf_store_recovery_recover_record_backup(
                cleanup_context, file_name, backup_path);
        }
        if(zf_store_has_suffix(file_name, ".counter")) {
            ...
            if(record_name_len > 0U && record_name_len < sizeof(file_name)) {
                ...
                if(zf_store_record_format_is_record_name(file_name)) {
                    return zf_store_recovery_recover_counter_backup(
                        cleanup_context->storage, file_name);
                }
            }
        }
    }
 ...
 */
void zf_store_recovery_cleanup_temp_files_with_buffer(
    Storage* storage,
    uint8_t* buffer,
    size_t buffer_size) {
    char name[96];
    ...
    zf_storage_remove_dir_entries_with_suffix(
        storage, ZF_APP_DATA_DIR, ".tmp", name, sizeof(name), path, sizeof(path));
    zf_storage_for_each_dir_entry(
        storage, ZF_APP_DATA_DIR, name, sizeof(name), zf_store_recovery_cleanup_visitor, &context);
}
