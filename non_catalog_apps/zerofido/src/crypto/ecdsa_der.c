...
size_t zf_ecdsa_der_encode_signature(
    const uint8_t r[ZF_PUBLIC_KEY_LEN],
    const uint8_t s[ZF_PUBLIC_KEY_LEN],
    uint8_t* out,
    size_t capacity) {
    uint8_t tmp[80];
    uint8_t r_len = 0;
...
static bool zf_der_decode_length(
    const uint8_t* input,
    size_t input_len,
    size_t* header_len,
    size_t* value_len) {
...
static bool zf_der_decode_int_fixed(
    const uint8_t* input,
    size_t input_len,
    size_t* consumed,
...
bool zf_ecdsa_der_decode_signature(
    ...
    if(!zf_der_decode_int_fixed(
           &signature[offset], signature_len - offset, &consumed, raw_signature)) {
        return false;
    }
    offset += consumed;
    if(!zf_der_decode_int_fixed(
