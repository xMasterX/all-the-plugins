 ...
 */
bool zf_pin_protocol_derive_keys(
    ...
    if(pin_protocol == ZF_PIN_PROTOCOL_V1) {
        if(!zf_crypto_ecdh_shared_secret(
               &state->key_agreement, platform_x, platform_y, zf_pin_protocol_hmac_key(keys))) {
            return false;
        }
        memcpy(
...
bool zf_pin_protocol_decrypt(
    ...
    if(pin_protocol == ZF_PIN_PROTOCOL_V1) {
        if(!zf_crypto_aes256_cbc_zero_iv_decrypt(
               zf_pin_protocol_aes_key(keys), ciphertext, plaintext, ciphertext_len)) {
            return false;
        }
    ...
    if(!zf_crypto_aes256_cbc_decrypt(
           zf_pin_protocol_aes_key(keys),
           ciphertext,
           ciphertext + ZF_PIN_PROTOCOL2_IV_LEN,
           plaintext,
...
bool zf_pin_protocol_encrypt(
    ...
    if(pin_protocol == ZF_PIN_PROTOCOL_V1) {
        if(out_capacity < plaintext_len ||
           !zf_crypto_aes256_cbc_zero_iv_encrypt(
               zf_pin_protocol_aes_key(keys), plaintext, out, plaintext_len)) {
            return false;
        }
    ...
    if(!zf_crypto_aes256_cbc_encrypt(
           zf_pin_protocol_aes_key(keys),
           out,
           plaintext,
           out + ZF_PIN_PROTOCOL2_IV_LEN,
           plaintext_len)) {
        return false;
    }
...
bool zf_pin_protocol_hmac_matches(
    ...
    if(!zf_crypto_hmac_sha256_parts_with_scratch(
           scratch, key, 32, first, first_len, second, second_len, hmac)) {
        return false;
    }
