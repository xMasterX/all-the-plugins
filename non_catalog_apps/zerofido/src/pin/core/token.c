...
static void
    zf_pin_usb_diag_hex(Storage* storage, const char* label, const uint8_t* data, size_t len) {
...
void zf_pin_reset_token_metadata(ZfClientPinState* state) {
    ...
    zf_crypto_secure_zero(
        state->pin_token_permissions_rp_id_hash, sizeof(state->pin_token_permissions_rp_id_hash));
}
...
void zf_pin_set_token_permissions(
    ZfClientPinState* state,
    uint64_t permissions,
    bool permission_scoped,
    bool permission_managed,
    const char* rp_id) {
    ...
    if(!state->pin_token_permissions_rp_id_set) {
        zf_crypto_secure_zero(
            state->pin_token_permissions_rp_id_hash,
...
static void
    zf_pin_consume_up_tested_permissions(ZfClientPinState* state, uint64_t required_permissions) {
...
uint8_t zerofido_pin_require_auth(
    ...
    *uv_verified = false;
    ...
    zf_usb_diag_logf(
        storage,
        "pin auth start uv=%u hpa=%u hpp=%u proto=%u len=%u",
        uv_requested ? 1U : 0U,
        has_pin_auth ? 1U : 0U,
        has_pin_protocol ? 1U : 0U,
    ...
    zf_usb_diag_logf(
        ...
        "managed=%u rpset=%u",
        state->pin_set ? 1U : 0U,
        state->pin_token_active ? 1U : 0U,
        state->pin_auth_blocked ? 1U : 0U,
        (unsigned)state->pin_token_permissions,
        (unsigned)required_permissions,
    ...
    if(has_pin_auth) {
        ...
        if(!zf_crypto_hmac_sha256(
               state->pin_token,
               sizeof(state->pin_token),
               client_data_hash,
...
#endif
        if(!zf_crypto_constant_time_equal(
               expected, pin_auth, zf_pin_uv_auth_param_len(pin_protocol))) {
#if ZF_USB_DIAGNOSTICS
           ...
           (required_permissions & (ZF_PIN_PERMISSION_MC | ZF_PIN_PERMISSION_GA)) != 0U) {
            ...
            if(!state->pin_token_permissions_rp_id_set) {
                zf_pin_set_token_permissions(
                    state,
                    state->pin_token_permissions,
                    state->pin_token_permissions_scoped,
                    state->pin_token_permissions_managed,
                    rp_id);
            }
        }
    ...
    if(!state->pin_set) {
#if ZF_USB_DIAGNOSTICS
        zf_pin_usb_diag_status(
            storage,
            "no-pin-auth-pin-not-set",
