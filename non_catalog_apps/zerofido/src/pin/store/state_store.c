...
static void zf_pin_compute_retry_state_digest(
...
static bool zf_pin_seal_retry_state(
    const uint8_t pin_hash[ZF_PIN_HASH_LEN],
    uint8_t pin_retries,
    uint8_t pin_consecutive_mismatches,
    uint8_t flags,
    ...
    zf_pin_compute_retry_state_digest(
        pin_hash, pin_retries, pin_consecutive_mismatches, flags, digest);
...
static bool zf_pin_verify_retry_state(
    const uint8_t pin_hash[ZF_PIN_HASH_LEN],
    uint8_t pin_retries,
    uint8_t pin_consecutive_mismatches,
    uint8_t flags,
...
ZfPinLoadStatus zf_pin_state_store_load(
    ...
    if(!zf_storage_read_file(storage, ZF_PIN_FILE_PATH, raw, sizeof(raw), &raw_size)) {
        return storage_file_exists(storage, ZF_PIN_FILE_PATH) ? ZfPinLoadInvalid :
                                                                ZfPinLoadMissing;
    }
    ...
    *pin_auth_blocked = (record->base.flags & ZF_PIN_FILE_FLAG_AUTH_BLOCKED) != 0;
    ...
    if(!furi_hal_crypto_enclave_load_key(
           FURI_HAL_CRYPTO_ENCLAVE_UNIQUE_KEY_SLOT, record->base.iv)) {
        return ZfPinLoadInvalid;
    }
    ok = furi_hal_crypto_decrypt(
        record->base.encrypted_pin_hash, pin_hash, sizeof(record->base.encrypted_pin_hash));
    furi_hal_crypto_enclave_unload_key(FURI_HAL_CRYPTO_ENCLAVE_UNIQUE_KEY_SLOT);
    if(!ok) {
        return ZfPinLoadInvalid;
    }
    if(!zf_pin_verify_retry_state(
           pin_hash,
           *pin_retries,
           *pin_consecutive_mismatches,
           record->base.flags,
           record->retry_seal_iv,
...
bool zf_pin_state_store_persist(Storage* storage, const ZfClientPinState* state) {
    ...
    if(!furi_hal_crypto_enclave_load_key(FURI_HAL_CRYPTO_ENCLAVE_UNIQUE_KEY_SLOT, record.base.iv)) {
        return false;
    }
    ok = furi_hal_crypto_encrypt(
        state->pin_hash, record.base.encrypted_pin_hash, sizeof(record.base.encrypted_pin_hash));
    furi_hal_crypto_enclave_unload_key(FURI_HAL_CRYPTO_ENCLAVE_UNIQUE_KEY_SLOT);
    if(!ok) {
        return false;
    }
    if(!zf_pin_seal_retry_state(
           state->pin_hash,
           state->pin_retries,
    ...
    return zf_storage_write_file_atomic(
        storage, ZF_PIN_FILE_PATH, ZF_PIN_FILE_TEMP_PATH, (const uint8_t*)&record, sizeof(record));
}
