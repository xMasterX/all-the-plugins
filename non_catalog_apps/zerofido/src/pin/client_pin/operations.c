...
static void zf_client_pin_diag_key_exchange(
    const ZfClientPinState* state,
    ...
    zf_client_pin_diag_secret_block(
        "cmd=CP key auth x", state->key_agreement.public_x, ZF_PUBLIC_KEY_LEN);
    zf_client_pin_diag_secret_block(
        "cmd=CP key auth y", state->key_agreement.public_y, ZF_PUBLIC_KEY_LEN);
    zf_client_pin_diag_secret_block(
        "cmd=CP key platform x", request->platform_x, ZF_PUBLIC_KEY_LEN);
    zf_client_pin_diag_secret_block(
        "cmd=CP key platform y", request->platform_y, ZF_PUBLIC_KEY_LEN);
    zf_client_pin_diag_secret_block(
        "cmd=CP key hmac", zf_pin_protocol_hmac_key(keys), ZF_PUBLIC_KEY_LEN);
    zf_client_pin_diag_secret_block(
        "cmd=CP key aes", zf_pin_protocol_aes_key(keys), ZF_PUBLIC_KEY_LEN);
}
...
static bool zf_client_pin_new_pin_ciphertext_len_is_valid(
...
static uint8_t zf_client_pin_request_permission_token_consent(
    ...
    const ZfClientPinRequest* request,
    const uint8_t pin_hash_plain[ZF_PIN_HASH_LEN],
    ...
    if(!zerofido_ui_request_approval(
           app,
           ZfUiProtocolFido2,
           "Authorize",
           target_id,
...
static uint8_t zf_client_pin_decrypt_new_pin(
    const ZfClientPinRequest* request,
    uint8_t* protocol_keys,
    uint8_t* new_pin_plain,
    size_t new_pin_plain_len,
    size_t* pin_len) {
    size_t decrypted_len = 0;

    if(!zf_pin_protocol_decrypt(
       ...
       decrypted_len != new_pin_plain_len) {
        ZF_PIN_OP_DIAG(
            "cmd=CP newpin decrypt fail dec=%u exp=%u",
            (unsigned)decrypted_len,
            (unsigned)new_pin_plain_len);
        return ZF_CTAP_ERR_PIN_AUTH_INVALID;
    }
    zf_client_pin_diag_secret_block(
        "cmd=CP newpin enc", request->new_pin_enc, request->new_pin_enc_len);
...
uint8_t zf_client_pin_handle_set_pin(
    Storage* storage,
    ZfClientPinState* state,
    const ZfClientPinRequest* request,
    ...
    ZF_PIN_OP_DIAG(
        "cmd=CP-SP proto=%lu new=%u auth=%u",
        (unsigned long)request->pin_protocol,
        (unsigned)request->new_pin_enc_len,
        (unsigned)request->pin_auth_len);
    ...
    if(!zf_pin_protocol_derive_keys(
           state, request->pin_protocol, request->platform_x, request->platform_y, protocol_keys)) {
        status = ZF_CTAP_ERR_INVALID_PARAMETER;
        goto cleanup;
    }
    ...
    if(!zf_pin_protocol_hmac_matches(
           &scratch->hmac_scratch,
           request->pin_protocol,
           zf_pin_protocol_hmac_key(protocol_keys),
           request->new_pin_enc,
           request->new_pin_enc_len,
           NULL,
           0,
    ...
    status = zf_client_pin_decrypt_new_pin(
        request, protocol_keys, new_pin_plain, new_pin_plain_len, &pin_len);
    if(status != ZF_CTAP_SUCCESS) {
        goto cleanup;
    }
...
uint8_t zf_client_pin_handle_change_pin(
    Storage* storage,
    ZfClientPinState* state,
    const ZfClientPinRequest* request,
    ZfClientPinCommandScratch* scratch,
    ...
    ZF_PIN_OP_DIAG(
        "cmd=CP-CH proto=%lu new=%u hash=%u auth=%u",
        (unsigned long)request->pin_protocol,
        (unsigned)request->new_pin_enc_len,
        (unsigned)request->pin_hash_enc_len,
        (unsigned)request->pin_auth_len);
    if(!zf_pin_protocol_supported(request->pin_protocol) ||
       !zf_client_pin_new_pin_ciphertext_len_is_valid(
           request->pin_protocol, request->new_pin_enc_len, &new_pin_plain_len) ||
    ...
    zf_client_pin_diag_key_exchange(state, request, protocol_keys);
    zf_client_pin_diag_secret_block(
        "cmd=CP pin hash enc", request->pin_hash_enc, request->pin_hash_enc_len);
    if(!zf_pin_protocol_hmac_matches(
           &scratch->hmac_scratch,
           request->pin_protocol,
           zf_pin_protocol_hmac_key(protocol_keys),
...
uint8_t zf_client_pin_handle_get_pin_token(
    ...
    const ZfClientPinRequest* request,
    ZfClientPinCommandScratch* scratch,
    bool permissions_mode,
    bool require_local_consent,
    ZfTransportSessionId session_id,
    uint8_t* out,
    size_t out_capacity,
    size_t* out_len) {
    ...
    if(!zf_crypto_constant_time_equal(pin_hash_plain, state->pin_hash, ZF_PIN_HASH_LEN)) {
        ...
        zf_client_pin_diag_secret_block(
            "cmd=CP pin hash stored", state->pin_hash, ZF_PIN_HASH_LEN);
        status = zf_pin_auth_failure(storage, state);
        goto cleanup;
    }
    ...
    if(require_local_consent) {
        ...
        status = zf_client_pin_request_permission_token_consent(
            app, state, request, pin_hash_plain, session_id);
        if(status != ZF_CTAP_SUCCESS) {
            goto cleanup;
        }
    }
    ...
    status = zf_client_pin_response_token(
        encrypted_token, encrypted_token_len, out, out_capacity, out_len);
    if(status == ZF_CTAP_SUCCESS) {
        memcpy(state->pin_token, next_pin_token, sizeof(state->pin_token));
        zf_pin_set_token_permissions(
            state,
            permissions_mode ? request->permissions :
                               (ZF_PIN_PERMISSION_MC | ZF_PIN_PERMISSION_GA),
            permissions_mode,
            require_local_consent,
            permissions_mode ? request->rp_id : NULL);
