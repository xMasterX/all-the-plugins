...
static bool zf_store_index_entry_rp_hash_matches(
    ...
    return entry && matcher &&
           zf_crypto_constant_time_equal(
               entry->rp_id_hash, matcher->rp_id_hash, sizeof(matcher->rp_id_hash));
#endif
}

static bool zf_store_index_entry_matches_rp(
    Storage* storage,
...
static bool zf_store_index_is_newer(
    const ZfCredentialIndexEntry* candidate,
...
static void zf_store_insert_sorted_index(
    ...
    while(insert_at > 0 &&
          zf_store_index_is_newer(
              &store->records[index], &store->records[out_indices[insert_at - 1]])) {
...
void zf_store_index_entry_from_record(
...
bool zf_store_init_with_buffer(
    Storage* storage,
...
bool zf_store_prepare_credential(
    ...
    zf_store_record_format_hex_encode(
        record->credential_id, record->credential_id_len, record->file_name);
    ...
    if(user_display_name) {
        strncpy(
            record->user_display_name, user_display_name, sizeof(record->user_display_name) - 1);
    }
    ...
    furi_hal_random_fill_buf(
        record->hmac_secret_without_uv, sizeof(record->hmac_secret_without_uv));
 ...
 */
bool zf_store_add_record_with_buffer(
    Storage* storage,
    ZfCredentialStore* store,
    const ZfCredentialRecord* record,
    uint8_t* buffer,
...
bool zf_store_write_record_file_with_buffer(
    Storage* storage,
    const ZfCredentialRecord* record,
    uint8_t* buffer,
    size_t buffer_size) {
...
bool zf_store_remove_record_file(Storage* storage, const ZfCredentialRecord* record) {
    ...
    if(record->file_name[0] != '\0') {
        ...
        zf_store_record_format_hex_encode(
            record->credential_id, record->credential_id_len, file_name);
    }
...
bool zf_store_update_record_with_buffer(
    Storage* storage,
    ZfCredentialStore* store,
    const ZfCredentialRecord* record,
    uint8_t* buffer,
    ...
    for(size_t i = 0; i < store->count; ++i) {
        ...
        if(record->sign_count > counter_high_water &&
           !zf_store_record_format_reserve_counter_with_buffer(
               storage, record, buffer, buffer_size, &counter_high_water)) {
            return false;
        }
        ...
        if(!zf_store_record_format_write_record_with_buffer(
               storage, &record_to_write, buffer, buffer_size)) {
...
typedef bool (*ZfStoreRecordLoader)(
    Storage* storage,
 ...
 */
static bool zf_store_load_record_internal_with_buffer(
    Storage* storage,
    const ZfCredentialIndexEntry* entry,
    ZfCredentialRecord* out_record,
    uint8_t* buffer,
...
bool zf_store_load_record_with_buffer(
    Storage* storage,
    const ZfCredentialIndexEntry* entry,
    ZfCredentialRecord* out_record,
    uint8_t* buffer,
...
bool zf_store_load_record_for_display_with_buffer(
    Storage* storage,
    const ZfCredentialIndexEntry* entry,
    ZfCredentialRecord* out_record,
    uint8_t* buffer,
...
bool zf_store_load_record_by_index_with_buffer(
    Storage* storage,
    const ZfCredentialStore* store,
    size_t index,
    ZfCredentialRecord* out_record,
    ...
    if(!zf_store_load_record_with_buffer(
           storage, &store->records[index], out_record, buffer, buffer_size)) {
        return false;
    }
...
bool zf_store_load_record_by_index_for_rp_with_buffer(
    Storage* storage,
    const ZfCredentialStore* store,
    size_t index,
    const char* rp_id,
    ...
    uint8_t* buffer,
    size_t buffer_size) {
    if(!rp_id || !zf_store_load_record_by_index_with_buffer(
                     storage, store, index, out_record, buffer, buffer_size)) {
        return false;
    }
...
bool zf_store_advance_counter(
    ...
    for(size_t i = 0; i < store->count; ++i) {
        ...
        if(!zf_store_prepare_counter_advance(
               storage, entry, record, buffer, sizeof(buffer), &counter_high_water)) {
            goto cleanup;
        }
...
bool zf_store_prepare_counter_advance(
    Storage* storage,
    const ZfCredentialIndexEntry* entry,
    const ZfCredentialRecord* record,
    uint8_t* buffer,
    ...
    if(record->sign_count > counter_high_water) {
        if(!zf_store_record_format_reserve_counter_with_buffer(
               storage, record, buffer, buffer_size, &counter_high_water)) {
            return false;
        }
    }
...
bool zf_store_publish_counter_advance(
    ZfCredentialStore* store,
    const ZfCredentialRecord* record,
...
bool zf_store_remove_resident_credential_files_for_user_with_buffer(
    ...
    return zf_store_remove_credential_files_by_indices(
        storage, store, deleted_indices, found_count, deleted_count);
}
 ...
 */
bool zf_store_remove_credential_files_by_indices(
    Storage* storage,
    const ZfCredentialStore* store,
...
void zf_store_publish_deleted_indices(
    ZfCredentialStore* store,
    const uint16_t* deleted_indices,
    size_t deleted_count) {
...
ZfStoreDeleteResult zf_store_delete_record(
    Storage* storage,
    ZfCredentialStore* store,
...
bool zf_store_find_index_by_id(
    const ZfCredentialStore* store,
    const uint8_t* credential_id,
    size_t credential_id_len,
    size_t* out_index) {
...
size_t zf_store_find_by_rp_filtered(
    Storage* storage,
    const ZfCredentialStore* store,
    const char* rp_id,
 ...
 */
bool zf_store_has_matching_credential_with_buffer(
    Storage* storage,
    const ZfCredentialStore* store,
    const char* rp_id,
    ZfStoreCredentialFilter filter,
