...
static void
    zf_u2f_format_app_id(const uint8_t* request, size_t request_len, char* out, size_t out_len) {
    ...
    snprintf(
        out,
        out_len,
        "app %02x%02x%02x%02x%02x%02x%02x%02x...",
        app_id[0],
        app_id[1],
        app_id[2],
        app_id[3],
 ...
 */
static bool zf_u2f_request_approval(
    ZerofidoApp* app,
    ...
    if(!zerofido_ui_request_approval(
           app, ZfUiProtocolU2f, operation, rp_text, "Touch required", session_id, &approved)) {
        return false;
    }
...
size_t zf_u2f_adapter_handle_msg(
    ...
    validation_status = u2f_validate_request_into_response(
        request, (uint16_t)request_len, response, (uint16_t)response_capacity);
