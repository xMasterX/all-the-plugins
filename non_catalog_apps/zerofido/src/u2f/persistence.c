...
static bool u2f_data_cert_key_encrypt(uint8_t* cert_key) {
    ...
    if(u2f_data_ensure_directories(storage)) {
        ...
        state = zf_storage_write_encrypted_blob_atomic(
            storage, U2F_CERT_KEY_FILE, U2F_CERT_KEY_FILE_TMP, &spec);
    }
    furi_record_close(RECORD_STORAGE);
    return state;
}
...
bool u2f_data_bootstrap_attestation_assets(
    const uint8_t* cert,
    size_t cert_len,
    ...
    do {
        ...
        if(!zf_storage_write_file_atomic(
               storage, U2F_CERT_FILE, U2F_CERT_FILE_TMP, cert, cert_len)) {
            break;
        }
...
static bool u2f_data_device_key_compute_mac(
    const uint8_t device_key[32],
    const uint8_t iv[16],
    const uint8_t key_encrypted[48],
    uint8_t mac[32]) {
    uint8_t material[64];
    bool ok = false;
    ...
    ok = zf_crypto_hmac_sha256_parts(
        device_key,
        32,
        (const uint8_t*)U2F_DEVICE_KEY_MAC_DOMAIN,
        sizeof(U2F_DEVICE_KEY_MAC_DOMAIN) - 1U,
        material,
        sizeof(material),
        mac);
...
static bool u2f_data_device_key_write_mac(
...
static bool u2f_data_device_key_verify_mac(
    const uint8_t* plaintext,
    size_t plaintext_len,
    const uint8_t iv[16],
    const uint8_t* encrypted,
