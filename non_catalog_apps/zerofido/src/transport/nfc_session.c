...
void zf_transport_nfc_attach_arena(
...
static void zf_transport_nfc_reset_exchange_internal_locked(ZfNfcTransportState* state) {
    ...
    zf_crypto_secure_zero(
        state->rx_complete_last_payload, sizeof(state->rx_complete_last_payload));
    zf_crypto_secure_zero(
        state->rx_complete_last_response, sizeof(state->rx_complete_last_response));
 ...
 */
bool zf_transport_nfc_send_get_response(
    ...
    if(state->response_is_error) {
        const uint16_t error_status = state->error_status_word ? state->error_status_word :
    ...
    if(state->response_is_u2f) {
        ...
        if(remaining <= 2U) {
            const uint16_t status_word = ((uint16_t)arena[state->response_len - 2U] << 8) |
                                         arena[state->response_len - 1U];
            zf_transport_nfc_reset_exchange_locked(state);
        ...
        const bool ok = zf_transport_nfc_send_apdu_payload(
            state, &arena[state->response_offset], remaining, ZF_NFC_SW_SUCCESS);
        zf_transport_nfc_reset_exchange_preserving_replay_locked(state);
        return ok;
    } else {
        chunk_len = le;
    ...
    return zf_transport_nfc_send_apdu_payload(
        state,
        &arena[state->response_offset - chunk_len],
        chunk_len,
        zf_transport_nfc_status_update_sw(remaining));
}

bool zf_transport_nfc_send_deferred_response_or_wtx(
    const ZerofidoApp* app,
    ZfNfcTransportState* state) {
    uint8_t* arena = zf_transport_nfc_arena(state);
    size_t arena_capacity = zf_transport_nfc_arena_capacity(state);
    ...
    if(state->response_len + 2U <= ZF_NFC_MAX_TX_FRAME_INF_SIZE) {
        const bool sent = zf_transport_nfc_send_apdu_payload(
            state, arena, state->response_len, ZF_NFC_SW_SUCCESS);
        zf_transport_nfc_reset_exchange_locked(state);
        return sent;
    }
...
bool zf_transport_nfc_queue_request_locked(
    ZerofidoApp* app,
    ZfNfcTransportState* state,
...
bool zf_transport_nfc_handle_select(
    ZfNfcTransportState* state,
    const ZfNfcApdu* apdu,
    ...
    zf_transport_nfc_clear_tx_chain(state);
    return zf_transport_nfc_send_apdu_payload(
        state, select_response, select_response_len, ZF_NFC_SW_SUCCESS);
}

/* Publishes worker output only if it still belongs to the active NFC request generation. */
void zf_transport_nfc_store_response(
    ZerofidoApp* app,
    ZfNfcTransportState* state,
