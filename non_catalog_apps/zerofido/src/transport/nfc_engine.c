...
static bool zf_transport_nfc_format_frame_status(
    char* text,
    size_t text_len,
    const char* prefix,
    uint8_t pcb,
...
static void zf_transport_nfc_set_frame_status(
    const char* prefix,
    uint8_t pcb,
    const uint8_t* payload,
    size_t payload_len) {
    ...
    if(!zf_transport_nfc_format_frame_status(
           text, sizeof(text), prefix, pcb, payload, payload_len)) {
        return;
    }
...
#else
static inline void zf_transport_nfc_set_frame_status(
    const char* prefix,
    uint8_t pcb,
    const uint8_t* payload,
    size_t payload_len) {
...
static bool zf_transport_nfc_is_native_desfire_get_version_payload(
    const uint8_t* payload,
    size_t payload_len) {
    return payload && payload_len == 1U && payload[0] == ZF_NFC_DESFIRE_CMD_GET_VERSION;
}

static bool zf_transport_nfc_is_native_desfire_additional_frame(
    const uint8_t* payload,
    size_t payload_len) {
...
static NfcCommand zf_transport_nfc_finish_unlocked(
    ZerofidoApp* app,
    bool connected,
    bool refresh_status,
    NfcCommand command) {
...
static NfcCommand zf_transport_nfc_sleep_probe_locked(
    ZerofidoApp* app,
...
static bool zf_transport_nfc_should_sleep_post_success_probe_locked(
    ZfNfcTransportState* state,
    const uint8_t* payload,
    size_t payload_len) {
...
static NfcCommand zf_transport_nfc_handle_empty_i_block_locked(
    ZerofidoApp* app,
    ZfNfcTransportState* state,
    uint8_t request_pcb,
    bool refresh_status) {
    ...
    const bool sent = replayed ||
                      (!replay_available && zf_transport_nfc_send_r_ack(state, request_pcb));

    zf_transport_nfc_set_frame_status(
        replayed ? "NFC I-empty replay" : "NFC I-empty", request_pcb, NULL, 0U);
    furi_mutex_release(app->ui_mutex);
    if(!sent) {
...
static NfcCommand zf_transport_nfc_handle_recovery_r_nak_locked(
    ZerofidoApp* app,
    ZfNfcTransportState* state,
    const uint8_t* frame,
    size_t frame_len,
...
static bool zf_transport_nfc_sync_i_block_header(
    ZfNfcTransportState* state,
    const uint8_t* frame,
    size_t frame_len,
...
static void zf_transport_nfc_remember_complete_i_block_locked(
    ZfNfcTransportState* state,
    uint8_t request_pcb,
    const uint8_t* payload,
    size_t payload_len) {
...
static bool zf_transport_nfc_is_duplicate_complete_i_block_locked(
    const ZfNfcTransportState* state,
    uint8_t request_pcb,
    const uint8_t* payload,
    size_t payload_len) {
...
static bool zf_transport_nfc_replay_complete_i_block_response_locked(ZfNfcTransportState* state) {
    ...
    sent = zf_transport_nfc_send_frame(
        state, state->rx_complete_last_response, state->rx_complete_last_response_len);
    if(sent) {
...
static ZfNfcRxChainAppendResult zf_transport_nfc_append_rx_chain_payload_locked(
    ZerofidoApp* app,
    ZfNfcTransportState* state,
    uint8_t request_pcb,
    const uint8_t* payload,
...
static void
    zf_transport_nfc_trace_apdu_after_response(const uint8_t* apdu_bytes, size_t apdu_len) {
    (void)apdu_bytes;
    (void)apdu_len;
}

static bool zf_transport_nfc_handle_native_desfire_locked(
    ZerofidoApp* app,
    ZfNfcTransportState* state,
    bool additional_frame) {
    static const uint8_t version_more[] = {
        ZF_NFC_DESFIRE_STATUS_MORE,
        0x04,
        0x01,
        0x01,
        0x01,
    ...
    if(!additional_frame) {
        ...
        return zf_transport_nfc_send_iso_response_preserving_replay(
            state, version_more, sizeof(version_more), false);
    }
    if(state->desfire_probe_frame >= 2U) {
        /* Shim-local discovery progress only; do not touch FIDO applet/session state. */
        state->desfire_probe_frame = 0U;
        return zf_transport_nfc_send_iso_response_preserving_replay(
            state, version_final, sizeof(version_final), false);
    }
...
static NfcCommand zf_transport_nfc_handle_rats_locked(
    ZerofidoApp* app,
    ZfNfcTransportState* state,
    const uint8_t* frame,
    size_t frame_len) {
    ...
    furi_mutex_release(app->ui_mutex);
    zf_transport_nfc_set_frame_status(
        sent ? "NFC RATS" : "NFC RATS fail", frame[0], &frame[1], frame_len - 1U);
...
static NfcCommand zf_transport_nfc_handle_iso4_payload_locked(
    ...
    if(!request_chained && !state->command_chain_active && payload_len == 0U) {
        return zf_transport_nfc_handle_empty_i_block_locked(
            app, state, request_pcb, refresh_status);
    }

    if(!request_chained && !state->command_chain_active &&
       zf_transport_nfc_is_duplicate_complete_i_block_locked(
           state, request_pcb, payload, payload_len)) {
        const bool replayed = zf_transport_nfc_replay_complete_i_block_response_locked(state);

        zf_transport_nfc_set_frame_status(
            replayed ? "NFC I-dup replay" : "NFC I-dup", request_pcb, payload, payload_len);
        furi_mutex_release(app->ui_mutex);
        return zf_transport_nfc_finish_unlocked(app, true, refresh_status, NfcCommandContinue);
    }
    ...
    if(!request_chained && !state->command_chain_active &&
       (zf_transport_nfc_is_native_desfire_get_version_payload(payload, payload_len) ||
        ...
        if(zf_transport_nfc_handle_native_desfire_locked(
            ...
            zf_transport_nfc_remember_complete_i_block_locked(
                state, request_pcb, payload, payload_len);
        }
    ...
    if(zf_transport_nfc_request_worker_active(state)) {
        if(zf_transport_nfc_send_apdu_payload(
    ...
    if(request_chained) {
        ...
        if(append_result == ZfNfcRxChainAppendDuplicate) {
            ...
            if(duplicate_count == 1U) {
                zf_transport_nfc_set_frame_status(
                    "NFC I-chain dup", request_pcb, payload, payload_len);
            }
            sent = zf_transport_nfc_send_r_ack(state, request_pcb);
            if(!sent) {
                zf_transport_nfc_set_frame_status(
                    "NFC I-chain dup fail", request_pcb, payload, payload_len);
            }
            (void)refresh_status;
            return NfcCommandContinue;
        }
        ...
        if(append_result == ZfNfcRxChainAppendStalled) {
            ...
            furi_mutex_release(app->ui_mutex);
            zf_transport_nfc_set_frame_status(
                "NFC I-chain stall", request_pcb, payload, payload_len);
        ...
        if(!sent) {
            zf_transport_nfc_set_frame_status(
                "NFC I-chain fail", request_pcb, payload, payload_len);
        }
        (void)refresh_status;
        return NfcCommandContinue;
    }
    ...
    if(state->command_chain_active) {
        ...
        zf_transport_nfc_set_frame_status(
            "NFC I-block", request_pcb, assembled_frame, assembled_len);
...
NfcCommand zf_transport_nfc_event_callback(NfcGenericEvent event, void* context) {
    ...
    if(frame_len == 1U && zf_transport_nfc_is_one_byte_recovery_r_nak(frame[0])) {
        return zf_transport_nfc_handle_recovery_r_nak_locked(
            app, state, frame, frame_len, refresh_status);
    }
    ...
    if(zf_transport_nfc_is_pps_frame(frame, frame_len)) {
        ...
        furi_mutex_release(app->ui_mutex);
        zf_transport_nfc_set_frame_status(
            pps_defer ? "NFC PPS defer" : (sent ? "NFC PPS ack" : "NFC PPS fail"),
            frame[0],
            &frame[1],
            frame_len - 1U);
        return zf_transport_nfc_finish_unlocked(app, true, refresh_status, NfcCommandContinue);
    ...
    if(zf_transport_nfc_is_r_block(frame[0])) {
        ...
        if(!chain_ack_sent) {
            zf_transport_nfc_set_frame_status(
                r_nack ? (sent ? "NFC R-NAK replay" : "NFC R-NAK") : "NFC R-ACK",
                frame[0],
                &frame[1],
                frame_len - 1U);
        }
    ...
    if(zf_transport_nfc_is_s_block(frame[0])) {
        ...
        furi_mutex_release(app->ui_mutex);
        zf_transport_nfc_set_frame_status(
            s_deselect ? "NFC S-deselect" : (s_wtx ? "NFC S-wtx" : "NFC S-block"),
            frame[0],
            &frame[1],
            frame_len - 1U);
        if(s_deselect) {
        ...
        return zf_transport_nfc_finish_unlocked(
            app, !s_deselect, true, s_deselect ? NfcCommandSleep : NfcCommandContinue);
    }
