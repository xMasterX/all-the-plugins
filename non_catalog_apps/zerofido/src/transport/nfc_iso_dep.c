...
static void zf_transport_nfc_trace_cache_state(
    const char* event,
    ...
    zf_transport_nfc_trace_format(
        "iso-cache %s pcb=%02X len=%u valid=%u cached=%u frame=%u",
        event,
        pcb,
        (unsigned)data_len,
...
static void zf_transport_nfc_cache_last_iso_response(
    ZfNfcTransportState* state,
    const uint8_t* data,
    size_t data_len) {
    if(!state || (!data && data_len > 0U) || data_len > sizeof(state->iso4_last_tx)) {
        zf_transport_nfc_trace_cache_state(
            "skip", state, data && data_len > 0U ? data[0] : 0U, data_len);
        return;
    }
...
static void
    zf_transport_nfc_snapshot_replay(ZfNfcTransportState* state, ZfNfcReplaySnapshot* snapshot) {
    if(!snapshot) {
        return;
    }
...
static void zf_transport_nfc_restore_replay(
    ZfNfcTransportState* state,
...
bool zf_transport_nfc_send_raw_bits(
    ZfNfcTransportState* state,
    const uint8_t* data,
    size_t bit_len) {
 ...
 */
bool zf_transport_nfc_send_iso_response(
    ZfNfcTransportState* state,
    const uint8_t* data,
    size_t data_len,
    bool chaining) {
 ...
 */
bool zf_transport_nfc_send_iso_response_preserving_replay(
    ZfNfcTransportState* state,
    const uint8_t* data,
    size_t data_len,
    bool chaining) {
...
bool zf_transport_nfc_replay_last_iso_response(ZfNfcTransportState* state) {
    ...
    if(state->iso4_last_tx_valid && state->iso4_last_tx_len > 0U &&
       state->iso4_last_tx_len <= sizeof(state->iso4_last_tx)) {
        ...
        zf_transport_nfc_trace_cache_state(
            sent ? "replay-array" : "replay-array-fail",
            state,
            state->iso4_last_tx[0],
            state->iso4_last_tx_len);
        return sent;
    }
...
static void zf_transport_nfc_build_tx_chain_chunk(
    const ZfNfcTransportState* state,
    uint8_t* chunk,
    size_t chunk_len) {
...
#if defined(ZF_RELEASE_DIAGNOSTICS) && ZF_RELEASE_DIAGNOSTICS
        zf_transport_nfc_trace_format(
            "tx-chain send fail len=%u rem=%u chain=%u",
            (unsigned)chunk_len,
            (unsigned)remaining,
            chaining ? 1U : 0U);
#endif
...
bool zf_transport_nfc_begin_chained_apdu_payload(
    ZfNfcTransportState* state,
    const uint8_t* data,
    size_t data_len,
...
bool zf_transport_nfc_send_apdu_payload(
    ZfNfcTransportState* state,
    const uint8_t* data,
    size_t data_len,
    uint16_t status_word) {
...
bool zf_transport_nfc_send_apdu_payload_preserving_replay(
    ZfNfcTransportState* state,
    const uint8_t* data,
    size_t data_len,
    uint16_t status_word) {
