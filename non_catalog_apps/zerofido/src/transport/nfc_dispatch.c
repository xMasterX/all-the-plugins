...
static bool zf_transport_nfc_send_desfire_version_part(
    ZerofidoApp* app,
...
static bool
    zf_transport_nfc_send_desfire_version_terminal(ZerofidoApp* app, ZfNfcTransportState* state) {
...
static bool zf_transport_nfc_begin_ctap_msg(
    ZerofidoApp* app,
    ZfNfcTransportState* state,
    const ZfNfcApdu* apdu) {
...
static void
    zf_transport_nfc_force_get_info_nfc_capabilities(ZfResolvedCapabilities* capabilities) {
    if(!capabilities) {
        return;
    }
...
static bool zf_transport_nfc_send_get_info_response(
    ZerofidoApp* app,
    ZfNfcTransportState* state,
    ...
    ZF_NFC_CTAP_DIAG(
        "GI caps f21=%u pin=%u p2=%u token=%u mcu=%u rk=1 nfc=%u",
        capabilities.advertise_fido_2_1 ? 1U : 0U,
        client_pin_set ? 1U : 0U,
    ...
    status = zf_ctap_build_get_info_response(
        &capabilities, client_pin_set, response + 1U, body_capacity, &body_len);
    ...
    ZF_NFC_CTAP_DIAG(
        "done cmd=GI status=%s body=%u",
        zf_transport_nfc_ctap_status_name(status),
        (unsigned)body_len);

    if(response_len + 2U <= ZF_NFC_MAX_TX_FRAME_INF_SIZE) {
        return zf_transport_nfc_send_apdu_payload(
            state, response, response_len, ZF_NFC_SW_SUCCESS);
    }
    return zf_transport_nfc_begin_chained_apdu_payload(
        state, response, response_len, ZF_NFC_SW_SUCCESS);
}

static bool zf_transport_nfc_send_ctap2_direct(
    ZerofidoApp* app,
    ZfNfcTransportState* state,
...
static bool zf_transport_nfc_send_u2f_response(
...
static inline size_t zf_transport_nfc_run_u2f_adapter(
    ZerofidoApp* app,
    ZfNfcTransportState* state,
    size_t u2f_request_len,
    const char* success_diag,
    const char* error_diag) {
    const size_t response_len = zf_u2f_adapter_handle_msg(
        app,
        state->session_id,
        app->transport_arena,
        u2f_request_len,
        app->transport_arena,
        ZF_TRANSPORT_ARENA_SIZE);
    UNUSED(success_diag);
    UNUSED(error_diag);
...
static bool zf_transport_nfc_send_encoded_u2f_immediate(
    ZerofidoApp* app,
    ZfNfcTransportState* state,
    const ZfNfcApdu* apdu,
    size_t u2f_request_len,
    ...
    response_len =
        zf_transport_nfc_run_u2f_adapter(app, state, u2f_request_len, success_diag, error_diag);
    ...
    return zf_transport_nfc_send_u2f_response(
        app, state, apdu, app->transport_arena, response_len);
}

static bool zf_transport_nfc_send_u2f_immediate(
    ZerofidoApp* app,
    ZfNfcTransportState* state,
    const ZfNfcApdu* apdu,
    size_t u2f_request_len,
...
static bool zf_transport_nfc_send_u2f_version_immediate(
    ZerofidoApp* app,
    ZfNfcTransportState* state,
    const ZfNfcApdu* apdu,
    bool* handled) {
...
static bool zf_transport_nfc_send_u2f_immediate_without_presence(
    ZerofidoApp* app,
    ZfNfcTransportState* state,
    const ZfNfcApdu* apdu,
    bool* handled) {
    ...
    response_len = u2f_validate_request_into_response(
        app->transport_arena,
        (uint16_t)u2f_request_len,
        app->transport_arena,
        ZF_TRANSPORT_ARENA_SIZE);
    if(response_len != 0U) {
        *handled = true;
    ...
    if(u2f_request_needs_user_presence(
           app->transport_arena, (uint16_t)u2f_request_len, &operation)) {
        return false;
    }
...
static bool zf_transport_nfc_finish_ctap2_msg(
    ZerofidoApp* app,
    ZfNfcTransportState* state,
    bool caller_holds_ui_mutex,
    bool request_was_extended_apdu) {
    ...
    if(!ctap_get_response_supported) {
        ...
        return zf_transport_nfc_send_ctap2_direct(
            app,
            state,
            app->transport_arena,
            state->request_len,
    ...
    if(!zf_transport_nfc_queue_request_locked(
           app, state, ZfNfcRequestKindCtap2, app->transport_arena, state->request_len)) {
 ...
 */
static bool zf_transport_nfc_handle_apdu_internal(
    ZerofidoApp* app,
    ZfNfcTransportState* state,
    ...
    zf_transport_nfc_attach_arena(
        state, app->transport_arena, zf_app_transport_arena_capacity(app));
    zf_runtime_get_effective_capabilities(app, &capabilities);
    ...
    if(raw_cla == 0x90U && apdu.ins == 0x60U && apdu.p1 == 0x00U && apdu.p2 == 0x00U) {
        zf_transport_nfc_note_ui_stage_locked(
            state,
            state->applet_selected ? ZfNfcUiStageAppletSelected : ZfNfcUiStageAppletWaiting);
...
#if ZF_RELEASE_DIAGNOSTICS
        snprintf(
            text, sizeof(text), "APDU %02X %02X %02X %02X", apdu.cla, apdu.ins, apdu.p1, apdu.p2);
        ZF_NFC_DIAG_EVENT(text);
#endif
        return zf_transport_nfc_send_status_word(state, ZF_NFC_SW_CONDITIONS_NOT_SATISFIED);
    }
    ...
    if(apdu.cla == 0x80U && apdu.ins == ZF_NFC_INS_CTAP_MSG && !apdu.chained &&
       apdu.data_len == 1U && apdu.data && apdu.data[0] == ZfCtapeCmdGetInfo) {
        ...
        return zf_transport_nfc_send_get_info_response(
            app, state, &capabilities, caller_holds_ui_mutex);
    }
    ...
    if(apdu.cla == 0x00U) {
        ...
        return zf_transport_nfc_send_u2f_immediate(
            app, state, &apdu, u2f_request_len, caller_holds_ui_mutex);
    }
