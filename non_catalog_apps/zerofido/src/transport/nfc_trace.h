...
static inline void
    zf_transport_nfc_trace_bytes(const char* label, const uint8_t* data, size_t data_len) {
    ...
    for(size_t i = 0; i < preview_len; ++i) {
        int written = snprintf(
            &hex[hex_offset], sizeof(hex) - hex_offset, "%s%02X", i == 0U ? "" : " ", data[i]);
    ...
    zf_transport_nfc_trace_format(
        "%s len=%u head=%s%s", name, (unsigned)data_len, hex, data_len > preview_len ? " ..." : "");
}
...
static inline void zf_transport_nfc_trace_apdu_header(
...
static inline void
    zf_transport_nfc_trace_apdu_tx(const uint8_t* data, size_t data_len, uint16_t status_word) {
    (void)data;
