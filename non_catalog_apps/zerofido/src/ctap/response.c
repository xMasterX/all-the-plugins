...
static bool zf_encode_cose_key(
    ...
    bool ok = zf_cbor_encode_p256_cose_key(
        &enc,
        -7,
        record->public_x,
        sizeof(record->public_x),
        record->public_y,
        sizeof(record->public_y));
    if(!ok) {
        return false;
    }
...
static bool zf_encode_make_credential_extensions(
    uint8_t cred_protect,
    bool include_cred_protect,
    bool include_hmac_secret,
    bool hmac_secret_created,
    uint8_t* out,
    size_t out_capacity,
    size_t* out_len) {
    ZfCborEncoder enc;
 ...
 */
static size_t zf_build_auth_data(
    const char* rp_id,
    bool user_present,
    bool user_verified,
    bool include_attested_data,
    bool include_extension_data,
    const ZfCredentialRecord* record,
    uint32_t sign_count,
...
static bool zf_make_credential_auth_data_size(
    const ZfCredentialRecord* record,
    size_t cose_len,
    size_t extension_data_len,
    size_t* out_len) {
...
uint8_t zf_ctap_build_none_make_credential_response_with_scratch(
    ...
    if((include_cred_protect || include_hmac_secret) && !zf_encode_make_credential_extensions(
                                                            record->cred_protect,
                                                            include_cred_protect,
                                                            include_hmac_secret,
                                                            record->hmac_secret,
    ...
    bool ok = zf_cbor_encode_map(&enc, 3) && zf_cbor_encode_uint(&enc, 1) &&
              ...
              zf_build_auth_data(
                  ...
                  user_verified,
                  true,
                  include_cred_protect || include_hmac_secret,
                  record,
                  record->sign_count,
                  scratch->extension_data,
                  extension_data_len,
...
uint8_t zf_ctap_build_packed_make_credential_response_with_scratch(
    ...
    if(!zf_attestation_ensure_ready() || !zf_attestation_get_leaf_cert_der_len(&cert_len) ||
       !zf_attestation_sign_parts(
           auth_data,
           auth_data_len,
           client_data_hash,
           ZF_CLIENT_DATA_HASH_LEN,
           scratch->signature,
           sizeof(scratch->signature),
           &signature_len)) {
...
uint8_t zf_ctap_build_assertion_response_with_scratch(
    ZfAssertionResponseScratch* scratch,
