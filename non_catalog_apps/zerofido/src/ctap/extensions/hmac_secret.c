...
uint8_t zf_ctap_hmac_secret_parse_get_assertion_input(
    ...
    for(size_t i = 0; i < pairs; ++i) {
        ...
        case 3:
            if(!zf_ctap_cbor_read_bytes_copy(
                   cursor,
                   request->assertion.hmac_secret_salt_auth,
...
uint8_t zf_ctap_hmac_secret_build_extension(
    ...
    *out_len = 0;
    ...
    if(!zf_pin_protocol_supported(request->hmac_secret_pin_protocol) ||
       !zf_pin_protocol_derive_keys(
           pin_state,
           request->hmac_secret_pin_protocol,
           request->hmac_secret_platform_x,
           request->hmac_secret_platform_y,
           scratch->protocol_keys)) {
    ...
    if(!zf_pin_protocol_hmac_matches(
           ...
           request->hmac_secret_salt_enc_len,
           NULL,
           0,
