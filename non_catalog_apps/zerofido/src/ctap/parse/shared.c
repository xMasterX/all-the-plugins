...
bool zf_ctap_cbor_read_bytes_copy(
    ZfCborCursor* cursor,
    uint8_t* out,
    size_t out_capacity,
    size_t* out_size) {
...
bool zf_ctap_parse_cose_p256_key_agreement(
    ZfCborCursor* cursor,
...
uint8_t zf_ctap_parse_options_map(
    ZfCborCursor* cursor,
    bool* up,
    bool* has_up,
    bool* uv,
...
uint8_t zf_ctap_parse_make_credential_extensions_map(
    ZfCborCursor* cursor,
...
static bool zf_ctap_parse_credential_descriptor(
    ZfCborCursor* cursor,
    ...
    for(size_t i = 0; i < items; ++i) {
        ...
        for(size_t j = 0; j < pairs; ++j) {
            ...
            case ZfCtapTextKeyType: {
                ...
                have_type = true;
                public_key = zf_ctap_classify_text_key(value, value_size) ==
...
uint8_t zf_ctap_parse_descriptor_array(ZfCborCursor* cursor, ZfCredentialDescriptorList* list) {
    ...
    for(size_t j = 0; j < items; ++j) {
        ...
        for(size_t i = 0; i < list->count; ++i) {
            ...
            if(memcmp(
                   existing->credential_id_digest,
                   parsed.credential_id_digest,
...
bool zf_ctap_descriptor_list_contains_id(
    const ZfCredentialDescriptorList* list,
