...
static uint8_t zf_parse_make_credential_client_data_hash(
    ZfCborCursor* cursor,
    ZfMakeCredentialRequest* request) {
    size_t hash_len = 0;
    if(!zf_ctap_cbor_read_bytes_copy(
           cursor, request->client_data_hash, sizeof(request->client_data_hash), &hash_len) ||
...
static uint8_t
    zf_parse_make_credential_rp(ZfCborCursor* cursor, ZfMakeCredentialRequest* request) {
    size_t rp_pairs = 0;
    bool saw_id = false;
    bool saw_name = false;
    bool saw_icon = false;
...
static uint8_t
    zf_parse_make_credential_user(ZfCborCursor* cursor, ZfMakeCredentialRequest* request) {
    size_t user_pairs = 0;
    bool saw_id = false;
    bool saw_name = false;
    ...
    for(size_t j = 0; j < user_pairs; ++j) {
        ...
        case ZfCtapTextKeyId:
            ...
            if(!zf_ctap_cbor_read_bytes_copy(
                   cursor, request->user_id, sizeof(request->user_id), &request->user_id_len)) {
                return ZF_CTAP_ERR_INVALID_CBOR;
            }
        ...
        case ZfCtapTextKeyName:
            ...
            if(!zf_ctap_cbor_read_text_copy(
                   cursor, request->user_name, sizeof(request->user_name))) {
                return ZF_CTAP_ERR_INVALID_CBOR;
            }
            saw_name = true;
            continue;
        ...
        case ZfCtapTextKeyDisplayName:
            ...
            if(!zf_ctap_cbor_read_text_copy(
                   cursor, request->user_display_name, sizeof(request->user_display_name))) {
                return ZF_CTAP_ERR_INVALID_CBOR;
            }
...
static uint8_t zf_parse_make_credential_pubkey_cred_params(
    ZfCborCursor* cursor,
    ZfMakeCredentialRequest* request) {
...
static uint8_t
    zf_parse_make_credential_exclude_list(ZfCborCursor* cursor, ZfMakeCredentialRequest* request) {
...
static uint8_t
    zf_parse_make_credential_extensions(ZfCborCursor* cursor, ZfMakeCredentialRequest* request) {
    return zf_ctap_parse_make_credential_extensions_map(
        cursor,
        &request->has_cred_protect,
        &request->cred_protect,
        &request->hmac_secret_requested);
}

static uint8_t
    zf_parse_make_credential_options(ZfCborCursor* cursor, ZfMakeCredentialRequest* request) {
    return zf_ctap_parse_options_map(
        cursor,
        &request->up,
        &request->has_up,
        &request->uv,
...
static uint8_t
    zf_parse_make_credential_pin_auth(ZfCborCursor* cursor, ZfMakeCredentialRequest* request) {
    if(!zf_ctap_cbor_read_bytes_copy(
...
static uint8_t
    zf_parse_make_credential_pin_protocol(ZfCborCursor* cursor, ZfMakeCredentialRequest* request) {
...
static uint8_t zf_parse_make_credential_attestation_formats(
    ZfCborCursor* cursor,
    ZfMakeCredentialRequest* request) {
    size_t count = 0;
...
static uint8_t zf_parse_make_credential_field(
    ZfCborCursor* cursor,
    ZfMakeCredentialRequest* request,
    uint64_t key) {
    switch(key) {
...
static uint8_t zf_validate_make_credential_request(
    const ZfCborCursor* cursor,
