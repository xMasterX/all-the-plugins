...
static uint8_t
    zf_parse_get_assertion_extensions(ZfCborCursor* cursor, ZfGetAssertionRequest* request) {
    size_t pairs = 0;
 ...
 */
uint8_t
    zf_ctap_parse_get_assertion(const uint8_t* data, size_t size, ZfGetAssertionRequest* request) {
    ZfCborCursor cursor;
    size_t pairs = 0;
    ...
    for(size_t i = 0; i < pairs; ++i) {
        ...
        case 1:
            if(!zf_ctap_cbor_read_text_copy(
                   &cursor, request->assertion.rp_id, sizeof(request->assertion.rp_id))) {
                return ZF_CTAP_ERR_INVALID_CBOR;
            }
            break;
        case 2: {
        ...
        case 5: {
            uint8_t status = zf_ctap_parse_options_map(
                &cursor,
                &request->up,
                &request->has_up,
                &request->uv,
