...
_Static_assert(
    sizeof(ZfGetAssertionScratch) <= ZF_COMMAND_SCRATCH_SIZE,
...
static uint8_t zf_prepare_assertion_response(
    ...
    status = zf_ctap_hmac_secret_build_extension(
        ...
        record,
        uv_verified,
        &response_scratch->hmac_secret,
        response_scratch->extension_data,
        sizeof(response_scratch->extension_data),
        &extension_data_len);
...
static bool zf_ctap_index_entry_matches_record(
...
static bool zf_ctap_snapshot_index_entry_locked(
    ZerofidoApp* app,
    size_t record_index,
...
static bool zf_ctap_selected_record_still_current_locked(
    ZerofidoApp* app,
    size_t record_index,
    const ZfCredentialRecord* record) {
    if(!app->store.records || record_index >= app->store.count) {
...
static size_t zf_ctap_resolve_assertion_matches_snapshot(
    ZerofidoApp* app,
    ...
    match_count = zf_ctap_resolve_assertion_matches(
        app->storage, &app->store, request, uv_verified, matches);
    furi_mutex_release(app->ui_mutex);
    return match_count;
}

static bool zf_ctap_load_assertion_record(
    ZerofidoApp* app,
    size_t record_index,
    const char* rp_id,
...
static uint8_t zf_ctap_publish_assertion_counter(
    ...
    const ZfGetAssertionRequest* request,
    bool uv_verified,
    const uint16_t* match_indices,
    size_t match_count,
    bool seed_assertion_queue,
    uint8_t* store_io,
    size_t store_io_size) {
    ...
    if(!zf_store_prepare_counter_advance(
           app->storage, entry, record, store_io, store_io_size, &prepared_counter_high_water)) {
        return ZF_CTAP_ERR_OTHER;
    }
    ...
    if(seed_assertion_queue) {
        zf_ctap_assertion_queue_seed(
            app, session_id, request, uv_verified, match_indices, match_count);
    }
    furi_mutex_release(app->ui_mutex);
...
static uint8_t zf_ctap_load_and_prepare_assertion(
    ...
    size_t out_capacity,
    size_t* out_len) {
    if(!zf_ctap_load_assertion_record(
           app, record_index, request->rp_id, record, entry, store_io, store_io_size)) {
        return ZF_CTAP_ERR_NO_CREDENTIALS;
    }
    return zf_prepare_assertion_response(
        request,
        pin_state,
        record,
        user_present,
...
static uint8_t zf_ctap_finish_assertion(
    ...
    bool include_user_details,
    bool include_user_selected,
    bool poll_before_publish,
    bool* maintenance_acquired,
    uint8_t* out,
    size_t out_capacity,
    size_t* out_len) {
    uint32_t next_sign_count = 0;
    ...
    *maintenance_acquired = true;
    ...
    if(resolve_match) {
        match_count = zf_ctap_resolve_assertion_matches_snapshot(
 ...
 */
uint8_t zf_ctap_handle_get_assertion(
    ZerofidoApp* app,
    ZfTransportSessionId session_id,
    const uint8_t* data,
    size_t data_len,
    ...
    if(request->has_pin_auth && request->pin_auth_len == 0) {
        status = zf_ctap_handle_empty_pin_auth_probe(
            app, session_id, "Authenticate", request->assertion.rp_id, "Touch required");
        goto cleanup;
    }
    ...
    status = zf_ctap_require_pin_auth_with_state(
        app,
        &scratch->pin_state,
        request->has_uv && request->uv,
        request->has_pin_auth,
        request->assertion.client_data_hash,
        request->pin_auth,
    ...
    if(!uses_allow_list && match_count > 1) {
        ...
        status = zf_ctap_request_assertion_selection(
            app,
            request->assertion.rp_id,
            matches,
            match_count,
    ...
    status = zf_ctap_request_approval(
        app, "Authenticate", request->assertion.rp_id, "Touch required", session_id);
    if(status != ZF_CTAP_SUCCESS) {
        goto cleanup;
    }
