...
static bool zf_ctap_queue_entry_matches_record(
 ...
 */
void zf_ctap_assertion_queue_seed(
    ZerofidoApp* app,
    ...
    memcpy(
        app->assertion_queue.record_indices,
        match_indices,
        sizeof(app->assertion_queue.record_indices[0]) * match_count);
}

static bool zf_ctap_assertion_queue_snapshot_entry_locked(
    ZerofidoApp* app,
    size_t record_index,
...
static bool zf_ctap_assertion_queue_still_current_locked(
    ZerofidoApp* app,
    ZfTransportSessionId session_id,
    size_t queue_index,
    size_t record_index,
...
static void zf_ctap_assertion_queue_clear_if_current(
    ZerofidoApp* app,
    ZfTransportSessionId session_id,
    size_t queue_index,
    size_t record_index,
    ...
    const char* rp_id) {
    furi_mutex_acquire(app->ui_mutex, FuriWaitForever);
    if(zf_ctap_assertion_queue_still_current_locked(
           app, session_id, queue_index, record_index, count, rp_id)) {
        zf_ctap_assertion_queue_clear(app);
    }
    furi_mutex_release(app->ui_mutex);
}
 ...
 */
uint8_t zf_ctap_assertion_queue_handle_next(
    ZerofidoApp* app,
    ZfTransportSessionId session_id,
    uint8_t* out,
    size_t out_capacity,
    ...
    _Static_assert(
        sizeof(ZfAssertionQueueScratch) <= ZF_COMMAND_SCRATCH_SIZE,
    ...
    if(!zf_store_load_record_with_buffer(
       ...
       !zf_ctap_queue_entry_matches_record(&scratch->entry, &scratch->record)) {
        status = ZF_CTAP_ERR_NOT_ALLOWED;
        zf_ctap_assertion_queue_clear_if_current(
    ...
    status = zf_ctap_hmac_secret_build_extension(
        ...
        &scratch->record,
        uv_verified,
        &scratch->work.response.hmac_secret,
        scratch->work.response.extension_data,
        sizeof(scratch->work.response.extension_data),
        &extension_data_len);
    if(status != ZF_CTAP_SUCCESS) {
        goto cleanup;
    }
    ...
    if(!zf_store_publish_counter_advance(
           &app->store, &scratch->record, prepared_counter_high_water)) {
        zf_ctap_assertion_queue_clear(app);
        status = ZF_CTAP_ERR_OTHER;
        furi_mutex_release(app->ui_mutex);
        goto cleanup;
