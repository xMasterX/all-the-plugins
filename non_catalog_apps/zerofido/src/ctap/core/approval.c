...
static uint8_t
    zf_ctap_status_from_interaction_state(ZfApprovalState state, bool timeout_is_denied) {
    switch(state) {
...
uint8_t zf_ctap_request_approval(
    ZerofidoApp* app,
    ...
    if(!zerofido_ui_request_approval(
           app, ZfUiProtocolFido2, operation, rp_id, user_text, session_id, &approved)) {
...
uint8_t zf_ctap_request_assertion_selection(
    ZerofidoApp* app,
    const char* rp_id,
    ...
    if(!zerofido_ui_request_assertion_selection(
           app, rp_id, match_indices, match_count, session_id, selected_record_index)) {
 ...
 */
uint8_t zf_ctap_handle_empty_pin_auth_probe(
    ZerofidoApp* app,
    ZfTransportSessionId session_id,
    const char* operation,
    const char* rp_id,
