...
#else
static inline void
    zf_cbor_cursor_init_inline(ZfCborCursor* cursor, const uint8_t* data, size_t size) {
    cursor->ptr = data;
    cursor->end = data + size;
}
#define zf_cbor_cursor_init(cursor, data, size) \
    zf_cbor_cursor_init_inline((cursor), (data), (size))
#endif
