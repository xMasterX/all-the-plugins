...
static unsigned char explosion_bits[] = {0x80, 0x00, 0x02, 0xf8, 0x20, 0x20, 0x21, 0xf8, 0x00,
                                         0x25, 0x10, 0xfa, 0x21, 0x02, 0x89, 0xf9, 0x06, 0xb0,
                                         0x60, 0xf8, 0x18, 0x81, 0x10, 0xf8, 0x60, 0x4e, 0xec,
                                         0xfb, 0xa0, 0x50, 0x00, 0xf8, 0x46, 0x92, 0x72, 0xf9,
                                         0x00, 0x9c, 0x00, 0xf8, 0x00, 0x13, 0x1d, 0xf8, 0x40,
                                         0x88, 0x60, 0xf8, 0x18, 0x8c, 0xc0, 0xf9};

#define fighter_jet_straight_width  27
#define fighter_jet_straight_height 13
static unsigned char fighter_jet_straight_bits[] = {
    0x00, 0x20, 0x00, 0xf8, 0x00, 0x50, 0x00, 0xf8, 0x00, 0xa8, 0x00, 0xf8, 0x00,
    0x74, 0x01, 0xf8, 0x00, 0x72, 0x03, 0xf8, 0x00, 0x01, 0x04, 0xf8, 0xc0, 0x24,
    0x19, 0xf8, 0x30, 0x54, 0x61, 0xf8, 0x0c, 0x04, 0x81, 0xf9, 0x02, 0x8e, 0x03,
    0xfa, 0xff, 0x71, 0xfc, 0xff, 0x00, 0x51, 0x04, 0xf8, 0x00, 0x8e, 0x03, 0xf8,
};
...
static unsigned char enemy_left_2_bits[] =
    {0x00, 0xfd, 0x80, 0xfd, 0x7c, 0xfd, 0x02, 0xfe, 0xff, 0xff};
...
static unsigned char enemy_left_3_bits[] =
    {0x00, 0xb0, 0xf8, 0xaf, 0x3c, 0xa0, 0x82, 0xca, 0x7f, 0xe0, 0x80, 0xa3, 0x00, 0xbc};
...
static unsigned char enemy_right_2_bits[] =
    {0x02, 0xfc, 0x06, 0xfc, 0xfe, 0xfc, 0x03, 0xfd, 0xff, 0xff};
...
static unsigned char enemy_right_3_bits[] =
    {0x06, 0x80, 0xfa, 0x8f, 0x02, 0x9e, 0xa9, 0xa0, 0x03, 0xff, 0xe2, 0x80, 0x1e, 0x80};
...
static unsigned char enemy_back_4_bits[] =
    {0x20, 0x82, 0xe0, 0x83, 0xf0, 0x87, 0xcf, 0xf9, 0xc8, 0x89, 0x30, 0x86};
...
static unsigned char enemy_front_3_bits[] =
    {0x10, 0xe1, 0x50, 0xe1, 0xf8, 0xe3, 0xff, 0xff, 0xe0, 0xe0};
...
static unsigned char enemy_front_4_bits[] =
    {0x20, 0x82, 0xa0, 0x82, 0xe0, 0x83, 0xff, 0xff, 0xf0, 0x87, 0xc0, 0x81};
...
void drawCompass(Canvas* canvas, double direction) {
    ...
    canvas_draw_line(
        canvas,
        (64 - COMPASS_WIDTH / 2) + COMPASS_WIDTH,
        0,
        (64 - COMPASS_WIDTH / 2) + COMPASS_WIDTH,
        5);
    ...
    if(direction >= PI2 / 8 && direction <= 3 * PI2 / 8) { // North
        canvas_draw_xbm(
            canvas,
            (64 - COMPASS_WIDTH / 2) +
                (direction - PI2 / 8) * (COMPASS_WIDTH - compass_width) / (PI2 / 4),
            0,
            compass_width,
            compass_height,
            compass_n_bits);
        ...
        canvas_draw_xbm(
            canvas,
            (64 - COMPASS_WIDTH / 2) +
                (direction - 3 * PI2 / 8) * (COMPASS_WIDTH - compass_width) / (PI2 / 4),
            0,
            compass_width,
            compass_height,
            compass_w_bits);
        ...
        canvas_draw_xbm(
            canvas,
            (64 - COMPASS_WIDTH / 2) +
                (direction - 5 * PI2 / 8) * (COMPASS_WIDTH - compass_width) / (PI2 / 4),
            0,
            compass_width,
            compass_height,
            compass_s_bits);
        ...
        canvas_draw_xbm(
            canvas,
            (64 - COMPASS_WIDTH / 2) +
                (direction - 7 * PI2 / 8) * (COMPASS_WIDTH - compass_width) / (PI2 / 4),
            0,
            compass_width,
            compass_height,
            compass_e_bits);
...
void drawExplosion(Canvas* canvas) {
    canvas_draw_xbm(
        canvas, 64 - (explosion_width / 2), 43, explosion_width, explosion_height, explosion_bits);
}
