...
uint8_t CalculateBuildingTile(Building* building, uint8_t x, uint8_t y) {
    ...
    if(IsRubble(building->type)) return RUBBLE_TILE;
...
uint8_t CalculateTile(int x, int y) {
    ...
    if(connections == RoadMask) {
        ...
        if(!IsTerrainClear(x, y)) return FIRST_ROAD_BRIDGE_TILE + (variant & 1);

        if(HasHighTraffic(x, y)) return FIRST_ROAD_TRAFFIC_TILE + variant;
        return FIRST_ROAD_TILE + variant;
    } else if(connections == PowerlineMask) {
        int variant = GetConnectivityTileVariant(x, y, connections);

        if(!IsTerrainClear(x, y)) return FIRST_POWERLINE_BRIDGE_TILE + (variant & 1);
...
void ResetVisibleTileCache() {
    ...
    for(int y = 0; y < VISIBLE_TILES_Y; y++) {
        for(int x = 0; x < VISIBLE_TILES_X; x++) {
            VisibleTileCache[y * VISIBLE_TILES_X + x] =
                CalculateTile(x + CachedScrollX, y + CachedScrollY);
        }
    }
}
...
void ScrollUp(int amount) {
    ...
    for(int n = 0; n < VISIBLE_TILES_Y - amount; n++) {
        for(int x = 0; x < VISIBLE_TILES_X; x++) {
            VisibleTileCache[y * VISIBLE_TILES_X + x] =
                VisibleTileCache[(y - amount) * VISIBLE_TILES_X + x];
        }
        y--;
    }
...
void ScrollLeft(int amount) {
    ...
    for(int n = 0; n < VISIBLE_TILES_X - amount; n++) {
        for(int y = 0; y < VISIBLE_TILES_Y; y++) {
            VisibleTileCache[y * VISIBLE_TILES_X + x] =
                VisibleTileCache[y * VISIBLE_TILES_X + x - amount];
        }
        x--;
    }
...
void DrawCursor() {
    ...
    if(cursorDrawX >= 0 && cursorDrawY >= 0 && cursorDrawX + cursorWidth < DISPLAY_WIDTH &&
       cursorDrawY + cursorHeight < DISPLAY_HEIGHT) {
...
void AnimatePowercuts() {
    ...
    for(int n = 0; n < MAX_BUILDINGS; n++) {
        ...
        if(building->type && building->type != Park && !IsRubble(building->type)) {
            ...
            if(screenX >= 0 && screenY >= 0 && screenX < VISIBLE_TILES_X &&
               screenY < VISIBLE_TILES_Y) {
                if(showPowercut && !building->hasPower) {
                    VisibleTileCache[screenY * VISIBLE_TILES_X + screenX] = POWERCUT_TILE;
                } else {
                    VisibleTileCache[screenY * VISIBLE_TILES_X + screenX] =
                        CalculateBuildingTile(building, 1, 1);
                }
            }
        }
    }
...
void RefreshTileAndConnectedNeighbours(uint8_t x, uint8_t y) {
    ...
    if(x > 0 && GetConnections(x - 1, y)) RefreshTile(x - 1, y);
    if(x < MAP_WIDTH - 1 && GetConnections(x + 1, y)) RefreshTile(x + 1, y);
    if(y > 0 && GetConnections(x, y - 1)) RefreshTile(x, y - 1);
    if(y < MAP_HEIGHT - 1 && GetConnections(x, y + 1)) RefreshTile(x, y + 1);
}
...
void RefreshBuildingTiles(Building* building) {
    ...
    for(int j = 0; j < height; j++) {
        ...
        for(int i = 0; i < width; i++) {
            ...
            if(screenX >= 0 && screenY >= 0 && screenX < VISIBLE_TILES_X &&
               screenY < VISIBLE_TILES_Y) {
                VisibleTileCache[screenY * VISIBLE_TILES_X + screenX] =
                    CalculateBuildingTile(building, i, j);
            }
        }
    }
    ...
    for(int i = x1; i <= x2; i++) {
        if(GetConnections(i, y1) & RoadMask) RefreshTile(i, y1);
        if(GetConnections(i, y2) & RoadMask) RefreshTile(i, y2);
    }
    for(int i = y1; i <= y2; i++) {
        if(GetConnections(x1, i) & RoadMask) RefreshTile(x1, i);
        if(GetConnections(x2, i) & RoadMask) RefreshTile(x2, i);
    }
}
...
void DrawUI() {
    if(UIState.state == ShowingToolbar) {
        ...
        DrawFilledRect(
            ...
            NUM_TOOLBAR_BUTTONS * (TILE_SIZE + 1) + 2,
            TILE_SIZE + 2,
            1);
        DrawFilledRect(
            0,
            DISPLAY_HEIGHT - TILE_SIZE - 2 - FONT_HEIGHT - 1,
            DISPLAY_WIDTH / 2 + FONT_WIDTH + 1,
            FONT_HEIGHT + 2,
            1);
    ...
    DrawRect(
        DISPLAY_WIDTH - 2 - currencyStrLen * FONT_WIDTH,
        0,
        currencyStrLen * FONT_WIDTH + 2,
        FONT_HEIGHT + 2,
        1);
}
...
void DrawSaveLoadMenu() {
    ...
    DrawRect(
        DISPLAY_WIDTH / 2 - menuWidth / 2 + 1,
        DISPLAY_HEIGHT / 2 - menuHeight / 2 + 1,
        menuWidth,
        menuHeight,
        0);
    DrawFilledRect(
        DISPLAY_WIDTH / 2 - menuWidth / 2,
        DISPLAY_HEIGHT / 2 - menuHeight / 2,
        menuWidth,
        menuHeight,
        1);
    DrawRect(
...
void DrawNewCityMenu() {
    ...
    DrawBitmap(
        GetTerrainData(State.terrainType),
        DISPLAY_WIDTH / 2 - MAP_WIDTH / 2,
        mapY,
        MAP_WIDTH,
        MAP_HEIGHT);
    ...
    DrawString(
        GetTerrainDescription(State.terrainType),
        DISPLAY_WIDTH / 2 - FONT_WIDTH * 3,
        mapY + MAP_HEIGHT + 5);
    DrawString(
        LeftArrowStr,
        DISPLAY_WIDTH / 2 - MAP_WIDTH / 2 - 6 - FONT_WIDTH,
        DISPLAY_HEIGHT / 2 - FONT_HEIGHT / 2);
    DrawString(
        RightArrowStr,
        DISPLAY_WIDTH / 2 + MAP_WIDTH / 2 + 6,
        DISPLAY_HEIGHT / 2 - FONT_HEIGHT / 2);
}
...
void DrawBudgetMenu() {
    ...
    DrawCurrency(
        State.taxesCollected - State.roadBudget -
            State.policeBudget * FIRE_AND_POLICE_MAINTENANCE_COST -
            State.fireBudget * FIRE_AND_POLICE_MAINTENANCE_COST,
        x2,
        y);
    y += spacing;
