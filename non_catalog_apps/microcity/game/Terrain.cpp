...
uint8_t GetTerrainTile(int x, int y) {
    ...
    if(IsTerrainClear(x, y)) {
        if(!northClear && !westClear) return NORTH_WEST_EDGE_TILE;
        if(!northClear && !eastClear) return NORTH_EAST_EDGE_TILE;
        if(!southClear && !westClear) return SOUTH_WEST_EDGE_TILE;
        if(!southClear && !eastClear) return SOUTH_EAST_EDGE_TILE;
