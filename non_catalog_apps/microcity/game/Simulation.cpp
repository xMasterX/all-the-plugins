...
#define SIM_FIRE_SPREAD_CHANCE \
    64 // If 8 bit rand value is less than this then attempt to spread fire
...
void DoBudget() {
    ...
    int32_t totalPopulation =
        (State.residentialPopulation + State.commercialPopulation + State.residentialPopulation) *
    ...
    for(int y = 0; y < MAP_HEIGHT; y++) {
        for(int x = 0; x < MAP_WIDTH; x++) {
            if(GetConnections(x, y) & RoadMask) numRoadTiles++;
        }
    }
    ...
    printf(
        ...
        FIRE_AND_POLICE_MAINTENANCE_COST * numPoliceDept);
    printf(
        "Fire cost: %d x $%d = $%d\n",
        numFireDept,
        FIRE_AND_POLICE_MAINTENANCE_COST,
        FIRE_AND_POLICE_MAINTENANCE_COST * numFireDept);
    printf("Road maintenance: %d tiles = $%d\n", numRoadTiles, State.roadBudget);
#endif

    int32_t cashFlow = State.taxesCollected - State.roadBudget -
...
void SimulateBuilding(Building* building) {
    ...
    if(building->onFire) {
        ...
        int fireDeptInfluence =
            SIM_FIRE_DEPT_BASE_INFLUENCE + closestFireDept * SIM_FIRE_DEPT_INFLUENCE_MULTIPLIER;
        ...
        building->type == Residential || building->type == Commercial ||
        ...
        if(building->hasPower) {
            ...
            // random effect
            int randomEffect =
                (GetRand() & SIM_RANDOM_STRENGTH_MASK) - (SIM_RANDOM_STRENGTH_MASK / 2);
            score += randomEffect;

            // tend towards average population density
            score += (AVERAGE_POPULATION_DENSITY - building->populationDensity) *
            ...
            case Residential:
                if(State.residentialPopulation < State.industrialPopulation) {
                    ...
                    State.residentialPopulation >
                    State.industrialPopulation + State.commercialPopulation) {
                    populationEffect -= SIM_UNEMPLOYMENT_PENALTY;
                }
                break;
            case Industrial:
                if(State.industrialPopulation < State.residentialPopulation ||
                   State.industrialPopulation < State.commercialPopulation) {
                    populationEffect += SIM_INDUSTRIAL_OPPORTUNITY_BOOST;
                }
                break;
            case Commercial:
                if(State.commercialPopulation < State.residentialPopulation ||
            ...
            if(isRoadConnected) {
                ...
                for(int n = 0; n < MAX_BUILDINGS; n++) {
                    ...
                    if(building != otherBuilding && otherBuilding->type &&
                       (otherBuilding->hasPower || otherBuilding->type == Park) &&
                       !otherBuilding->onFire) {
                        uint8_t distance = GetManhattanDistance(building, otherBuilding);

                        if(otherBuilding->type == PoliceDept &&
                           distance < closestPoliceStationDistance) {
                        ...
                        if(otherBuilding->type == Industrial) {
                            buildingPollution = SIM_INDUSTRIAL_BASE_POLLUTION +
                                                otherBuilding->populationDensity - distance;
                        ...
                        if(buildingPollution > 0) pollution += buildingPollution;

                        if(distance <= SIM_LOCAL_BUILDING_DISTANCE &&
                           GetNumRoadConnections(otherBuilding) >= 3) {
                            switch(otherBuilding->type) {
                            case Industrial:
                                if(otherBuilding->populationDensity >=
                                       building->populationDensity &&
                                   building->type == Residential) {
                                    localInfluence += SIM_LOCAL_BUILDING_INFLUENCE;
                                } else if(
                                    otherBuilding->populationDensity >
                                        building->populationDensity &&
            ...
            if(building->type == Residential) {
                if(pollution > SIM_MAX_POLLUTION) pollution = SIM_MAX_POLLUTION;
...
void Simulate() {
    ...
    if(State.timeToNextDisaster == 0) {
        StartRandomFire();
        State.timeToNextDisaster =
            (GetRand() % (MAX_TIME_BETWEEN_DISASTERS - MIN_TIME_BETWEEN_DISASTERS)) +
...
bool StartRandomFire() {
    ...
    while(attemptsLeft) {
        ...
        if(index < MAX_BUILDINGS && State.buildings[index].type &&
           !State.buildings[index].onFire && !IsRubble(State.buildings[index].type) &&
           State.buildings[index].type != Park) {
