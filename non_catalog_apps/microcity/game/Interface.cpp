...
void WrapMenuInput(uint8_t input, uint8_t numOptions) {
    if(input & INPUT_UP) {
        if(UIState.selection > 0)
            ...
            UIState.selection = numOptions - 1;
    }
    if(input & INPUT_DOWN) {
                ...
                if(building && !IsRubble(building->type)) {
                    ...
                    if(onGround ||
                       (currentConnections == 0 &&
                        IsSuitableForBridgedTile(UIState.selectX, UIState.selectY, mask))) {
                        if((currentConnections & mask) == 0) {
                            if(State.money >= cost) {
                                State.money -= cost;
                                SetConnections(
                                    UIState.selectX, UIState.selectY, currentConnections | mask);
                                ...
                                RefreshTileAndConnectedNeighbours(
                                    UIState.selectX, UIState.selectY);
                            } else {
                                // TODO: not enough cash
                            }
                        }
