...
bool IsSuitableForBridgedTile(int x, int y, uint8_t mask) {
    ...
    if(neighbours == Neighbour_North || neighbours == Neighbour_East ||
       neighbours == Neighbour_South || neighbours == Neighbour_West ||
       neighbours == (Neighbour_North | Neighbour_South) ||
       neighbours == (Neighbour_East | Neighbour_West)) {
        if(neighbours & Neighbour_North) {
            if(!IsTerrainClear(x, y - 1) &&
               (GetNeighbouringConnectivity(x, y - 1, mask) & (Neighbour_East | Neighbour_West)))
                return false;
        }
        if(neighbours & Neighbour_East) {
            if(!IsTerrainClear(x + 1, y) &&
               (GetNeighbouringConnectivity(x + 1, y, mask) & (Neighbour_North | Neighbour_South)))
                return false;
        }
...
void CalculatePowerConnectivity() {
    ...
    for(int n = 0; n < MAX_BUILDINGS; n++) {
        if(State.buildings[n].type) {
            State.buildings[n].hasPower =
                IsTilePowered(State.buildings[n].x, State.buildings[n].y);
        }
    }
}
...
uint8_t GetFilledNeighbourCount(uint8_t x, uint8_t y) {
    ...
    if(x == 0 || !IsFillEmpty(x - 1, y)) count++;
    if(x == MAP_WIDTH - 1 || !IsFillEmpty(x + 1, y)) count++;
    if(y == 0 || !IsFillEmpty(x, y - 1)) count++;
    if(y == MAP_HEIGHT - 1 || !IsFillEmpty(x, y + 1)) count++;
    ...
    Fill_Start:
        ...
        case 1:
            if(backtrack) {
                ...
                IsFilledInDir(x, y, FillFrontLeft(fillDir)) == false &&
...
void PowerFloodFill(uint8_t x, uint8_t y) {
    ...
    while(stackSize) {
        ...
        while(y1 < MAP_HEIGHT && (GetConnections(x, y1) & PowerlineMask) &&
              !IsTilePowered(x, y1)) {
            SetTilePowered(x, y1);

            bool canFillLeft = (GetConnections(x - 1, y1) & PowerlineMask) &&
                               !IsTilePowered(x - 1, y1);
            ...
            bool canFillRight = (GetConnections(x + 1, y1) & PowerlineMask) &&
                                !IsTilePowered(x + 1, y1);
