...
#ifndef ARDULIB_DISABLE_EXIT
    static uint8_t ExitInputMaskFromKey_(InputKey key);
    void handleExitInput_(const InputEvent* event);

    uint8_t exit_input_held_ = 0;
    bool exit_timer_running_ = false;
    bool exit_timer_blocked_ = false;
    uint32_t exit_timer_started_ms_ = 0;
#endif
