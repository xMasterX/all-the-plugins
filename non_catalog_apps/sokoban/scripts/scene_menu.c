...
void menu_render_callback(Canvas* const canvas, void* context) {
    ...
    if(menuState == MenuState_Main) {
        ...
        ContinuousPageInfo pageInfo = pagination_continuous_centered(
            database->collectionsCount, 5, gameplayState->selectedCollection);
        ...
        canvas_set_font(canvas, FontKeyboard);
        ContinuousPageInfo pageInfo = pagination_continuous_centered(
            database->collections[gameplayState->selectedCollection].levelsCount,
            5,
            gameplayState->selectedLevel);
        for(int i = 0, item = pageInfo.start; i < 5 && item <= pageInfo.end; i++, item++) {
            int y = 12 + i * 10;
            int x = 0;
            ...
            x += 46;
            LevelItem levelItem =
                database->collections[gameplayState->selectedCollection].levels[item];
            const Icon* icon;
...
void menu_input_callback(InputKey key, InputType type, void* context) {
    ...
    if(state == MenuState_CollectionSelection)
        gameplayState->selectedCollection = wrap_single(
            gameplayState->selectedCollection + delta, 0, database->collectionsCount - 1);
    else if(state == MenuState_LevelSelection)
        gameplayState->selectedLevel = wrap_single(
            gameplayState->selectedLevel + delta,
            0,
            database->collections[gameplayState->selectedCollection].levelsCount - 1);
}
