...
static void verify_level_completed(GameState* state) {
    for(int y = 0; y < state->levelHeight; y++) {
        for(int x = 0; x < state->levelWidth; x++) {
            if((state->board[y][x] & CellHasBox) && !(state->board[y][x] & CellHasTarget)) return;
        }
    }
...
static bool is_in_bounds(GameState* state, int x, int y) {
    return 0 <= x && x < state->levelWidth && 0 <= y && y < state->levelHeight;
}

static void undo_push(GameState* state, UndoToken token) {
    state->undoHead += 1;
    if(state->undoHead == state->undoCapacity) state->undoHead = 0;
    state->undoBuffer[state->undoHead] = token;

    if(state->undoHead == state->undoTail) {
        state->undoTail += 1;
        if(state->undoTail == state->undoCapacity) state->undoTail = 0;
    }
}

static UndoToken undo_pop(GameState* state) {
    if(state->undoHead == state->undoTail) return UndoInvalid;

    UndoToken token = state->undoBuffer[state->undoHead];
    state->undoHead -= 1;
    if(state->undoHead < 0) state->undoHead = state->undoCapacity - 1;
    return token;
}

void game_state_apply_move(GameState* state, int dx, int dy) {
    int newX = state->playerX + dx, newY = state->playerY + dy;
    if(!is_in_bounds(state, newX, newY)) return;

    UndoToken token;
    if(dx < 0) token = UndoLeft;
    if(dx > 0) token = UndoRight;
    if(dy < 0) token = UndoUp;
    if(dy > 0) token = UndoDown;

    CellType newCell = state->board[newY][newX];
    if(newCell & CellHasWall) return;

    if(newCell & CellHasBox) {
        int newBoxX = newX + dx, newBoxY = newY + dy;
        if(!is_in_bounds(state, newBoxX, newBoxY)) return;

        CellType newBoxCell = state->board[newBoxY][newBoxX];
        if((newBoxCell & CellHasWall) || (newBoxCell & CellHasBox)) return;
...
void game_state_undo_move(GameState* state) {
    ...
    if(token == UndoInvalid) return;

    int dx = 0, dy = 0;
    if((token & UndoDirectionMask) == UndoLeft) dx = -1;
    if((token & UndoDirectionMask) == UndoRight) dx = +1;
    if((token & UndoDirectionMask) == UndoUp) dy = -1;
    if((token & UndoDirectionMask) == UndoDown) dy = +1;
