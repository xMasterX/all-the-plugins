...
static NfcCommand nfc_stock_scene_scan_callback(NfcGenericEvent event, void* context) {
    ScanContext* ctx = (ScanContext*)context;
    if(event.protocol != NfcProtocolIso14443_3a || !event.event_data) {
        return NfcCommandContinue;
    }

    const Iso14443_3aPollerEvent* iso_ev = (const Iso14443_3aPollerEvent*)event.event_data;
    if(iso_ev->type != Iso14443_3aPollerEventTypeReady) {
        return NfcCommandContinue;
    }

    const Iso14443_3aData* data = (const Iso14443_3aData*)nfc_poller_get_data(ctx->poller);
    ...
    if(stock_db_find_by_uid(app->active_db_path, uid, uid_len, &found)) {
        ...
        view_dispatcher_send_custom_event(app->view_dispatcher, NfcStockEventScanKnown);
    } else {
        ...
        view_dispatcher_send_custom_event(app->view_dispatcher, NfcStockEventScanUnknown);
    }
...
void nfc_stock_scene_scan_on_enter(void* context) {
    ...
    widget_add_string_element(
        app->ui->widget, 64, 6, AlignCenter, AlignTop, FontPrimary, "NFC Scanner");
    widget_add_string_element(
        app->ui->widget, 64, 21, AlignCenter, AlignTop, FontSecondary, "[    NFC TAG    ]");
    widget_add_string_element(
        app->ui->widget, 64, 35, AlignCenter, AlignTop, FontPrimary, "Bring tag near back");
    widget_add_string_element(
        app->ui->widget, 64, 49, AlignCenter, AlignTop, FontSecondary, "Back: cancel");
...
static bool nfc_stock_details_input_callback(InputEvent* event, void* context) {
    ...
    if(event->type == InputTypeShort) {
        if(event->key == InputKeyUp) {
            view_dispatcher_send_custom_event(app->view_dispatcher, NfcStockEventDetailsStockUp);
            return true;
        }
        if(event->key == InputKeyDown) {
            view_dispatcher_send_custom_event(app->view_dispatcher, NfcStockEventDetailsStockDown);
            return true;
        }
        if(event->key == InputKeyOk) {
            view_dispatcher_send_custom_event(app->view_dispatcher, NfcStockEventDetailsOpenEdit);
            return true;
        }
    }
    return false;
...
static void nfc_stock_edit_enter_cb(void* context, uint32_t index) {
    ...
    text_input_set_result_callback(
        app->ui->text_input,
        nfc_stock_edit_after_text_input,
        app,
        s_text_buf,
        sizeof(s_text_buf),
        false);
...
static void nfc_stock_edit_refresh_list(NfcStockApp* app) {
    ...
    variable_item_list_set_enter_callback(app->ui->edit_list, nfc_stock_edit_enter_cb, app);
}
...
static void nfc_stock_inventory_show_empty(NfcStockApp* app) {
    ...
    widget_reset(app->ui->widget);
    widget_add_string_element(
        app->ui->widget, 0, 10, AlignLeft, AlignTop, FontPrimary, "No items in inventory");
    widget_add_string_element(
        app->ui->widget, 0, 26, AlignLeft, AlignTop, FontSecondary, "Scan a tag or add from NFC");
    widget_add_string_element(
        app->ui->widget, 0, 42, AlignLeft, AlignTop, FontSecondary, "Back: return");
    view_dispatcher_switch_to_view(app->view_dispatcher, NfcStockViewWidget);
}
...
static void nfc_stock_inventory_refresh(NfcStockApp* app) {
    ...
    size_t count = 0;
    if(!fs_read_all_stock_items(app->active_db_path, &items, &count) || count == 0) {
        if(items) {
            free(items);
        }
    ...
    for(size_t i = 0; i < count; i++) {
        ...
        snprintf(
            ...
            items[i].quantity,
            alert ? " !" : "");
        variable_item_list_add(app->ui->inventory_list, row_labels[i], 0, NULL, NULL);
    }
    free(items);

    variable_item_list_set_enter_callback(
        app->ui->inventory_list, nfc_stock_inventory_callback, app);
...
static void nfc_stock_inventory_action_callback(void* context, uint32_t index) {
    ...
    if(index == InvRow_Edit) {
        if(fs_read_stock_at(app->active_db_path, selected_index, &app->current_item)) {
            app->item_is_new = false;
            scene_manager_next_scene(app->scene_manager, NfcStockSceneEdit);
        }
...
static void nfc_stock_inventory_callback(void* context, uint32_t index) {
    ...
    submenu_add_item(
        app->ui->submenu, "Edit", InvRow_Edit, nfc_stock_inventory_action_callback, app);
    submenu_add_item(
        app->ui->submenu, "Delete", InvRow_Delete, nfc_stock_inventory_action_callback, app);
...
static void nfc_stock_ui_configure_settings_menu(NfcStockUi* ui, NfcStockApp* app);
...
static void nfc_stock_settings_build_db_path(char* out, size_t out_size, const char* name) {
...
static void nfc_stock_db_label_from_path(const char* path, char* out, size_t out_size) {
...
static void nfc_stock_settings_select_existing_callback(void* context, uint32_t index) {
    NfcStockApp* app = (NfcStockApp*)context;
    if(index >= s_db_count) {
        return;
    }
    storage_helper_set_active_db(s_db_paths[index]);
    strncpy(app->active_db_path, s_db_paths[index], sizeof(app->active_db_path) - 1);
    app->active_db_path[sizeof(app->active_db_path) - 1] = '\0';
...
static void nfc_stock_ui_open_existing_db_menu(NfcStockApp* app) {
    ...
    if(storage_dir_open(dir, STORAGE_PATH)) {
        FileInfo file_info;
        char name[96];
        while(s_db_count < 32 && storage_dir_read(dir, &file_info, name, sizeof(name))) {
            ...
            strncpy(s_db_paths[s_db_count], STORAGE_PATH, sizeof(s_db_paths[s_db_count]) - 1);
            s_db_paths[s_db_count][sizeof(s_db_paths[s_db_count]) - 1] = '\0';
            s_db_paths[s_db_count][base_len] = '/';
            strncpy(
                &s_db_paths[s_db_count][base_len + 1],
                name,
                sizeof(s_db_paths[s_db_count]) - base_len - 2);
            s_db_paths[s_db_count][needed - 1] = '\0';
            nfc_stock_db_label_from_path(
                s_db_paths[s_db_count], s_db_labels[s_db_count], sizeof(s_db_labels[s_db_count]));
            s_db_count++;
        }
    ...
    if(s_db_count == 0) {
        widget_reset(app->ui->widget);
        widget_add_string_element(
            app->ui->widget, 0, 10, AlignLeft, AlignTop, FontPrimary, "No .bin databases found");
        widget_add_string_element(
            app->ui->widget, 0, 26, AlignLeft, AlignTop, FontSecondary, "Create one from");
        widget_add_string_element(
            app->ui->widget, 0, 38, AlignLeft, AlignTop, FontSecondary, "Select warehouse");
        view_dispatcher_switch_to_view(app->view_dispatcher, NfcStockViewWidget);
        return;
    }
    ...
    for(size_t i = 0; i < s_db_count; i++) {
        submenu_add_item(
            ...
            s_db_labels[i],
            (uint32_t)i,
            nfc_stock_settings_select_existing_callback,
            app);
    }
    view_dispatcher_switch_to_view(app->view_dispatcher, NfcStockViewSubmenu);
}

static void nfc_stock_settings_db_action_callback(void* context, uint32_t index) {
    NfcStockApp* app = (NfcStockApp*)context;
    ...
    text_input_set_result_callback(
        app->ui->text_input,
        nfc_stock_settings_db_text_done,
        app,
        s_db_name_buf,
        sizeof(s_db_name_buf),
        false);
    view_dispatcher_switch_to_view(app->view_dispatcher, NfcStockViewTextInput);
}
...
static void nfc_stock_ui_open_select_db_menu(NfcStockApp* app) {
    ...
    submenu_add_item(
        app->ui->submenu,
        "Switch existing",
        SettingsDbActionSelect,
        nfc_stock_settings_db_action_callback,
        app);
    submenu_add_item(
        app->ui->submenu,
        "New database",
        SettingsDbActionCreate,
        nfc_stock_settings_db_action_callback,
        app);
    submenu_add_item(
        app->ui->submenu,
        "Rename current",
        SettingsDbActionRename,
        nfc_stock_settings_db_action_callback,
        app);
    view_dispatcher_switch_to_view(app->view_dispatcher, NfcStockViewSubmenu);
}
...
static void nfc_stock_ui_export_csv_execute(NfcStockApp* app) {
    ...
    widget_reset(app->ui->widget);
    widget_add_string_element(
        app->ui->widget, 0, 10, AlignLeft, AlignTop, FontPrimary, "Exported to:");
    ...
    widget_add_string_element(
        app->ui->widget, 0, 25, AlignLeft, AlignTop, FontSecondary, csv_line_1);
    widget_add_string_element(
        app->ui->widget, 0, 38, AlignLeft, AlignTop, FontSecondary, csv_line_2);
    view_dispatcher_switch_to_view(app->view_dispatcher, NfcStockViewWidget);
}
...
static void nfc_stock_ui_settings_callback(void* context, uint32_t index) {
    ...
    case Settings_Credits: {
        ...
        widget_reset(app->ui->widget);
        widget_add_string_element(
            app->ui->widget, 0, 5, AlignLeft, AlignTop, FontPrimary, "NFC Stock Manager");
        widget_add_string_element(
            app->ui->widget, 0, 20, AlignLeft, AlignTop, FontSecondary, version_line);
        widget_add_string_element(
            app->ui->widget,
            0,
            35,
        ...
        widget_add_string_element(
            app->ui->widget, 0, 50, AlignLeft, AlignTop, FontSecondary, "/flipper-nfc-stock");
        view_dispatcher_switch_to_view(app->view_dispatcher, NfcStockViewWidget);
        break;
    }
    }
...
static void nfc_stock_ui_configure_settings_menu(NfcStockUi* ui, NfcStockApp* app) {
    ...
    submenu_add_item(
        ui->submenu, "Select warehouse", Settings_SelectDB, nfc_stock_ui_settings_callback, app);
    submenu_add_item(
        ui->submenu, "Export CSV", Settings_ExportCSV, nfc_stock_ui_settings_callback, app);
    submenu_add_item(
        ui->submenu, "Reset stock to 0", Settings_ResetStock, nfc_stock_ui_settings_callback, app);
    submenu_add_item(
        ui->submenu, "Clear all inventory", Settings_ClearAll, nfc_stock_ui_settings_callback, app);
    submenu_add_item(
        ui->submenu, "Credits", Settings_Credits, nfc_stock_ui_settings_callback, app);
}
