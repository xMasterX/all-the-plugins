...
static void subghz_signal_gen_show_splash_screen(SubGhzSignalGenApp* app) {
    view_port_draw_callback_set(
        app->view_port, subghz_signal_gen_splash_screen_draw_callback, app);
...
static int32_t subghz_signal_gen_tx_thread(void* context) {
    ...
    case SignalModeTriangleWave:
        for(size_t i = 0; i < sizeof(signal_data); i++) {
            signal_data[i] = (i < sizeof(signal_data) / 2) ?
                                 (i * 255 / (sizeof(signal_data) / 2)) :
                                 (255 - (i * 255 / (sizeof(signal_data) / 2)));
        }
        break;
    case SignalModeChirp:
        for(size_t i = 0; i < sizeof(signal_data); i++) {
            signal_data[i] =
                (uint8_t)(127 * sinf(2 * M_PI * i * (1 + (float)i / sizeof(signal_data))));
        }
        break;
    case SignalModeGaussianNoise:
        for(size_t i = 0; i < sizeof(signal_data); i++) {
    ...
    while(app->tx_running) {
        if(app->subghz_txrx) {
            bool write_result =
                subghz_tx_rx_worker_write(app->subghz_txrx, signal_data, sizeof(signal_data));
            if(!write_result) {
                furi_delay_ms(20);
            }
        } else {
