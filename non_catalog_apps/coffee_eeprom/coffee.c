...
bool write_buffer(uint8_t* buffer, size_t buffer_size, uint8_t start_address) {
    ...
    if(furi_hal_i2c_is_device_ready(
           &furi_hal_i2c_handle_external, EEPROM_I2C_ADDR, (uint32_t)1000)) {
        ...
        for(size_t i = 0; i < buffer_size; i++) {
            ...
            while(!result) {
                ...
                FURI_LOG_E(
                    "COFFEE",
                    "Write %.2X, byte %d/%d at address %.2X, result %d",
                    buffer[i],
                    i + 1,
                    buffer_size,
                    start_address + i,
...
float read_credit() {
    ...
    furi_hal_i2c_acquire(&furi_hal_i2c_handle_external);
    bool is_ready = false;
    is_ready = furi_hal_i2c_is_device_ready(
        &furi_hal_i2c_handle_external, EEPROM_I2C_ADDR, (uint32_t)1000);
    if(is_ready) {
        ...
        int exponent = 14;
        int hi, lo = 0;
        for(size_t i = 0; i < sizeof(data_buffer); i++) { //iterate 2 bit at times
            hi = 0;
            lo = 0;
            for(int j = 3; j >= 0; j--) {
