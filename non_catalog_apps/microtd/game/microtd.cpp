...
#define UPGRADE_RANGE_FRACTION_INCREASE \
    4 /**< Fractional increase (+ 1/x) of
                                                 range by upgrade. */
#define UPGRADE_DAMAGE_FRACTION_INCREASE \
    2 /**< Fractional increase (+ 1/x) of
                                                 damage by upgrade. */
...
#define getterSetter(dataType, memberVar) getter(dataType, memberVar) setter(dataType, memberVar)
...
const uint8_t PROGMEM ditherImage[] = {0xaa, 0x55, 0xaa, 0x55, 0xaa, 0x55, 0xaa, 0x55};

/// 15 x 5 wave decoration image.
const uint8_t PROGMEM waveImage[] =
    {0x19, 0x1c, 0x1c, 0x1c, 0x1c, 0x19, 0x13, 0x13, 0x07, 0x07, 0x07, 0x07, 0x13, 0x13, 0x19};

/// 25 x 18 logo image
const uint8_t PROGMEM logoImage[] = {
    0xf7, 0x63, 0xb7, 0x5f, 0x5b, 0x41, 0x40, 0x41, 0x5b, 0x5f, 0x58, 0x41, 0x40, 0x41, 0x58,
    0x5f, 0x5b, 0x41, 0x40, 0x41, 0x5b, 0x5f, 0xaf, 0x47, 0xef, 0x80, 0x55, 0x80, 0x3e, 0x7f,
    0x41, 0x6f, 0x6f, 0x71, 0x7f, 0x3e, 0x00, 0x03, 0x01, 0x7f, 0x01, 0x03, 0x00, 0x7f, 0x41,
    0x63, 0x3e, 0x80, 0x7f, 0x80, 0x03, 0x03, 0x02, 0x01, 0x00, 0x01, 0x00, 0x01, 0x00, 0x01,
    0x00, 0x01, 0x00, 0x01, 0x00, 0x01, 0x00, 0x01, 0x00, 0x01, 0x00, 0x01, 0x02, 0x03, 0x03};
...
const uint8_t PROGMEM towerBigSprites[] = {
    ...
    0xe5, 0xb9, 0x93, 0x87, 0x3f, 0x3f, 0xff, 0xff, 0xff, 0xff, 0xff, 0x8e,
    0x96, 0xb0, 0x96, 0xbf, 0xbf, 0x96, 0xb0, 0x96, 0x8e, 0xff, 0xff, 0xff,
    0xff, 0xff, 0x3f, 0xbf, 0x87, 0x83, 0x81, 0x99, // TOWER_WATER upgraded
    0x99, 0x81, 0x83, 0x87, 0xbf, 0x3f, 0xff, 0xff, 0xff, 0xff, 0xff, 0x8e,
    0xa6, 0x80, 0xa0, 0x80, 0x80, 0xa0, 0x80, 0xa6, 0x8e, 0xff, 0xff, 0xff,
    0xff, 0xff, 0xff, 0xe3, 0xc9, 0x1d, 0x73, 0xe7, // TOWER_FIRE
    0xe7, 0x73, 0x1d, 0xc9, 0xe3, 0xff, 0xff, 0xff, 0xff, 0xff, 0xfb, 0x91,
    0xa4, 0xb4, 0x95, 0xbf, 0xbf, 0x95, 0xb4, 0xa4, 0x91, 0xfb, 0xff, 0xff,
    0xff, 0xff, 0xff, 0xe3, 0xc9, 0x19, 0x33, 0x07, // TOWER_FIRE upgraded
    0x07, 0x33, 0x19, 0xc9, 0xe3, 0xff, 0xff, 0xff, 0xff, 0xff, 0xfb, 0x95,
    0xa4, 0x84, 0xa4, 0x8e, 0x8e, 0xa4, 0x84, 0xa4, 0x95, 0xfb, 0xff, 0xff};
...
struct BytePosition {
    ...
    BytePosition(uint8_t x = 0, uint8_t y = 0)
        : mX(x)
        , mY(y) {
    }
...
bool distanceIsGreater(struct BytePosition p1, struct BytePosition p2, uint8_t limit) {
...
public:
    ...
    void addSegment(PathSegment segment) {
        if(mNumSegments >= MAX_SEGMENTS) return;
...
public:
    ...
    void update(uint16_t frameNumber) {
        if(isAlive() && frameNumber % (getSpeed() * 2) == 0) mPosition += 1;

        if(frameNumber % 3 == 0) {
            uint8_t freezeCounter = getFreezeCounter();

            if(freezeCounter > 0) setTypeFreeze(getTypeIndex(), freezeCounter - 1);
        }
    }
    ...
    void setTypeFreeze(IndexPointer creepType, uint8_t freezeCounter) {
        mTypeFreeze = (creepType & 0b00001111) | ((freezeCounter & 0b00001111) << 4);
    }
    ...
    void kill() {
        ...
        if(getLives() == 0 ||
           getTypeIndex() ==
               CREEP_SPIDER_BIG) { // ^ big spiders have to be killed to spawn small ones
            setHealthLives(0, 0);
        } else {
            // respawn
    ...
    bool hit(uint8_t damage, bool freeze) {
        ...
        if(freeze) setTypeFreeze(getTypeIndex(), getTypeIndex() == CREEP_WOLF ? 7 : 15);
    ...
    getterSetter(IndexPointer, Path) getterSetter(int16_t, Position)
};
...
protected:
    ...
    void hitCreep(CreepInstance* c, uint16_t* money, bool splash = false) {
        if(!canAttack(c->getTypeIndex())) return;

        if(c->hit(
               getDamage() / (splash ? 2 : 1),
               (getTypeIndex() == TOWER_ICE) ||
...
public:
    bool canAttack(IndexPointer creepType) {
        ...
        if(creepType == CREEP_BAT && ti == TOWER_CANNON) return false; // bats are immune to cannon
    ...
    void update(
        CreepInstance* creepArray,
        uint8_t creepArraySize,
        const CreepPath* creepPaths,
        BytePosition tile,
        uint16_t frame,
        uint16_t* money) {
        ...
        if(mTarget == TARGET_NONE) // no target => look for a new one
        {
            for(uint8_t i = 0; i < creepArraySize; ++i) {
                ...
                if(!c.entered() || !c.isAlive() || !canAttack(c.getTypeIndex())) continue;
                ...
                if(!distanceIsGreater(p, selfPosition, range)) {
                    ...
                    if((getTypeIndex() != TOWER_ICE) ||
                       !creepArray[mTarget]
                            .isFrozen()) { // ^ ice tower keeps on looking for unfrozen targets
                        break;
                    }
                }
            }
            ...
            if(!c.isAlive() || !c.entered()) {
                ...
                if(getAttackProgress() == 0) // attack finished
                {
                    ...
                    if(getTypeIndex() == TOWER_CANNON || getTypeIndex() == TOWER_FIRE) {
                        ...
                        for(uint8_t i = 0; i < creepArraySize; ++i) {
                            if(i == mTarget) continue;

                            CreepInstance* c = &creepArray[i];

                            if(c->isAlive() && c->entered() &&
                               !distanceIsGreater(
                                   c->getPixelPosition(creepPaths), p, SPLASH_RANGE)) {
    ...
    TowerInstance(const IndexPointer type = 0)
        : mTarget(TARGET_NONE) {
    ...
    bool hasUpgraded(IndexPointer upgradeType) {
        ...
        return (tt.mUpgrades[0] == upgradeType && upgraded(0)) ||
    ...
    uint8_t getRange() {
        ...
        return r + (hasUpgraded(UPGRADE_RANGE) ? r / UPGRADE_RANGE_FRACTION_INCREASE : 0);
    }

    uint8_t getDamage() {
        uint8_t d = getType().mDamage;

        return d + (hasUpgraded(UPGRADE_DAMAGE) ? d / UPGRADE_DAMAGE_FRACTION_INCREASE : 0);
    }

    uint8_t getAttackSpeed() {
        uint8_t s = getType().mAttackSpeed;

        return s - (hasUpgraded(UPGRADE_SPEED) ? 2 : 0);
    }
...
public:
    ...
    getter(uint8_t, StartLives) getter(uint8_t, StartMoney) getter(uint8_t, NumPaths)
        getter(const CreepPath*, Paths) getterSetter(Spawner*, Spawner)
};
...
protected:
    bool addCreep(CreepInstance c, CreepInstance* array, uint16_t* count) {
        if(*count >= MAX_CREEPS) return false;
    ...
    IndexPointer cycleCreeps(uint8_t index, IndexPointer levelFrom, IndexPointer levelTo) {
    ...
    void cyclingSpawn(
        ...
        for(uint8_t i = 0; i < total; ++i) {
            if(i % groupSize == 0) position -= positionOffset;
...
#define helperExpr(d1, d2, t1, t2, t3, t4, t5, t6, n)                                      \
    (curDir == DIRECTION_##d1 || prevDir == DIRECTION_##d2 || curTile == TILE_PATH_##t1 || \
             curTile == TILE_PATH_##t2 || curTile == TILE_PATH_##t3 ||                     \
             curTile == TILE_PATH_##t4 || curTile == TILE_PATH_##t5 ||                     \
             curTile == TILE_PATH_##t6 || curTile == TILE_PATH_NESW ?                      \
         n :                                                                               \
         0)

        uint8_t tileFeatures = helperExpr(N, S, NS, NE, NW, NEW, NSW, NEW, 8) | // N
...
#undef helperExpr
        ...
        case 0b1010:
            ...
            break;
        case 0b1011:
            return TILE_PATH_NSW;
            break;
        case 0b1100:
            return TILE_PATH_NE;
            break;
        case 0b1101:
            return TILE_PATH_NEW;
            break;
        case 0b1110:
            return TILE_PATH_NES;
            break;
        case 0b1111:
            return TILE_PATH_NESW;
            break;
        ...
        for(uint8_t i = 0; i < gameMap->getNumPaths(); ++i) {
            ...
            for(uint8_t j = 0; j < p->getNumSegments(); ++j) {
                ...
                for(uint8_t k = 0; k < s.getLength(); ++k) {
                    TileType newTile =
                        makePathTile(s.getDirection(), previousDirection, getTile(pos)->mType);
...
struct Game {
    ...
    void buttonDown(uint8_t button) {
            ...
            if(button == A_BUTTON) {
                ...
                if(mCursor.mX < TILES_X - 1 && mCursor.mY < TILES_Y - 1) {
                    if(mPlayground.getTile(mCursor.right())->hasBigTower())
                        ...
                        mCursor.mY += 1;
                    else if(mPlayground.getTile(mCursor.right().down())->hasBigTower())
        ...
        case STATE_PLAYING_MENU:
            ...
            case LEFT_BUTTON:
                mSelectedMenuItem = mSelectedMenuItem == 0 ? 12 : mSelectedMenuItem - 1;
                break;
        ...
        case STATE_MENU:
            ...
            case LEFT_BUTTON:
                mSelectedMenuItem = mSelectedMenuItem == 0 ? (MAIN_MENU_ITEMS - 1) :
                                                             mSelectedMenuItem - 1;
                break;
        ...
        case STATE_GAME_OVER:
            if(button == A_BUTTON || button == B_BUTTON) {
                if(isRecord()) mRecords[mMapNumber] = mRound;
    ...
    void gameMenuConfirm() {
        if(getGameMenuItem(mSelectedMenuItem).mState != ITEM_STATE_AVAILABLE) return;

        if(buildSelectedTower()) return;
    ...
    bool buildSelectedTower() {
        if(mSelectedMenuItem >= 8) return false; // no tower selected
        ...
        if(tt.mPrice > mMoney && mCheat != CHEAT_MONEY) return false; // not enough money
    ...
    GameMenuItem getGameMenuItem(uint8_t index) {
        ...
        if(index < 13) // total items
        {
            ...
            TowerType tt = tile->mType == TILE_TOWER ? tile->mTower.getType() : towerTypes[0];
            ...
            if(index == GAME_MENU_UPGRADE2 && (tile->mTower.getTypeIndex() == TOWER_WATER ||
...
#define C(c)               \
    ...
    pos++;
            ...
            case TOWER_GUARD:
                C('g') C('u') C('a') C('r') C('d') C(0) break;
            case TOWER_CANNON:
                C('c') C('a') C('n') C('n') C('o') C('n') C(0) break;
            case TOWER_ICE:
                C('i') C('c') C('e') C(0) break;
            case TOWER_ELECTRO:
                C('e') C('l') C('e') C('c') C('t') C('r') C(0) break;
            case TOWER_SNIPER:
                C('s') C('n') C('i') C('p') C('e') C('r') C(0) break;
            case TOWER_MAGIC:
                C('m') C('a') C('g') C('i') C('c') C(0) break;
            case TOWER_WATER:
                C('w') C('a') C('t') C('e') C('r') C(0) break;
            case TOWER_FIRE:
                C('f') C('i') C('r') C('e') C(0) break;
            case GAME_MENU_UPGRADE1:
            case GAME_MENU_UPGRADE2:
                if(tile->mType == TILE_TOWER)
                    getUpgradeName(
                        tt.mUpgrades[index == GAME_MENU_UPGRADE1 ? 0 : 1], result.mText);
                else
                    result.mText[0] = 0;

                break;
            case GAME_MENU_DESTROY:
                C('s') C('e') C('l') C('l') C(0) break;
            case GAME_MENU_NEXT_WAVE:
                C('g') C('o') C('!') C(0) break;
            case GAME_MENU_QUIT:
                C('q') C('u') C('i') C('t') C(0) break;
            default:
                result.mText[0] = 0;
                break;
    ...
    void applySpeedAura(BytePosition center) {
        ...
        for(uint8_t y = y0; y <= y1; ++y)
            for(uint8_t x = x0; x <= x1; ++x) {
                if(x == center.mX && y == center.mY) continue;
                ...
                if(apply) t->mTower.setAttackProgress(min(7, t->mTower.getAttackProgress() + 2));
            }
    }
            ...
            for(uint8_t i = 0; i < mCreepCount; ++i) {
                ...
                if(!creep->isAlive() && creep->getTypeIndex() == CREEP_SPIDER_BIG &&
                   !creep->exited(mMap.getPaths())) {
                    ...
                    mCreepArray[i] = CreepInstance(
                        CREEP_SPIDER, mCreepArray[i].getPath(), mCreepArray[i].getPosition(), 0);

                    CreepInstance spider2 = CreepInstance(
                        CREEP_SPIDER,
                        mCreepArray[i].getPath(),
                        mCreepArray[i].getPosition() - 10,
                        0);
                    ...
                    if(!secondAdded) mCreepArray[i].setHealthLives(mCreepArray[i].getLives(), 1);
...
bool microtd_handle_back_long() {
    if(game.mState == STATE_MENU) return true; // long Back in main menu exits app
...
void drawConfirmDialog() {
    ...
    arduboy.fillRect(
        padding, padding, DISPLAY_WIDTH - 2 * padding, DISPLAY_HEIGHT - 2 * padding, WHITE);

    arduboy.drawRect(
        padding, padding, DISPLAY_WIDTH - 2 * padding, DISPLAY_HEIGHT - 2 * padding, BLACK);
...
void drawMenu() {
    ...
    Sprites::drawOverwrite(LOGO_X - TILE_WIDTH * 2 - margin, LOGO_Y, towerBigSprites, 0);
...
void drawMap() {
    ...
    for(uint8_t y = 0; y < TILES_Y; ++y)
        for(uint8_t x = 0; x < TILES_X; ++x) {
            ...
            if(t->mType == TILE_TOWER || // real tower
               (
                   // tower preview
                   game.mState == STATE_PLAYING_MENU && game.mCursor.mX == x &&
                   game.mCursor.mY == y && game.mSelectedMenuItem < 8 &&
                   (game.mFrame >> 3) % 4 != 0 &&
                   game.getGameMenuItem(game.mSelectedMenuItem).mState == ITEM_STATE_AVAILABLE)) {
                TowerType tt;
                ...
                if(tt.mSprite >= TOWER_WATER) {
                    ...
                    Sprites::drawOverwrite(
                        ...
                        (y - 1) * TILE_HEIGHT,
                        towerBigSprites,
                        (tt.mSprite == TOWER_WATER ? 0 : 2) + (t->mTower.upgraded(0) ? 1 : 0));
                } else {
                    ...
                    case TOWER_ELECTRO:
                        ...
                        if(attackPercentage < 50)
                            arduboy.drawLine(
                                selfPos.mX,
                                selfPos.mY,
                                p.mX - 1 + game.mFrame % 3,
                                p.mY - 1 + game.mFrame % 4,
                                BLACK);
    ...
    for(uint8_t i = 0; i < game.mCreepCount; ++i) {
        ...
        if(creep->entered() && creep->isAlive()) {
            ...
            Sprites::drawPlusMask(
                p.mX - TILE_WIDTH_HALF, p.mY - TILE_HEIGHT_HALF + bob, creepSprites, ct.mSprite);
        }
    }

    // draw cursor

    BytePosition corner(game.mCursor.mX * TILE_WIDTH, game.mCursor.mY * TILE_HEIGHT);
    ...
    for(uint8_t i = 0; i < TILE_WIDTH; ++i) {
        if(((game.mFrame >> 3) + i) % 3 == 0) {
            ...
            arduboy.drawPixel(corner.mX + TILE_WIDTH - i, corner.mY + TILE_HEIGHT - 1, WHITE);
    ...
    if(game.mState == STATE_PLAYING_WAVE && arduboy.pressed(A_BUTTON)) {
        ...
        for(uint8_t i = 0; i < game.mCreepCount; ++i) {
            ...
            if(creep.entered() && creep.isAlive()) {
                ...
                uint8_t healthFraction = ((uint16_t)creep.getHealth()) * 5 / ct.mMaxHealth;
...
#define helperCond(u, i, upgrade)                                                 \
    (game.mSelectedMenuItem == GAME_MENU_UPGRADE##u &&                            \
     game.getGameMenuItem(GAME_MENU_UPGRADE##u).mState == ITEM_STATE_AVAILABLE && \
     cursorTile->mTower.getType().mUpgrades[i] == upgrade)
            ...
            else if(helperCond(1, 0, UPGRADE_SPEED_AURA) || helperCond(2, 1, UPGRADE_SPEED_AURA))
...
#undef helperCond
    ...
    if(rangeToDraw != 255) {
        ...
        arduboy.drawCircle(rangeToDrawCenter.mX, rangeToDrawCenter.mY, rangeToDraw, BLACK);
    ...
    if(game.mState == STATE_PLAYING_MENU) {
        ...
        arduboy.fillRect(
            0, DISPLAY_HEIGHT - TILE_HEIGHT - 1, DISPLAY_WIDTH, TILE_HEIGHT + 1, WHITE);

        arduboy.drawFastHLine(0, DISPLAY_HEIGHT - TILE_HEIGHT - 1, DISPLAY_WIDTH, BLACK);
        ...
        for(uint8_t i = 0; i < DISPLAY_WIDTH / TILE_WIDTH; ++i) {
            ...
            if(item.mState == ITEM_STATE_NULL) break;
    ...
    if(checkRepeat) {
        if(arduboy.pressed(button)) {
            if(buttonRepeatCounter == 0) {
                if(arduboy.frameCount() % BUTTON_REPEAT_PERIOD == 0) game.buttonDown(button);
            } else
                buttonRepeatCounter--;
...
void loop() {
    ...
    uint8_t updates = game.mState == STATE_PLAYING_WAVE && arduboy.pressed(A_BUTTON) &&
    ...
    if(game.mState == STATE_CONFIRM) drawConfirmDialog();
...
string toStr(TowerInstance t) {
    ...
    if(t.upgraded(0)) result += "0";

    if(t.upgraded(1)) result += "1";
...
string toStr(Tile t) {
    ...
    case TILE_TOWER:
        return "T";
        break;
    case TILE_CREEP_START:
        return "S";
        break;
    case TILE_CREEP_EXIT:
        return "E";
        break;
