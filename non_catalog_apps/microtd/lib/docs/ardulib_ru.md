...
- **`ARDULIB_DISABLE_EXIT`** — Отключить стандартный выход по удержанию `Return/Back`, если игра реализует собственный backend выхода.
...
- `arduboy.exit()` - Завершение runtime, короткий алиас для `arduboy.exitToBootloader()`
