...
- **`ARDULIB_DISABLE_EXIT`** — Disable the standard `Return/Back` hold-to-exit behavior if the game provides its own exit backend.
...
- `arduboy.exit()` - Stop the runtime, short alias for `arduboy.exitToBootloader()`
