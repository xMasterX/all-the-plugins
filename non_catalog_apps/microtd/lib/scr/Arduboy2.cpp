...
void Arduboy2Base::begin(
    ...
    frame_count_ = 0;
    last_frame_ms_ = 0;
#ifndef ARDULIB_DISABLE_EXIT
    __atomic_store_n(&exit_input_held_, 0, __ATOMIC_RELAXED);
    __atomic_store_n(&exit_timer_running_, false, __ATOMIC_RELAXED);
    __atomic_store_n(&exit_timer_blocked_, false, __ATOMIC_RELAXED);
    __atomic_store_n(&exit_timer_started_ms_, 0, __ATOMIC_RELAXED);
#endif
    resetInputState();
    audio.begin();
}
...
#ifndef ARDULIB_DISABLE_EXIT
    arduboy_ptr->handleExitInput_(event);
#endif
...
#ifndef ARDULIB_DISABLE_EXIT
void Arduboy2Base::pollExitTimer() {
    const uint8_t return_mask = ExitInputMaskFromKey_(InputKeyBack);
    const uint8_t other_mask = (uint8_t)~return_mask;
    const uint8_t held = __atomic_load_n(&exit_input_held_, __ATOMIC_RELAXED);
    const bool timer_running = __atomic_load_n(&exit_timer_running_, __ATOMIC_RELAXED);
    const bool timer_blocked = __atomic_load_n(&exit_timer_blocked_, __ATOMIC_RELAXED);

    if(timer_running && (held == return_mask) && !timer_blocked) {
        const uint32_t now = millis();
        const uint32_t started_ms = __atomic_load_n(&exit_timer_started_ms_, __ATOMIC_RELAXED);
        if((uint32_t)(now - started_ms) >= 500u) {
            exit();
        }
    } else if((held & return_mask) == 0) {
        __atomic_store_n(&exit_timer_running_, false, __ATOMIC_RELAXED);
        __atomic_store_n(&exit_timer_blocked_, false, __ATOMIC_RELAXED);
    } else if((held & other_mask) != 0) {
        __atomic_store_n(&exit_timer_running_, false, __ATOMIC_RELAXED);
        __atomic_store_n(&exit_timer_blocked_, true, __ATOMIC_RELAXED);
    }
}
#endif
...
#ifndef ARDULIB_DISABLE_EXIT
uint8_t Arduboy2Base::ExitInputMaskFromKey_(InputKey key) {
    switch(key) {
    case InputKeyBack:
        return 0x01;
    case InputKeyUp:
...
void Arduboy2Base::handleExitInput_(const InputEvent* event) {
    const uint8_t key_mask = ExitInputMaskFromKey_(event->key);
    if(!key_mask) return;

    const uint8_t return_mask = ExitInputMaskFromKey_(InputKeyBack);
    const bool is_return = (key_mask == return_mask);

    if(event->type == InputTypePress) {
        const uint8_t held = __atomic_load_n(&exit_input_held_, __ATOMIC_RELAXED) | key_mask;
        __atomic_store_n(&exit_input_held_, held, __ATOMIC_RELAXED);

        if(is_return) {
            if(held == return_mask) {
                __atomic_store_n(&exit_timer_running_, true, __ATOMIC_RELAXED);
                __atomic_store_n(&exit_timer_blocked_, false, __ATOMIC_RELAXED);
                __atomic_store_n(&exit_timer_started_ms_, millis(), __ATOMIC_RELAXED);
            } else {
                __atomic_store_n(&exit_timer_running_, false, __ATOMIC_RELAXED);
                __atomic_store_n(&exit_timer_blocked_, true, __ATOMIC_RELAXED);
            }
        } else if(held & return_mask) {
            __atomic_store_n(&exit_timer_running_, false, __ATOMIC_RELAXED);
            __atomic_store_n(&exit_timer_blocked_, true, __ATOMIC_RELAXED);
        }
    } else if(event->type == InputTypeRelease) {
        const uint8_t held = __atomic_load_n(&exit_input_held_, __ATOMIC_RELAXED) & (uint8_t)~key_mask;
        __atomic_store_n(&exit_input_held_, held, __ATOMIC_RELAXED);
