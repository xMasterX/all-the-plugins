...
typedef struct {
    const Custom_TextInputKey* rows[3];
...
typedef struct {
    ...
    Custom_TextInputValidatorCallback validator_callback;
    void* validator_callback_context;
    FuriString* validator_text;
    bool validator_message_visible;
} Custom_TextInputModel;
...
static const Custom_TextInputKey keyboard_keys_row_1[] = {
    {'q', 1, 8},
    {'w', 10, 8},
    {'e', 19, 8},
    {'r', 28, 8},
...
static const Custom_TextInputKey keyboard_keys_row_2[] = {
    {'a', 1, 20},
    {'s', 10, 20},
    {'d', 19, 20},
    {'f', 28, 20},
...
static const Custom_TextInputKey keyboard_keys_row_3[] = {
...
static const Custom_TextInputKey symbol_keyboard_keys_row_1[] = {
    {'!', 2, 8},
    {'@', 12, 8},
    {'#', 22, 8},
    {'$', 32, 8},
...
static const Custom_TextInputKey symbol_keyboard_keys_row_2[] = {
    {'~', 2, 20},
    {'+', 12, 20},
    {'-', 22, 20},
    {'=', 32, 20},
...
static const Custom_TextInputKey symbol_keyboard_keys_row_3[] = {
    {SWITCH_KEYBOARD_KEY, 1, 23},
    {'.', 15, 32},
    {',', 29, 32},
    {':', 41, 32},
...
static void switch_keyboard(Custom_TextInputModel* model) {
...
static const Custom_TextInputKey* get_row(const Keyboard* keyboard, uint8_t row_index) {
    const Custom_TextInputKey* row = NULL;
    if(row_index < 3) {
...
static char get_selected_char(Custom_TextInputModel* model) {
    return get_row(
...
static void custom_text_input_backspace_cb(Custom_TextInputModel* model) {
...
static void custom_text_input_view_draw_callback(Canvas* canvas, void* _model) {
    Custom_TextInputModel* model = _model;
    ...
    for(uint8_t row = 0; row < keyboard_row_count; row++) {
        ...
        const Custom_TextInputKey* keys = get_row(keyboards[model->selected_keyboard], row);
...
static void
    custom_text_input_handle_up(Custom_TextInput* custom_text_input, Custom_TextInputModel* model) {
    UNUSED(custom_text_input);
...
static void custom_text_input_handle_down(
    Custom_TextInput* custom_text_input,
    Custom_TextInputModel* model) {
    UNUSED(custom_text_input);
...
static void custom_text_input_handle_left(
    Custom_TextInput* custom_text_input,
    Custom_TextInputModel* model) {
    UNUSED(custom_text_input);
    if(model->cursor_select) {
...
static void custom_text_input_handle_right(
    Custom_TextInput* custom_text_input,
    Custom_TextInputModel* model) {
    UNUSED(custom_text_input);
    if(model->cursor_select) {
...
static void custom_text_input_handle_ok(
    Custom_TextInput* custom_text_input,
    Custom_TextInputModel* model,
    InputType type) {
    ...
    if(selected == ENTER_KEY) {
        if(model->validator_callback &&
           (!model->validator_callback(
            ...
            furi_timer_start(custom_text_input->timer, furi_kernel_get_tick_frequency() * 4);
        ...
        if(selected == BACKSPACE_KEY) {
            custom_text_input_backspace_cb(model);
        } else if(!repeat) {
            if(model->clear_default_text) {
                text_length = 0;
            }
...
static bool custom_text_input_view_input_callback(InputEvent* event, void* context) {
    Custom_TextInput* custom_text_input = context;
    furi_assert(custom_text_input);

    bool consumed = false;

    // Acquire model
    Custom_TextInputModel* model = view_get_model(custom_text_input->view);
    ...
    if((!(event->type == InputTypePress) && !(event->type == InputTypeRelease)) &&
       model->validator_message_visible) {
        ...
        switch(event->key) {
        case InputKeyUp:
            custom_text_input_handle_up(custom_text_input, model);
            break;
        case InputKeyDown:
            custom_text_input_handle_down(custom_text_input, model);
            break;
        case InputKeyLeft:
            custom_text_input_handle_left(custom_text_input, model);
            break;
        case InputKeyRight:
            custom_text_input_handle_right(custom_text_input, model);
            break;
        case InputKeyOk:
            custom_text_input_handle_ok(custom_text_input, model, event->type);
            break;
        default:
            consumed = false;
            break;
    ...
    // Commit model
    view_commit_model(custom_text_input->view, consumed);

    return consumed;
}

void custom_text_input_timer_callback(void* context) {
    furi_assert(context);
    Custom_TextInput* custom_text_input = context;

    with_view_model(
        custom_text_input->view,
        Custom_TextInputModel * model,
        { model->validator_message_visible = false; },
        true);
}
...
Custom_TextInput* custom_text_input_alloc() {
    Custom_TextInput* custom_text_input = malloc(sizeof(Custom_TextInput));
    custom_text_input->view = view_alloc();
    view_set_context(custom_text_input->view, custom_text_input);
    view_allocate_model(
        custom_text_input->view, ViewModelTypeLocking, sizeof(Custom_TextInputModel));
    view_set_draw_callback(custom_text_input->view, custom_text_input_view_draw_callback);
    view_set_input_callback(custom_text_input->view, custom_text_input_view_input_callback);

    custom_text_input->timer =
        furi_timer_alloc(custom_text_input_timer_callback, FuriTimerTypeOnce, custom_text_input);
    ...
    custom_text_input_reset(custom_text_input);

    return custom_text_input;
}

void custom_text_input_free(Custom_TextInput* custom_text_input) {
    furi_assert(custom_text_input);
    with_view_model(
        custom_text_input->view,
        Custom_TextInputModel * model,
    ...
    // Send stop command
    furi_timer_stop(custom_text_input->timer);
    // Release allocated memory
    furi_timer_free(custom_text_input->timer);

    view_free(custom_text_input->view);

    free(custom_text_input);
}

void custom_text_input_reset(Custom_TextInput* custom_text_input) {
    furi_assert(custom_text_input);
    with_view_model(
        custom_text_input->view,
        Custom_TextInputModel * model,
...
View* custom_text_input_get_view(Custom_TextInput* custom_text_input) {
    furi_assert(custom_text_input);
    return custom_text_input->view;
}
...
void custom_text_input_set_minimum_length(
    Custom_TextInput* custom_text_input,
    size_t minimum_length) {
    with_view_model(
        custom_text_input->view,
...
Custom_TextInputValidatorCallback
    custom_text_input_get_validator_callback(Custom_TextInput* custom_text_input) {
    Custom_TextInputValidatorCallback validator_callback = NULL;
    with_view_model(
        custom_text_input->view,
        Custom_TextInputModel * model,
...
void* custom_text_input_get_validator_callback_context(Custom_TextInput* custom_text_input) {
...
void custom_text_input_set_header_text(Custom_TextInput* custom_text_input, const char* text) {
    with_view_model(
        custom_text_input->view, Custom_TextInputModel * model, { model->header = text; }, true);
}
