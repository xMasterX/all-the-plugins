...
void scene_settings_on_enter(void* ctx) {
    ...
    item =
        variable_item_list_add(vil, "Vibration", 2, vibration_cb, context); // Index 1 (vibration)
    variable_item_set_current_value_index(item, context->game_state->settings.vibration);
    variable_item_set_current_value_text(
        item, off_on_options[context->game_state->settings.vibration]);
    ...
    variable_item_set_current_value_text(
        item, off_on_options[context->game_state->settings.sound]);
