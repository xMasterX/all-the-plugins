...
static void
    scene_main_refresh_view(ButtonPanel* button_panel, struct ApplicationContext* context) {
    uint16_t x, y;
    ...
    button_panel_add_item(
        ...
        0,
        0, // Location in the matrix declared above (x, y)
        70,
        6, // Coordinates where to draw the icon (x, y)
    ...
    button_panel_add_item(
        ...
        candy_button, // Index
        0,
        1, // Location in the matrix declared above (x, y)
        70,
        38, // Coordinates where to draw the icon (x, y)
...
bool scene_main_on_event(void* ctx, SceneManagerEvent event) {
    ...
    case SceneManagerEventTypeBack:
        ...
        view_dispatcher_stop(
            context->view_dispatcher); // Stop the view dispatcher to terminate the GUI
        return true;
