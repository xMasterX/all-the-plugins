...
void menu_callback_main_menu(void* context, uint32_t index) {
    ...
    case QuickStart_Menu:
        scene_manager_handle_custom_event(app->scene_manager, AppEvent_ShowQuickStart);
        break;
    case CreateTask_Menu:
        scene_manager_handle_custom_event(app->scene_manager, AppEvent_ShowCreateTask);
        break;
    case ViewTasks_Menu:
...
void scene_on_enter_main_menu(void* context) {
    ...
    if(app->tasks->array != NULL) {
        ...
        for(size_t i = 0; i < app->tasks->size; i++) {
            ...
            if(!task->completed) {
                uint32_t task_timestamp = datetime_datetime_to_timestamp(&task->last_start_time);
