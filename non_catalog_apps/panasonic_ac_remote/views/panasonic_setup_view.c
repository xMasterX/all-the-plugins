#include "panasonic_setup_view.h"
...
typedef struct {
    ...
    uint8_t focus;
} PanasonicSetupViewModel;

struct PanasonicSetupView {
    View* view;
};

static uint8_t row_count(void) {
    return panasonic_ir_get_option_count() > 0 ? 2 : 1;
}
...
static void panasonic_setup_view_draw(Canvas* canvas, void* model) {
    PanasonicSetupViewModel* m = model;
    if(!m->state) return;
    ...
    bool has_opt = panasonic_ir_get_option_count() > 0;
    ...
    if(has_opt) {
        ...
        canvas_set_font(canvas, FontSecondary);
        canvas_draw_str_aligned(
            canvas, 32, oy + 3, AlignCenter, AlignTop, panasonic_ir_get_option_label());
        ...
        canvas_draw_str_aligned(
            ...
            AlignCenter,
            AlignTop,
            panasonic_ir_get_option_name(m->state->option));
        ...
        snprintf(
            ...
            (unsigned)panasonic_ir_get_option_count());
...
static bool panasonic_setup_view_input(InputEvent* event, void* context) {
    PanasonicSetupView* view = context;
    bool consumed = false;

    with_view_model(
        view->view,
        PanasonicSetupViewModel * m,
        {
            if(event->type != InputTypeShort && event->type != InputTypeRepeat) {
                // nothing
            } else {
                uint8_t rows = row_count();
                uint8_t opts = panasonic_ir_get_option_count();
                switch(event->key) {
                case InputKeyUp:
...
PanasonicSetupView* panasonic_setup_view_alloc(void) {
    PanasonicSetupView* view = malloc(sizeof(PanasonicSetupView));
    view->view = view_alloc();

    view_allocate_model(view->view, ViewModelTypeLocking, sizeof(PanasonicSetupViewModel));
    view_set_context(view->view, view);
    view_set_draw_callback(view->view, panasonic_setup_view_draw);
    view_set_input_callback(view->view, panasonic_setup_view_input);
...
void panasonic_setup_view_set_state(PanasonicSetupView* view, PanasonicState* state) {
    with_view_model(view->view, PanasonicSetupViewModel * m, { m->state = state; }, true);
}
