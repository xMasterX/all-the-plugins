...
struct BalluExtraView {
    View* view;
    BalluExtraViewSendCallback send_callback;
    void* send_context;
};

static void ballu_extra_view_draw(Canvas* canvas, void* model) {
    BalluExtraViewModel* m = model;
    if(!m->state) return;
    ...
    uint8_t total = (uint8_t)BalluExtraCount;
    ...
    for(uint8_t r = 0; r < shown; r++) {
        ...
        canvas_draw_str_aligned(
            canvas, 32, y + 2, AlignCenter, AlignTop, ballu_ir_get_extra_name((BalluExtra)i));
    ...
    BalluRequest req = ballu_state_request(m->state);
    ballu_ir_format_extra(&req, (BalluExtra)m->selected, buf, sizeof(buf));
...
static bool ballu_extra_view_input(InputEvent* event, void* context) {
    BalluExtraView* view = context;
    bool consumed = false;
    bool send = false;
    BalluExtra to_send = (BalluExtra)0;

    with_view_model(
        view->view,
        BalluExtraViewModel * m,
        {
            if(event->type == InputTypeShort || event->type == InputTypeRepeat) {
                ...
                case InputKeyUp:
                    ...
                    break;
                case InputKeyDown:
                    if(m->selected + 1 < (uint8_t)BalluExtraCount) m->selected++;
...
BalluExtraView* ballu_extra_view_alloc(void) {
    BalluExtraView* view = malloc(sizeof(BalluExtraView));
    view->view = view_alloc();
    view->send_callback = NULL;
    view->send_context = NULL;

    view_allocate_model(view->view, ViewModelTypeLocking, sizeof(BalluExtraViewModel));
    view_set_context(view->view, view);
    view_set_draw_callback(view->view, ballu_extra_view_draw);
    view_set_input_callback(view->view, ballu_extra_view_input);
...
void ballu_extra_view_set_state(BalluExtraView* view, BalluState* state) {
    with_view_model(view->view, BalluExtraViewModel * m, { m->state = state; }, true);
}

void ballu_extra_view_set_last_sent(BalluExtraView* view, const char* last_sent) {
    with_view_model(view->view, BalluExtraViewModel * m, { m->last_sent = last_sent; }, true);
}
