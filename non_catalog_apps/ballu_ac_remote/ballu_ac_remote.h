#pragma once

#include <furi.h>
#include <gui/gui.h>
#include <gui/view_dispatcher.h>
#include <notification/notification_messages.h>
#include <infrared_worker.h>

#include "ballu_state.h"
#include "ballu_ir_protocol.h"
#include "views/ballu_main_view.h"
#include "views/ballu_extra_view.h"
#include "views/ballu_setup_view.h"

// View IDs
typedef enum {
    BalluViewMain = 0,
    BalluViewExtra,
    BalluViewSetup,
} BalluViewId;

// Main application structure
typedef struct {
    // System
    Gui* gui;
    ViewDispatcher* view_dispatcher;
    NotificationApp* notifications;

    // Views
    BalluMainView* main_view;
    BalluExtraView* extra_view;
    BalluSetupView* setup_view;

    // State
    BalluState* state;

    // IR
    InfraredWorker* ir_worker;
    uint32_t ir_timings[BALLU_IR_MAX_TIMINGS];
    size_t ir_timings_count;

    // Payload of the most recent transmission, shown on the Extra screen.
    // The Extra view holds a pointer to this, so it is always current.
    char last_sent[BALLU_CODE_STR_LEN];

    // Animation timer
    FuriTimer* send_timer;
    bool is_sending;
    uint8_t anim_frame;

    // IR stop timer (non-blocking TX stop)
    FuriTimer* ir_tx_stop_timer;

    // Current view tracking
    BalluViewId current_view;
} BalluApp;
