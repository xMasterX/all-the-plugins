...
} DaikinExtraViewModel;

struct DaikinExtraView {
    View* view;
    DaikinExtraViewSendCallback send_callback;
    void* send_context;
};

static void daikin_extra_view_draw(Canvas* canvas, void* model) {
    DaikinExtraViewModel* m = model;
    if(!m->state) return;
    ...
    uint8_t total = (uint8_t)DaikinExtraCount;
    ...
    for(uint8_t r = 0; r < shown; r++) {
        ...
        canvas_draw_str_aligned(
            canvas, 32, y + 2, AlignCenter, AlignTop, daikin_ir_get_extra_name((DaikinExtra)i));
    ...
    DaikinRequest req = daikin_state_request(m->state);
    daikin_ir_format_extra(&req, (DaikinExtra)m->selected, buf, sizeof(buf));
...
static bool daikin_extra_view_input(InputEvent* event, void* context) {
    DaikinExtraView* view = context;
    bool consumed = false;
    bool send = false;
    DaikinExtra to_send = (DaikinExtra)0;

    with_view_model(
        view->view,
        DaikinExtraViewModel * m,
        {
            if(event->type == InputTypeShort || event->type == InputTypeRepeat) {
                ...
                case InputKeyUp:
                    ...
                    break;
                case InputKeyDown:
                    if(m->selected + 1 < (uint8_t)DaikinExtraCount) m->selected++;
                ...
                case InputKeyOk:
                    if(event->type == InputTypeShort) {
                        to_send = (DaikinExtra)m->selected;
                        send = true;
                    }
                    consumed = true;
                    break;
...
DaikinExtraView* daikin_extra_view_alloc(void) {
    DaikinExtraView* view = malloc(sizeof(DaikinExtraView));
    view->view = view_alloc();
    view->send_callback = NULL;
    view->send_context = NULL;

    view_allocate_model(view->view, ViewModelTypeLocking, sizeof(DaikinExtraViewModel));
    view_set_context(view->view, view);
    view_set_draw_callback(view->view, daikin_extra_view_draw);
    view_set_input_callback(view->view, daikin_extra_view_input);
...
void daikin_extra_view_set_state(DaikinExtraView* view, DaikinState* state) {
    with_view_model(view->view, DaikinExtraViewModel * m, { m->state = state; }, true);
}

void daikin_extra_view_set_last_sent(DaikinExtraView* view, const char* last_sent) {
    with_view_model(view->view, DaikinExtraViewModel * m, { m->last_sent = last_sent; }, true);
}
