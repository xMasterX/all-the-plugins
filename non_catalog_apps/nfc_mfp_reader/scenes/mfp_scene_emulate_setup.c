...
static void emulate_setup_cb(void* ctx, uint32_t index) {
    ...
    if(app->allow_overwrite != new_state) {
        ...
        if(app->save_path[0] != '\0' && storage_file_exists(app->storage, app->save_path)) {
...
void mfp_scene_emulate_setup_on_enter(void* ctx) {
    ...
    submenu_add_item(app->submenu, "Writable", EmulateSetupWritable, emulate_setup_cb, app);
    submenu_add_item(
        app->submenu, "Read-only (discard)", EmulateSetupReadOnly, emulate_setup_cb, app);
