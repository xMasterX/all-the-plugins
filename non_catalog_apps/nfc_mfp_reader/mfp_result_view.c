...
static void draw_stats(Canvas* canvas, const MfpResultViewModel* m) {
    ...
    char frac[16];
    snprintf(frac, sizeof(frac), "%u/%u", (unsigned)m->sectors_ok, (unsigned)m->total_sectors);
...
void mfp_result_view_set_actions_callback(
    MfpResultView* inst,
    MfpResultViewActionsCallback cb,
    void* ctx) {
    inst->actions_cb = cb;
