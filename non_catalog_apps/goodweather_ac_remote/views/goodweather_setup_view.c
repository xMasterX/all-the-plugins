#include "goodweather_setup_view.h"
...
typedef struct {
    ...
    uint8_t focus;
} GoodweatherSetupViewModel;

struct GoodweatherSetupView {
    View* view;
};

static uint8_t row_count(void) {
    return goodweather_ir_get_option_count() > 0 ? 2 : 1;
}
...
static void goodweather_setup_view_draw(Canvas* canvas, void* model) {
    GoodweatherSetupViewModel* m = model;
    if(!m->state) return;
    ...
    bool has_opt = goodweather_ir_get_option_count() > 0;
    ...
    if(has_opt) {
        ...
        canvas_set_font(canvas, FontSecondary);
        canvas_draw_str_aligned(
            canvas, 32, oy + 3, AlignCenter, AlignTop, goodweather_ir_get_option_label());
        ...
        canvas_draw_str_aligned(
            ...
            AlignCenter,
            AlignTop,
            goodweather_ir_get_option_name(m->state->option));
        ...
        snprintf(
            ...
            (unsigned)goodweather_ir_get_option_count());
...
static bool goodweather_setup_view_input(InputEvent* event, void* context) {
    GoodweatherSetupView* view = context;
    bool consumed = false;

    with_view_model(
        view->view,
        GoodweatherSetupViewModel * m,
        {
            if(event->type != InputTypeShort && event->type != InputTypeRepeat) {
                // nothing
            } else {
                uint8_t rows = row_count();
                uint8_t opts = goodweather_ir_get_option_count();
                switch(event->key) {
                case InputKeyUp:
...
GoodweatherSetupView* goodweather_setup_view_alloc(void) {
    GoodweatherSetupView* view = malloc(sizeof(GoodweatherSetupView));
    view->view = view_alloc();

    view_allocate_model(view->view, ViewModelTypeLocking, sizeof(GoodweatherSetupViewModel));
    view_set_context(view->view, view);
    view_set_draw_callback(view->view, goodweather_setup_view_draw);
    view_set_input_callback(view->view, goodweather_setup_view_input);
...
void goodweather_setup_view_set_state(GoodweatherSetupView* view, GoodweatherState* state) {
    with_view_model(view->view, GoodweatherSetupViewModel * m, { m->state = state; }, true);
}
