...
typedef struct {
    ToshibaState* state;
    uint8_t focus;
} ToshibaSetupViewModel;

struct ToshibaSetupView {
    View* view;
};

static uint8_t row_count(void) {
    return toshiba_ir_get_option_count() > 0 ? 2 : 1;
}
...
static void toshiba_setup_view_draw(Canvas* canvas, void* model) {
    ToshibaSetupViewModel* m = model;
    if(!m->state) return;
    ...
    bool has_opt = toshiba_ir_get_option_count() > 0;
    ...
    if(has_opt) {
        ...
        canvas_set_font(canvas, FontSecondary);
        canvas_draw_str_aligned(
            canvas, 32, oy + 3, AlignCenter, AlignTop, toshiba_ir_get_option_label());
        ...
        canvas_draw_str_aligned(
            ...
            AlignCenter,
            AlignTop,
            toshiba_ir_get_option_name(m->state->option));
        ...
        snprintf(
            ...
            (unsigned)toshiba_ir_get_option_count());
...
static bool toshiba_setup_view_input(InputEvent* event, void* context) {
    ToshibaSetupView* view = context;
    bool consumed = false;

    with_view_model(
        view->view,
        ToshibaSetupViewModel * m,
        {
            if(event->type != InputTypeShort && event->type != InputTypeRepeat) {
                // nothing
            } else {
                uint8_t rows = row_count();
                uint8_t opts = toshiba_ir_get_option_count();
                switch(event->key) {
                case InputKeyUp:
...
ToshibaSetupView* toshiba_setup_view_alloc(void) {
    ToshibaSetupView* view = malloc(sizeof(ToshibaSetupView));
    view->view = view_alloc();

    view_allocate_model(view->view, ViewModelTypeLocking, sizeof(ToshibaSetupViewModel));
    view_set_context(view->view, view);
    view_set_draw_callback(view->view, toshiba_setup_view_draw);
    view_set_input_callback(view->view, toshiba_setup_view_input);
...
void toshiba_setup_view_set_state(ToshibaSetupView* view, ToshibaState* state) {
    with_view_model(view->view, ToshibaSetupViewModel * m, { m->state = state; }, true);
}
