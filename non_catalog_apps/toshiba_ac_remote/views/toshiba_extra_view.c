...
} ToshibaExtraViewModel;

struct ToshibaExtraView {
    View* view;
    ToshibaExtraViewSendCallback send_callback;
    void* send_context;
};

static void toshiba_extra_view_draw(Canvas* canvas, void* model) {
    ToshibaExtraViewModel* m = model;
    if(!m->state) return;
    ...
    uint8_t total = (uint8_t)ToshibaExtraCount;
    ...
    for(uint8_t r = 0; r < shown; r++) {
        ...
        canvas_draw_str_aligned(
            canvas, 32, y + 2, AlignCenter, AlignTop, toshiba_ir_get_extra_name((ToshibaExtra)i));
    ...
    ToshibaRequest req = toshiba_state_request(m->state);
    toshiba_ir_format_extra(&req, (ToshibaExtra)m->selected, buf, sizeof(buf));
...
static bool toshiba_extra_view_input(InputEvent* event, void* context) {
    ToshibaExtraView* view = context;
    bool consumed = false;
    bool send = false;
    ToshibaExtra to_send = (ToshibaExtra)0;

    with_view_model(
        view->view,
        ToshibaExtraViewModel * m,
        {
            if(event->type == InputTypeShort || event->type == InputTypeRepeat) {
                ...
                case InputKeyUp:
                    ...
                    break;
                case InputKeyDown:
                    if(m->selected + 1 < (uint8_t)ToshibaExtraCount) m->selected++;
                ...
                case InputKeyOk:
                    if(event->type == InputTypeShort) {
                        to_send = (ToshibaExtra)m->selected;
                        send = true;
                    }
                    consumed = true;
                    break;
...
ToshibaExtraView* toshiba_extra_view_alloc(void) {
    ToshibaExtraView* view = malloc(sizeof(ToshibaExtraView));
    view->view = view_alloc();
    view->send_callback = NULL;
    view->send_context = NULL;

    view_allocate_model(view->view, ViewModelTypeLocking, sizeof(ToshibaExtraViewModel));
    view_set_context(view->view, view);
    view_set_draw_callback(view->view, toshiba_extra_view_draw);
    view_set_input_callback(view->view, toshiba_extra_view_input);
...
void toshiba_extra_view_set_state(ToshibaExtraView* view, ToshibaState* state) {
    with_view_model(view->view, ToshibaExtraViewModel * m, { m->state = state; }, true);
}

void toshiba_extra_view_set_last_sent(ToshibaExtraView* view, const char* last_sent) {
    with_view_model(view->view, ToshibaExtraViewModel * m, { m->last_sent = last_sent; }, true);
}
