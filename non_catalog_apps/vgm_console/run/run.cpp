...
VideoGameModuleConsoleRun::VideoGameModuleConsoleRun(void* appContext)
...
void VideoGameModuleConsoleRun::drawCommand(Canvas* canvas, DrawCommandType type, const char* data) {
    ...
    case DRAW_COMMAND_BLIT: {
        ...
        for(uint16_t j = 0; j < (uint16_t)height; j++) {
            ...
            for(int i = 0; i < width && i < 128; i++) {
                ...
                if(pixel >= 0xE6) xbm[i / 8] |= (uint8_t)(1u << (i % 8));
            }
    ...
    case DRAW_COMMAND_BLIT1: {
        ...
        for(int j = 0; j < height; j++) {
            ...
            for(int k = 0; k < 16; k++) {
                uint8_t b = row_raw[k];
                if(b == 0x02) b = 0x00; // unescape all-black
                if(b == 0x0B) b = 0x0A; // unescape newline
                xbm[k] = b;
                inv_xbm[k] = ~b;
            }
    ...
    case InputKeyOk:
        return app->sendData(
            event->type == InputTypeShort ?
                "0" :
                "3"); // OK sends "0" for A or "3" for Start if long pressed
    case InputKeyBack:
