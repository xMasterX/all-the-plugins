...
typedef struct {
    NeoclimaState* state;
    uint8_t focus;
} NeoclimaSetupViewModel;

struct NeoclimaSetupView {
    View* view;
};

static uint8_t row_count(void) {
    return neoclima_ir_get_option_count() > 0 ? 2 : 1;
}
...
static void neoclima_setup_view_draw(Canvas* canvas, void* model) {
    NeoclimaSetupViewModel* m = model;
    if(!m->state) return;
    ...
    bool has_opt = neoclima_ir_get_option_count() > 0;
    ...
    if(has_opt) {
        ...
        canvas_set_font(canvas, FontSecondary);
        canvas_draw_str_aligned(
            canvas, 32, oy + 3, AlignCenter, AlignTop, neoclima_ir_get_option_label());
        ...
        canvas_draw_str_aligned(
            ...
            AlignCenter,
            AlignTop,
            neoclima_ir_get_option_name(m->state->option));
        ...
        snprintf(
            ...
            (unsigned)neoclima_ir_get_option_count());
...
static bool neoclima_setup_view_input(InputEvent* event, void* context) {
    NeoclimaSetupView* view = context;
    bool consumed = false;

    with_view_model(
        view->view,
        NeoclimaSetupViewModel * m,
        {
            if(event->type != InputTypeShort && event->type != InputTypeRepeat) {
                // nothing
            } else {
                uint8_t rows = row_count();
                uint8_t opts = neoclima_ir_get_option_count();
                switch(event->key) {
                case InputKeyUp:
...
NeoclimaSetupView* neoclima_setup_view_alloc(void) {
    NeoclimaSetupView* view = malloc(sizeof(NeoclimaSetupView));
    view->view = view_alloc();

    view_allocate_model(view->view, ViewModelTypeLocking, sizeof(NeoclimaSetupViewModel));
    view_set_context(view->view, view);
    view_set_draw_callback(view->view, neoclima_setup_view_draw);
    view_set_input_callback(view->view, neoclima_setup_view_input);
...
void neoclima_setup_view_set_state(NeoclimaSetupView* view, NeoclimaState* state) {
    with_view_model(view->view, NeoclimaSetupViewModel * m, { m->state = state; }, true);
}
