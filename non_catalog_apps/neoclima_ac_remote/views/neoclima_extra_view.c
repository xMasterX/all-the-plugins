...
} NeoclimaExtraViewModel;

struct NeoclimaExtraView {
    View* view;
    NeoclimaExtraViewSendCallback send_callback;
    void* send_context;
};

static void neoclima_extra_view_draw(Canvas* canvas, void* model) {
    NeoclimaExtraViewModel* m = model;
    if(!m->state) return;
    ...
    uint8_t total = (uint8_t)NeoclimaExtraCount;
    ...
    for(uint8_t r = 0; r < shown; r++) {
        ...
        canvas_draw_str_aligned(
            canvas, 32, y + 2, AlignCenter, AlignTop, neoclima_ir_get_extra_name((NeoclimaExtra)i));
    ...
    char buf[NEOCLIMA_CODE_STR_LEN];
    ...
    NeoclimaRequest req = neoclima_state_request(m->state);
    neoclima_ir_format_extra(&req, (NeoclimaExtra)m->selected, buf, sizeof(buf));
...
static bool neoclima_extra_view_input(InputEvent* event, void* context) {
    NeoclimaExtraView* view = context;
    bool consumed = false;
    bool send = false;
    NeoclimaExtra to_send = (NeoclimaExtra)0;

    with_view_model(
        view->view,
        NeoclimaExtraViewModel * m,
        {
            if(event->type == InputTypeShort || event->type == InputTypeRepeat) {
                ...
                case InputKeyUp:
                    ...
                    break;
                case InputKeyDown:
                    if(m->selected + 1 < (uint8_t)NeoclimaExtraCount) m->selected++;
                ...
                case InputKeyOk:
                    if(event->type == InputTypeShort) {
                        to_send = (NeoclimaExtra)m->selected;
                        send = true;
                    }
                    consumed = true;
                    break;
...
NeoclimaExtraView* neoclima_extra_view_alloc(void) {
    NeoclimaExtraView* view = malloc(sizeof(NeoclimaExtraView));
    view->view = view_alloc();
    view->send_callback = NULL;
    view->send_context = NULL;

    view_allocate_model(view->view, ViewModelTypeLocking, sizeof(NeoclimaExtraViewModel));
    view_set_context(view->view, view);
    view_set_draw_callback(view->view, neoclima_extra_view_draw);
    view_set_input_callback(view->view, neoclima_extra_view_input);
...
void neoclima_extra_view_set_state(NeoclimaExtraView* view, NeoclimaState* state) {
    with_view_model(view->view, NeoclimaExtraViewModel * m, { m->state = state; }, true);
}

void neoclima_extra_view_set_last_sent(NeoclimaExtraView* view, const char* last_sent) {
    with_view_model(view->view, NeoclimaExtraViewModel * m, { m->last_sent = last_sent; }, true);
}
