...
static bool parse_hex(const char* s, uint8_t* out, size_t n) {
    for(size_t i = 0; i < n; i++) {
        int hi = hex_nyb(s[2 * i]);
        int lo = hex_nyb(s[2 * i + 1]);
...
static uint16_t encode_manuf(const char* c) {
    return (uint16_t)((((c[0] - 64) & 0x1F) << 10) | (((c[1] - 64) & 0x1F) << 5) |
                      (((c[2] - 64) & 0x1F)));
}
                    ...
                    if(c2 && (size_t)(c2 - c1) == 8) {
                        *c2++ = 0;
                        if(strlen(line) >= 3 && strlen(c2) >= 32) {
                            ...
                            uint8_t k[16];
                            if(parse_hex(c2, k, 16)) key_store_set(ks, m, id, k);
                        }
                    }
                }
            }
...
bool key_store_save(KeyStore* ks) {
    ...
    for(int i = 0; i < KEY_STORE_MAX; i++) {
        ...
        char m[4];
        wmbus_manuf_decode(ks->entries[i].m, m);
        char buf[80];
        int n = snprintf(buf, sizeof(buf), "%s,%08lX,", m, (unsigned long)ks->entries[i].id);
        storage_file_write(f, buf, (uint16_t)n);
        for(int k = 0; k < 16; k++) {
            char hx[3];
            snprintf(hx, sizeof(hx), "%02X", ks->entries[i].key[k]);
...
bool key_store_lookup(KeyStore* ks, uint16_t m, uint32_t id, uint8_t out[16]) {
    for(int i = 0; i < KEY_STORE_MAX; i++)
        if(ks->entries[i].used && ks->entries[i].m == m && ks->entries[i].id == id) {
            memcpy(out, ks->entries[i].key, 16);
            return true;
        }
    return false;
}
...
void key_store_iter(KeyStore* ks, KeyStoreIterCb cb, void* ctx) {
    for(int i = 0; i < KEY_STORE_MAX; i++)
        if(ks->entries[i].used) cb(ks->entries[i].m, ks->entries[i].id, ks->entries[i].key, ctx);
}
