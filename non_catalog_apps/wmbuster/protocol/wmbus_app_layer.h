...
size_t wmbus_app_parse(const uint8_t* apdu, size_t len, WmbusRecordCb cb, void* ctx);

/* Convenience: render an entire APDU into a multi-line string buffer. */
size_t wmbus_app_render(const uint8_t* apdu, size_t len, char* out, size_t out_size);
