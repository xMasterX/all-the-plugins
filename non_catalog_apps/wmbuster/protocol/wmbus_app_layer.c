...
static int dif_data_len(uint8_t dif_field) {
    ...
    case 0xD:
        return -1; /* variable length, follows */
    case 0xE:
        return 6; /* 12-digit BCD */
    case 0xF:
        return 0; /* manufacturer-specific marker */
    }
    return 0;
}
...
static const VifEntry* vif_table_lookup(const VifEntry* tbl, size_t n, uint8_t vif) {
    uint8_t v = vif & 0x7F;
...
static const VifEntry* vif_lookup_fb(uint8_t vife) {
    return vif_table_lookup(k_vif_fb, sizeof(k_vif_fb) / sizeof(k_vif_fb[0]), vife);
}
static const VifEntry* vif_lookup_fd(uint8_t vife) {
    return vif_table_lookup(k_vif_fd, sizeof(k_vif_fd) / sizeof(k_vif_fd[0]), vife);
}
    ...
    while(i < len) {
        ...
        if(kind_hint == VifKindAscii && dlen > 0) {
            ...
            for(int k = dlen - 1; k >= 0 && wp + 2 <= max_n; k--) {
                wp += snprintf(r.s + wp, sizeof(r.s) - (size_t)wp, "%02X", d[k]);
            }
            r.s[wp] = 0;
            r.s_len = (size_t)wp;
            ...
            if(dlen >= 4) {
                ...
                snprintf(
                    r.s, sizeof(r.s), "%04d-%02d-%02d %02d:%02d", 2000 + yr, mon, day, hr, min);
                r.kind = WmbusValueDate;
                r.s_len = strlen(r.s);
            }
            ...
            for(int k = 0; k < n; k++)
                r.s[k] = d[n - 1 - k]; /* LSB-first reversed */
            r.s[n] = 0;
            r.s_len = n;
        } else {
...
static void format_scaled_int(int64_t i, int e, const char* unit, char* o, size_t cap) {
    /* Apply 10^e scale via integer math; never use float. */
    int64_t scaled = i;
    int frac_digits = 0;
    if(e >= 0) {
        while(e > 0 && scaled < (int64_t)1000000000000LL) {
            scaled *= 10;
            e--;
        }
    } else {
    ...
    if(frac == 0)
        ...
        zeros[leading] = 0;
        snprintf(
            o, cap, "%lld.%s%lld %s", (long long)whole, zeros, (long long)frac, unit ? unit : "");
        (void)fbuf;
        (void)fi;
    }
}
