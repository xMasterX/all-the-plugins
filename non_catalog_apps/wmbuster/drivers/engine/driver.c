...
static bool mvt_matches(const WmbusMVT* m, const char* code, uint8_t v, uint8_t med) {
...
static const WmbusDriver* find_in_registry(uint16_t manuf, uint8_t version, uint8_t medium) {
    char code[4];
    wmbus_manuf_decode(manuf, code);
...
static size_t decode_unknown_oms(
    uint16_t m,
    uint8_t v,
    uint8_t med,
    const uint8_t* a,
...
const WmbusDriver* wmbus_engine_find(uint16_t manuf, uint8_t version, uint8_t medium) {
...
size_t wmbus_engine_decode_ctx(const WmbusDecodeCtx* ctx, char* out, size_t cap) {
    ...
    if(d->decode) {
        return d->decode(
            ctx->manuf, ctx->version, ctx->medium, ctx->apdu, ctx->apdu_len, out, cap);
    }
    /* No custom decode → render with title + OMS walker. */
    return wmbus_engine_render_oms(d->title, ctx->apdu, ctx->apdu_len, out, cap);
}

const char* wmbus_engine_model_name(uint16_t manuf, uint8_t version, uint8_t medium) {
    const WmbusDriver* d = find_in_registry(manuf, version, medium);
