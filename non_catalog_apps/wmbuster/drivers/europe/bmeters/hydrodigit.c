...
static double get_voltage(uint8_t b) {
    ...
    case 0x0D:
        return (double)3.50;
    default:
        return (double)3.70; /* 0x0, 0x0E, 0x0F */
    }
}
...
static size_t decode_hydrodigit(const WmbusDecodeCtx* ctx, char* o, size_t cap) {
    ...
    size_t pos = oms_split_emit(
        "BMeters Hydrodigit", ctx->ci, ctx->apdu, ctx->apdu_len, o, cap, &m, &mlen, &off);
    ...
    if((fid & MASK_BATTERY) && i < mlen) {
        pos += (size_t)snprintf(o + pos, cap - pos, "Volt  %.2f V\n", get_voltage(m[i]));
        i++;
    }
    if((fid & MASK_FRAUD) && i + 2 < mlen) {
        /* BCD year/month/day. Year is 20YY for sane future-proofing. */
        pos += (size_t)snprintf(
            o + pos, cap - pos, "Fraud  20%02X-%02X-%02X\n", m[i], m[i + 1], m[i + 2]);
        i += 3;
    }
    if((fid & MASK_LEAK) && i + 2 < mlen) {
        pos += (size_t)snprintf(
            o + pos, cap - pos, "Leak  20%02X-%02X-%02X\n", m[i], m[i + 1], m[i + 2]);
        i += 3;
    }
    if((fid & MASK_BACKFLOW) && i + 3 < mlen) {
        uint32_t v = (uint32_t)m[i] | ((uint32_t)m[i + 1] << 8) | ((uint32_t)m[i + 2] << 16) |
                     ((uint32_t)m[i + 3] << 24);
        pos += (size_t)snprintf(
            o + pos, cap - pos, "Backflow  %.3f m3\n", (double)v / (double)1000.0);
        i += 4;
    }
    if((fid & MASK_HISTORY) && i + 35 < mlen) {
        for(int month = 1; month <= 4; month++) {
            ...
            pos += (size_t)snprintf(o + pos, cap - pos, "Mn%d  %.2f m3\n", month, m3);
            i += 3;
        }
    }
    return pos;
