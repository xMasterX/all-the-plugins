...
static size_t decode_zenner0b(const WmbusDecodeCtx* ctx, char* out, size_t cap) {
    ...
    size_t pos = oms_split_emit(
        "Zenner B.One water", ctx->ci, ctx->apdu, ctx->apdu_len, out, cap, &m, &mlen, &off);
    ...
    pos += (size_t)snprintf(
        ...
        status == 0 ? "OK" : "fault");
    if(status != 0) {
        pos += (size_t)snprintf(out + pos, cap - pos, "StatHex %08lX\n", (unsigned long)status);
    }
    return pos;
}

static const WmbusMVT k_mvt[] = {{"ZRI", 0x16, 0x0B}, {{0}, 0, 0}};
const WmbusDriver wmbus_drv_zenner0b = {
    .id = "zenner0b",
    .title = "Zenner B.One water",
    .mvt = k_mvt,
    .decode_ex = decode_zenner0b,
};
