...
size_t techem_hex_dump(const uint8_t* a, size_t l, char* o, size_t cap, const char* title) {
    size_t pos = 0;
...
static void techem_prev_date(uint16_t w, char* out, size_t cap, int* yr_out) {
    ...
    if(!day || !mon || mon > 12)
        ...
        snprintf(out, cap, "%04d-%02d-%02d", yr, mon, day);
}
...
static void
    techem_curr_date(uint16_t w, int yr_prev, int day_prev, int mon_prev, char* out, size_t cap) {
    int day = (w >> 4) & 0x1F;
    int mon = (w >> 9) & 0x0F;
    int yr = yr_prev;
...
size_t techem_decode_fhkv(uint8_t version, const uint8_t* a, size_t l, char* o, size_t cap) {
    ...
    techem_curr_date(curr_date_w, yr_prev, day_prev, mon_prev, dcurr, sizeof(dcurr));
    ...
    if(troom != -32768)
        pos += append(
            o, cap, pos, "Room   %d.%02d C\n", troom / 100, (troom < 0 ? -troom : troom) % 100);
    if(trad != -32768)
        pos +=
            append(o, cap, pos, "Rad    %d.%02d C\n", trad / 100, (trad < 0 ? -trad : trad) % 100);
    return pos;
}

size_t techem_decode_water(bool with_dates, const uint8_t* a, size_t l, char* o, size_t cap) {
...
size_t techem_decode_compact5(const uint8_t* a, size_t l, char* o, size_t cap) {
...
size_t techem_decode_vario(const uint8_t* a, size_t l, char* o, size_t cap) {
...
size_t techem_decode_smoke(const uint8_t* a, size_t l, char* o, size_t cap) {
    size_t pos = 0;
