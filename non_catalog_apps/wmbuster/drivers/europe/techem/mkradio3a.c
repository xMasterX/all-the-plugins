...
static size_t decode_mk3a(
    ...
    uint32_t total_raw = ((uint32_t)a[6] << 16) | ((uint32_t)a[5] << 8) | (uint32_t)a[4];
    ...
    if(l >= 10) {
        double curr_month;
        if(d <= 15)
            curr_month = (double)a[8] / (double)10.0;
        else
            curr_month = (double)((unsigned)a[9] + (unsigned)a[8]) / (double)10.0;
        pos += (size_t)snprintf(o + pos, cap - pos, "ThisMon  %.1f m3\n", curr_month);
    }
    return pos;
}

static const WmbusMVT k_mvt[] = {{"TCH", 0x72, 0x50}, {{0}, 0, 0}};
const WmbusDriver wmbus_drv_techem_mkradio3a = {
    .id = "mkradio3a",
    .title = "Techem MK-Radio 3a",
    .mvt = k_mvt,
    .decode = decode_mk3a,
};
