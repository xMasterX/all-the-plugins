...
static void mitsubishi_heavy_main_view_draw(Canvas* canvas, void* model) {
    ...
    draw_selector_button(
        ...
        BTN_W,
        "Mode",
        mitsubishi_heavy_ir_get_mode_name(m->state->mode),
        m->focus == SLOT_MODE);
    ...
    for(int slot = SLOT_TOGGLE_0; slot < SLOT_COUNT; slot++) {
        ...
        if(slot == SLOT_EXTRA) {
            ...
            draw_button(
                ...
                y,
                btn_h,
                mitsubishi_heavy_ir_get_toggle_name(t),
                focused,
...
static void schedule_state_send(MitsubishiHeavyMainView* view) {
    with_view_model(
        view->view, MitsubishiHeavyMainViewModel * m, { m->state_pending = true; }, false);
                ...
                case InputKeyOk:
                    if(event->type == InputTypeShort) {
                        if(m->picker_kind == PickerMode) {
                            mitsubishi_heavy_state_set_mode(
                                m->state, (MitsubishiHeavyMode)m->picker_index);
                        } else {
                            mitsubishi_heavy_state_set_fan(
                                m->state, (MitsubishiHeavyFan)m->picker_index);
                        }
