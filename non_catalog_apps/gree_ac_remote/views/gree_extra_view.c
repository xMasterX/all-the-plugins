...
struct GreeExtraView {
    View* view;
    GreeExtraViewSendCallback send_callback;
    void* send_context;
};

static void gree_extra_view_draw(Canvas* canvas, void* model) {
    GreeExtraViewModel* m = model;
    if(!m->state) return;
    ...
    uint8_t total = (uint8_t)GreeExtraCount;
    ...
    for(uint8_t r = 0; r < shown; r++) {
        ...
        canvas_draw_str_aligned(
            canvas, 32, y + 2, AlignCenter, AlignTop, gree_ir_get_extra_name((GreeExtra)i));
    ...
    GreeRequest req = gree_state_request(m->state);
    gree_ir_format_extra(&req, (GreeExtra)m->selected, buf, sizeof(buf));
...
static bool gree_extra_view_input(InputEvent* event, void* context) {
    GreeExtraView* view = context;
    bool consumed = false;
    bool send = false;
    GreeExtra to_send = (GreeExtra)0;

    with_view_model(
        view->view,
        GreeExtraViewModel * m,
        {
            if(event->type == InputTypeShort || event->type == InputTypeRepeat) {
                ...
                case InputKeyUp:
                    ...
                    break;
                case InputKeyDown:
                    if(m->selected + 1 < (uint8_t)GreeExtraCount) m->selected++;
...
GreeExtraView* gree_extra_view_alloc(void) {
    GreeExtraView* view = malloc(sizeof(GreeExtraView));
    view->view = view_alloc();
    view->send_callback = NULL;
    view->send_context = NULL;

    view_allocate_model(view->view, ViewModelTypeLocking, sizeof(GreeExtraViewModel));
    view_set_context(view->view, view);
    view_set_draw_callback(view->view, gree_extra_view_draw);
    view_set_input_callback(view->view, gree_extra_view_input);
...
void gree_extra_view_set_state(GreeExtraView* view, GreeState* state) {
    with_view_model(view->view, GreeExtraViewModel * m, { m->state = state; }, true);
}

void gree_extra_view_set_last_sent(GreeExtraView* view, const char* last_sent) {
    with_view_model(view->view, GreeExtraViewModel * m, { m->last_sent = last_sent; }, true);
}
