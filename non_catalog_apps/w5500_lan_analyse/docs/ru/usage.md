...
| AT Internet IP | IP-адрес | 1.1.1.1 | Адрес для проверки доступа в интернет (ping, затем TCP, если ping блокируется) |
| AT TCP port | 1-65535 | 443 | Порт для TCP-проверки на шаге Internet |
...
Проверка доступности через TCP-соединение вместо ICMP. Полезно, когда firewall
блокирует ping, но обычный трафик проходит.

**Ввод**: IP-адрес цели (по умолчанию — шлюз DHCP), затем порт (по умолчанию 443).

**Результат попытки:**
- `open` -- порт принял соединение
- `RST` -- хост отклонил соединение. Он всё равно ответил, значит доступен
- `timeout` -- ответа нет: хост недоступен или порт молча отбрасывает пакеты

Повторяется **Count** раз с **Timeout** и **Interval** из Settings, затем выводится
число ответивших попыток и min/avg/max времени отклика.
...
| AT Internet IP | IP-адрес | 1.1.1.1 | Адрес для проверки доступа в интернет в Auto Test (ping, затем TCP, если ping блокируется) |
| AT TCP port | 1-65535 | 443 | Порт для TCP-проверки на шаге Internet в Auto Test |
