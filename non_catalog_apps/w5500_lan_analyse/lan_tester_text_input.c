...
#include "lan_tester_icons.h"
#include <furi.h>

struct LanTesterTextInput {
    View* view;
    FuriTimer* timer;
};
...
typedef struct {
    ...
    const uint8_t x;
    const uint8_t y;
} LanTesterTextInputKey;

typedef struct {
    const LanTesterTextInputKey* rows[3];
...
typedef struct {
    ...
    LanTesterTextInputValidatorCallback validator_callback;
    void* validator_callback_context;
    FuriString* validator_text;
    bool validator_message_visible;
} LanTesterTextInputModel;
...
static const LanTesterTextInputKey keyboard_keys_row_1[] = {
    {'q', 1, 8},
    {'w', 10, 8},
    {'e', 19, 8},
    {'r', 28, 8},
...
static const LanTesterTextInputKey keyboard_keys_row_2[] = {
    {'a', 1, 20},
    {'s', 10, 20},
    {'d', 19, 20},
    {'f', 28, 20},
...
static const LanTesterTextInputKey keyboard_keys_row_3[] = {
...
static const LanTesterTextInputKey symbol_keyboard_keys_row_1[] = {
    {'!', 2, 8},
    {'@', 12, 8},
    {'#', 22, 8},
    {'$', 32, 8},
...
static const LanTesterTextInputKey symbol_keyboard_keys_row_2[] = {
    {'~', 2, 20},
    {'+', 12, 20},
    {'-', 22, 20},
    {'=', 32, 20},
...
static const LanTesterTextInputKey symbol_keyboard_keys_row_3[] = {
    {SWITCH_KEYBOARD_KEY, 1, 23},
    {'.', 15, 32},
    {',', 29, 32},
    {':', 41, 32},
...
static void switch_keyboard(LanTesterTextInputModel* model) {
...
static const LanTesterTextInputKey* get_row(const Keyboard* keyboard, uint8_t row_index) {
    const LanTesterTextInputKey* row = NULL;
    if(row_index < 3) {
...
static char get_selected_char(LanTesterTextInputModel* model) {
    return get_row(
...
static void lan_tester_text_input_backspace_cb(LanTesterTextInputModel* model) {
...
static void lan_tester_text_input_view_draw_callback(Canvas* canvas, void* _model) {
    LanTesterTextInputModel* model = _model;
    ...
    for(uint8_t row = 0; row < keyboard_row_count; row++) {
        ...
        const LanTesterTextInputKey* keys = get_row(keyboards[model->selected_keyboard], row);
...
static void lan_tester_text_input_handle_up(
    LanTesterTextInput* text_input,
    LanTesterTextInputModel* model) {
    UNUSED(text_input);
...
static void lan_tester_text_input_handle_down(
    LanTesterTextInput* text_input,
    LanTesterTextInputModel* model) {
    UNUSED(text_input);
...
static void lan_tester_text_input_handle_left(
    LanTesterTextInput* text_input,
    LanTesterTextInputModel* model) {
    UNUSED(text_input);
    if(model->cursor_select) {
...
static void lan_tester_text_input_handle_right(
    LanTesterTextInput* text_input,
    LanTesterTextInputModel* model) {
    UNUSED(text_input);
    if(model->cursor_select) {
...
static void lan_tester_text_input_handle_ok(
    LanTesterTextInput* text_input,
    LanTesterTextInputModel* model,
    InputType type) {
    ...
    if(selected == ENTER_KEY) {
        ...
        if(selected == BACKSPACE_KEY) {
            lan_tester_text_input_backspace_cb(model);
        } else if(!repeat) {
            if(model->clear_default_text) {
                text_length = 0;
            }
...
static bool lan_tester_text_input_view_input_callback(InputEvent* event, void* context) {
    LanTesterTextInput* text_input = context;
    furi_assert(text_input);

    bool consumed = false;

    // Acquire model
    LanTesterTextInputModel* model = view_get_model(text_input->view);
    ...
    if((!(event->type == InputTypePress) && !(event->type == InputTypeRelease)) &&
       model->validator_message_visible) {
        ...
        switch(event->key) {
        case InputKeyUp:
            lan_tester_text_input_handle_up(text_input, model);
            break;
        case InputKeyDown:
            lan_tester_text_input_handle_down(text_input, model);
            break;
        case InputKeyLeft:
            lan_tester_text_input_handle_left(text_input, model);
            break;
        case InputKeyRight:
            lan_tester_text_input_handle_right(text_input, model);
            break;
        case InputKeyOk:
            lan_tester_text_input_handle_ok(text_input, model, event->type);
            break;
        default:
            consumed = false;
            break;
...
static void lan_tester_text_input_timer_callback(void* context) {
    furi_assert(context);
    LanTesterTextInput* text_input = context;
...
LanTesterTextInput* lan_tester_text_input_alloc() {
    LanTesterTextInput* text_input = malloc(sizeof(LanTesterTextInput));
    text_input->view = view_alloc();
    view_set_context(text_input->view, text_input);
    view_allocate_model(text_input->view, ViewModelTypeLocking, sizeof(LanTesterTextInputModel));
    view_set_draw_callback(text_input->view, lan_tester_text_input_view_draw_callback);
    view_set_input_callback(text_input->view, lan_tester_text_input_view_input_callback);

    text_input->timer =
        furi_timer_alloc(lan_tester_text_input_timer_callback, FuriTimerTypeOnce, text_input);
    ...
    lan_tester_text_input_reset(text_input);
...
LanTesterTextInputValidatorCallback
    ...
    LanTesterTextInputValidatorCallback validator_callback = NULL;
    with_view_model(
        text_input->view,
        LanTesterTextInputModel * model,
...
void lan_tester_text_input_set_header_text(LanTesterTextInput* text_input, const char* text) {
    with_view_model(
        text_input->view, LanTesterTextInputModel * model, { model->header = text; }, true);
}
