...
} CarrierExtraViewModel;

struct CarrierExtraView {
    View* view;
    CarrierExtraViewSendCallback send_callback;
    void* send_context;
};

static void carrier_extra_view_draw(Canvas* canvas, void* model) {
    CarrierExtraViewModel* m = model;
    if(!m->state) return;
    ...
    uint8_t total = (uint8_t)CarrierExtraCount;
    ...
    for(uint8_t r = 0; r < shown; r++) {
        ...
        canvas_draw_str_aligned(
            canvas, 32, y + 2, AlignCenter, AlignTop, carrier_ir_get_extra_name((CarrierExtra)i));
    ...
    CarrierRequest req = carrier_state_request(m->state);
    carrier_ir_format_extra(&req, (CarrierExtra)m->selected, buf, sizeof(buf));
...
static bool carrier_extra_view_input(InputEvent* event, void* context) {
    CarrierExtraView* view = context;
    bool consumed = false;
    bool send = false;
    CarrierExtra to_send = (CarrierExtra)0;

    with_view_model(
        view->view,
        CarrierExtraViewModel * m,
        {
            if(event->type == InputTypeShort || event->type == InputTypeRepeat) {
                ...
                case InputKeyUp:
                    ...
                    break;
                case InputKeyDown:
                    if(m->selected + 1 < (uint8_t)CarrierExtraCount) m->selected++;
                ...
                case InputKeyOk:
                    if(event->type == InputTypeShort) {
                        to_send = (CarrierExtra)m->selected;
                        send = true;
                    }
                    consumed = true;
                    break;
...
CarrierExtraView* carrier_extra_view_alloc(void) {
    CarrierExtraView* view = malloc(sizeof(CarrierExtraView));
    view->view = view_alloc();
    view->send_callback = NULL;
    view->send_context = NULL;

    view_allocate_model(view->view, ViewModelTypeLocking, sizeof(CarrierExtraViewModel));
    view_set_context(view->view, view);
    view_set_draw_callback(view->view, carrier_extra_view_draw);
    view_set_input_callback(view->view, carrier_extra_view_input);
...
void carrier_extra_view_set_state(CarrierExtraView* view, CarrierState* state) {
    with_view_model(view->view, CarrierExtraViewModel * m, { m->state = state; }, true);
}

void carrier_extra_view_set_last_sent(CarrierExtraView* view, const char* last_sent) {
    with_view_model(view->view, CarrierExtraViewModel * m, { m->last_sent = last_sent; }, true);
}
