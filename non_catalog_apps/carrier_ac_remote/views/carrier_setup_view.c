...
typedef struct {
    CarrierState* state;
    uint8_t focus;
} CarrierSetupViewModel;

struct CarrierSetupView {
    View* view;
};

static uint8_t row_count(void) {
    return carrier_ir_get_option_count() > 0 ? 2 : 1;
}
...
static void carrier_setup_view_draw(Canvas* canvas, void* model) {
    CarrierSetupViewModel* m = model;
    if(!m->state) return;
    ...
    bool has_opt = carrier_ir_get_option_count() > 0;
    ...
    if(has_opt) {
        ...
        canvas_set_font(canvas, FontSecondary);
        canvas_draw_str_aligned(
            canvas, 32, oy + 3, AlignCenter, AlignTop, carrier_ir_get_option_label());
        ...
        canvas_draw_str_aligned(
            ...
            AlignCenter,
            AlignTop,
            carrier_ir_get_option_name(m->state->option));
        ...
        snprintf(
            ...
            (unsigned)carrier_ir_get_option_count());
...
static bool carrier_setup_view_input(InputEvent* event, void* context) {
    CarrierSetupView* view = context;
    bool consumed = false;

    with_view_model(
        view->view,
        CarrierSetupViewModel * m,
        {
            if(event->type != InputTypeShort && event->type != InputTypeRepeat) {
                // nothing
            } else {
                uint8_t rows = row_count();
                uint8_t opts = carrier_ir_get_option_count();
                switch(event->key) {
                case InputKeyUp:
...
CarrierSetupView* carrier_setup_view_alloc(void) {
    CarrierSetupView* view = malloc(sizeof(CarrierSetupView));
    view->view = view_alloc();

    view_allocate_model(view->view, ViewModelTypeLocking, sizeof(CarrierSetupViewModel));
    view_set_context(view->view, view);
    view_set_draw_callback(view->view, carrier_setup_view_draw);
    view_set_input_callback(view->view, carrier_setup_view_input);
...
void carrier_setup_view_set_state(CarrierSetupView* view, CarrierState* state) {
    with_view_model(view->view, CarrierSetupViewModel * m, { m->state = state; }, true);
}
