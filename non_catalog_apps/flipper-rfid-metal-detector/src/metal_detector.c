...
static void trigger_feedback(MetalDetectorApp* app, uint16_t delta) {
    ...
    bool play_sound =
        (app->alert_mode == AlertModeSoundVibro || app->alert_mode == AlertModeSound);
    bool play_vibro =
        (app->alert_mode == AlertModeSoundVibro || app->alert_mode == AlertModeVibro);
