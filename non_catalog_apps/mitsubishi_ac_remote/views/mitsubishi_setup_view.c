#include "mitsubishi_setup_view.h"
...
typedef struct {
    ...
    uint8_t focus;
} MitsubishiSetupViewModel;

struct MitsubishiSetupView {
    View* view;
};

static uint8_t row_count(void) {
    return mitsubishi_ir_get_option_count() > 0 ? 2 : 1;
}
...
static void mitsubishi_setup_view_draw(Canvas* canvas, void* model) {
    MitsubishiSetupViewModel* m = model;
    if(!m->state) return;
    ...
    bool has_opt = mitsubishi_ir_get_option_count() > 0;
    ...
    if(has_opt) {
        ...
        canvas_set_font(canvas, FontSecondary);
        canvas_draw_str_aligned(
            canvas, 32, oy + 3, AlignCenter, AlignTop, mitsubishi_ir_get_option_label());
        ...
        canvas_draw_str_aligned(
            ...
            AlignCenter,
            AlignTop,
            mitsubishi_ir_get_option_name(m->state->option));
        ...
        snprintf(
            ...
            (unsigned)mitsubishi_ir_get_option_count());
...
static bool mitsubishi_setup_view_input(InputEvent* event, void* context) {
    MitsubishiSetupView* view = context;
    bool consumed = false;

    with_view_model(
        view->view,
        MitsubishiSetupViewModel * m,
        {
            if(event->type != InputTypeShort && event->type != InputTypeRepeat) {
                // nothing
            } else {
                uint8_t rows = row_count();
                uint8_t opts = mitsubishi_ir_get_option_count();
                switch(event->key) {
                case InputKeyUp:
...
MitsubishiSetupView* mitsubishi_setup_view_alloc(void) {
    MitsubishiSetupView* view = malloc(sizeof(MitsubishiSetupView));
    view->view = view_alloc();

    view_allocate_model(view->view, ViewModelTypeLocking, sizeof(MitsubishiSetupViewModel));
    view_set_context(view->view, view);
    view_set_draw_callback(view->view, mitsubishi_setup_view_draw);
    view_set_input_callback(view->view, mitsubishi_setup_view_input);
...
void mitsubishi_setup_view_set_state(MitsubishiSetupView* view, MitsubishiState* state) {
    with_view_model(view->view, MitsubishiSetupViewModel * m, { m->state = state; }, true);
}
