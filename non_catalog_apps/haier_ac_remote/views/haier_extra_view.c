...
struct HaierExtraView {
    View* view;
    HaierExtraViewSendCallback send_callback;
    void* send_context;
};

static void haier_extra_view_draw(Canvas* canvas, void* model) {
    HaierExtraViewModel* m = model;
    if(!m->state) return;
    ...
    uint8_t total = (uint8_t)HaierExtraCount;
    ...
    for(uint8_t r = 0; r < shown; r++) {
        ...
        canvas_draw_str_aligned(
            canvas, 32, y + 2, AlignCenter, AlignTop, haier_ir_get_extra_name((HaierExtra)i));
    ...
    HaierRequest req = haier_state_request(m->state);
    haier_ir_format_extra(&req, (HaierExtra)m->selected, buf, sizeof(buf));
...
static bool haier_extra_view_input(InputEvent* event, void* context) {
    HaierExtraView* view = context;
    bool consumed = false;
    bool send = false;
    HaierExtra to_send = (HaierExtra)0;

    with_view_model(
        view->view,
        HaierExtraViewModel * m,
        {
            if(event->type == InputTypeShort || event->type == InputTypeRepeat) {
                ...
                case InputKeyUp:
                    ...
                    break;
                case InputKeyDown:
                    if(m->selected + 1 < (uint8_t)HaierExtraCount) m->selected++;
...
HaierExtraView* haier_extra_view_alloc(void) {
    HaierExtraView* view = malloc(sizeof(HaierExtraView));
    view->view = view_alloc();
    view->send_callback = NULL;
    view->send_context = NULL;

    view_allocate_model(view->view, ViewModelTypeLocking, sizeof(HaierExtraViewModel));
    view_set_context(view->view, view);
    view_set_draw_callback(view->view, haier_extra_view_draw);
    view_set_input_callback(view->view, haier_extra_view_input);
...
void haier_extra_view_set_state(HaierExtraView* view, HaierState* state) {
    with_view_model(view->view, HaierExtraViewModel * m, { m->state = state; }, true);
}

void haier_extra_view_set_last_sent(HaierExtraView* view, const char* last_sent) {
    with_view_model(view->view, HaierExtraViewModel * m, { m->last_sent = last_sent; }, true);
}
