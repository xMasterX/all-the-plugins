...
typedef struct {
    SamsungState* state;
    uint8_t focus;
} SamsungSetupViewModel;

struct SamsungSetupView {
    View* view;
};

static uint8_t row_count(void) {
    return samsung_ir_get_option_count() > 0 ? 2 : 1;
}
...
static void samsung_setup_view_draw(Canvas* canvas, void* model) {
    SamsungSetupViewModel* m = model;
    if(!m->state) return;
    ...
    bool has_opt = samsung_ir_get_option_count() > 0;
    ...
    if(has_opt) {
        ...
        canvas_set_font(canvas, FontSecondary);
        canvas_draw_str_aligned(
            canvas, 32, oy + 3, AlignCenter, AlignTop, samsung_ir_get_option_label());
        ...
        canvas_draw_str_aligned(
            ...
            AlignCenter,
            AlignTop,
            samsung_ir_get_option_name(m->state->option));
        ...
        snprintf(
            ...
            (unsigned)samsung_ir_get_option_count());
...
static bool samsung_setup_view_input(InputEvent* event, void* context) {
    SamsungSetupView* view = context;
    bool consumed = false;

    with_view_model(
        view->view,
        SamsungSetupViewModel * m,
        {
            if(event->type != InputTypeShort && event->type != InputTypeRepeat) {
                // nothing
            } else {
                uint8_t rows = row_count();
                uint8_t opts = samsung_ir_get_option_count();
                switch(event->key) {
                case InputKeyUp:
...
SamsungSetupView* samsung_setup_view_alloc(void) {
    SamsungSetupView* view = malloc(sizeof(SamsungSetupView));
    view->view = view_alloc();

    view_allocate_model(view->view, ViewModelTypeLocking, sizeof(SamsungSetupViewModel));
    view_set_context(view->view, view);
    view_set_draw_callback(view->view, samsung_setup_view_draw);
    view_set_input_callback(view->view, samsung_setup_view_input);
...
void samsung_setup_view_set_state(SamsungSetupView* view, SamsungState* state) {
    with_view_model(view->view, SamsungSetupViewModel * m, { m->state = state; }, true);
}
