...
} SamsungExtraViewModel;

struct SamsungExtraView {
    View* view;
    SamsungExtraViewSendCallback send_callback;
    void* send_context;
};

static void samsung_extra_view_draw(Canvas* canvas, void* model) {
    SamsungExtraViewModel* m = model;
    if(!m->state) return;
    ...
    uint8_t total = (uint8_t)SamsungExtraCount;
    ...
    for(uint8_t r = 0; r < shown; r++) {
        ...
        canvas_draw_str_aligned(
            canvas, 32, y + 2, AlignCenter, AlignTop, samsung_ir_get_extra_name((SamsungExtra)i));
    ...
    SamsungRequest req = samsung_state_request(m->state);
    samsung_ir_format_extra(&req, (SamsungExtra)m->selected, buf, sizeof(buf));
...
static bool samsung_extra_view_input(InputEvent* event, void* context) {
    SamsungExtraView* view = context;
    bool consumed = false;
    bool send = false;
    SamsungExtra to_send = (SamsungExtra)0;

    with_view_model(
        view->view,
        SamsungExtraViewModel * m,
        {
            if(event->type == InputTypeShort || event->type == InputTypeRepeat) {
                ...
                case InputKeyUp:
                    ...
                    break;
                case InputKeyDown:
                    if(m->selected + 1 < (uint8_t)SamsungExtraCount) m->selected++;
                ...
                case InputKeyOk:
                    if(event->type == InputTypeShort) {
                        to_send = (SamsungExtra)m->selected;
                        send = true;
                    }
                    consumed = true;
                    break;
...
SamsungExtraView* samsung_extra_view_alloc(void) {
    SamsungExtraView* view = malloc(sizeof(SamsungExtraView));
    view->view = view_alloc();
    view->send_callback = NULL;
    view->send_context = NULL;

    view_allocate_model(view->view, ViewModelTypeLocking, sizeof(SamsungExtraViewModel));
    view_set_context(view->view, view);
    view_set_draw_callback(view->view, samsung_extra_view_draw);
    view_set_input_callback(view->view, samsung_extra_view_input);
...
void samsung_extra_view_set_state(SamsungExtraView* view, SamsungState* state) {
    with_view_model(view->view, SamsungExtraViewModel * m, { m->state = state; }, true);
}

void samsung_extra_view_set_last_sent(SamsungExtraView* view, const char* last_sent) {
    with_view_model(view->view, SamsungExtraViewModel * m, { m->last_sent = last_sent; }, true);
}
