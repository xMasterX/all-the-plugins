...
struct TclExtraView {
    View* view;
    TclExtraViewSendCallback send_callback;
    void* send_context;
};

static void tcl_extra_view_draw(Canvas* canvas, void* model) {
    TclExtraViewModel* m = model;
    if(!m->state) return;
    ...
    uint8_t total = (uint8_t)TclExtraCount;
    ...
    for(uint8_t r = 0; r < shown; r++) {
        ...
        canvas_draw_str_aligned(
            canvas, 32, y + 2, AlignCenter, AlignTop, tcl_ir_get_extra_name((TclExtra)i));
    ...
    TclRequest req = tcl_state_request(m->state);
    tcl_ir_format_extra(&req, (TclExtra)m->selected, buf, sizeof(buf));
...
static bool tcl_extra_view_input(InputEvent* event, void* context) {
    TclExtraView* view = context;
    bool consumed = false;
    bool send = false;
    TclExtra to_send = (TclExtra)0;
        ...
        TclExtraViewModel * m,
        {
            if(event->type == InputTypeShort || event->type == InputTypeRepeat) {
                ...
                case InputKeyUp:
                    ...
                    break;
                case InputKeyDown:
                    if(m->selected + 1 < (uint8_t)TclExtraCount) m->selected++;
...
TclExtraView* tcl_extra_view_alloc(void) {
    TclExtraView* view = malloc(sizeof(TclExtraView));
    view->view = view_alloc();
    view->send_callback = NULL;
    view->send_context = NULL;

    view_allocate_model(view->view, ViewModelTypeLocking, sizeof(TclExtraViewModel));
    view_set_context(view->view, view);
    view_set_draw_callback(view->view, tcl_extra_view_draw);
    view_set_input_callback(view->view, tcl_extra_view_input);
...
void tcl_extra_view_set_state(TclExtraView* view, TclState* state) {
    with_view_model(view->view, TclExtraViewModel * m, { m->state = state; }, true);
}

void tcl_extra_view_set_last_sent(TclExtraView* view, const char* last_sent) {
    with_view_model(view->view, TclExtraViewModel * m, { m->last_sent = last_sent; }, true);
}
