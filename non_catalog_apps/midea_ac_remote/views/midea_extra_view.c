...
struct MideaExtraView {
    View* view;
    MideaExtraViewSendCallback send_callback;
    void* send_context;
};

static void midea_extra_view_draw(Canvas* canvas, void* model) {
    MideaExtraViewModel* m = model;
    if(!m->state) return;
    ...
    uint8_t total = (uint8_t)MideaExtraCount;
    ...
    for(uint8_t r = 0; r < shown; r++) {
        ...
        canvas_draw_str_aligned(
            canvas, 32, y + 2, AlignCenter, AlignTop, midea_ir_get_extra_name((MideaExtra)i));
    ...
    MideaRequest req = midea_state_request(m->state);
    midea_ir_format_extra(&req, (MideaExtra)m->selected, buf, sizeof(buf));
...
static bool midea_extra_view_input(InputEvent* event, void* context) {
    MideaExtraView* view = context;
    bool consumed = false;
    bool send = false;
    MideaExtra to_send = (MideaExtra)0;

    with_view_model(
        view->view,
        MideaExtraViewModel * m,
        {
            if(event->type == InputTypeShort || event->type == InputTypeRepeat) {
                ...
                case InputKeyUp:
                    ...
                    break;
                case InputKeyDown:
                    if(m->selected + 1 < (uint8_t)MideaExtraCount) m->selected++;
...
MideaExtraView* midea_extra_view_alloc(void) {
    MideaExtraView* view = malloc(sizeof(MideaExtraView));
    view->view = view_alloc();
    view->send_callback = NULL;
    view->send_context = NULL;

    view_allocate_model(view->view, ViewModelTypeLocking, sizeof(MideaExtraViewModel));
    view_set_context(view->view, view);
    view_set_draw_callback(view->view, midea_extra_view_draw);
    view_set_input_callback(view->view, midea_extra_view_input);
...
void midea_extra_view_set_state(MideaExtraView* view, MideaState* state) {
    with_view_model(view->view, MideaExtraViewModel * m, { m->state = state; }, true);
}

void midea_extra_view_set_last_sent(MideaExtraView* view, const char* last_sent) {
    with_view_model(view->view, MideaExtraViewModel * m, { m->last_sent = last_sent; }, true);
}
