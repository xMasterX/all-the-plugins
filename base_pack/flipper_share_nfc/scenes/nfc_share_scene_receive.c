...
#include "nfc_share.h"
#include "nfc_share_scene.h"

#include <gui/gui.h>
#include <gui/view.h>
#include <gui/elements.h>

#define NSH_IDLE_OPERATION 50 //ms

#define TAG "NfcShareRecv"
...
static void progress_view_draw_callback(Canvas* canvas, void* context) {
    ...
    // nsh_parts concurrently). Skip on contention rather than block the renderer.
    char fname[NSH_FILENAME_LENGTH];
    uint32_t fsize = 0;
    uint32_t rcv = 0, need = 0, start = 0, last_progress = 0;
    bool finalizing = false;
    uint8_t levels[NSH_PARTS_COUNT];
    if(nsh_try_lock_ms(10)) {
    ...
    elements_multiline_text_aligned(
        canvas, 64, SCENE_HEADER_POSITION_Y, AlignCenter, AlignTop, "Receiving via NFC...");
    ...
    // If no new block has arrived for NSH_STALL_MS, show "stalled" instead of a
    ...
    if(finalizing) {
        ...
        uint64_t e = (elapsed_ms >= NSH_ETA_WARMUP_MS && recv_bytes > 0) ?
                         ((uint64_t)rem_bytes * elapsed_ms / ((uint64_t)recv_bytes * 1000u)) :
                         ((uint64_t)rem_bytes / NSH_PAYLOAD_THROUGHPUT_BPS);
        if(e > NSH_ETA_MAX_SEC) e = NSH_ETA_MAX_SEC;
        char eta[16];
        nsh_fmt_duration((uint32_t)e, eta, sizeof(eta));
        snprintf(
            info,
            sizeof(info),
            "%u%%  %lu KB  ETA %s",
    ...
    for(uint32_t i = 0; i < NSH_PARTS_COUNT; ++i) {
        uint8_t lv = levels[i];
        if(!lv) continue;
        int fill = (lv * h) / 255;
        if(fill < 1) fill = 1;
...
static void progress_view_init(NfcShareApp* app) {
    ...
    view_dispatcher_add_view(app->view_dispatcher, NfcShareViewIdProgress, progress_view);
}

static void progress_view_deinit(NfcShareApp* app) {
    if(!progress_view) return;
    view_dispatcher_remove_view(app->view_dispatcher, NfcShareViewIdProgress);
    view_free(progress_view);
    progress_view = NULL;
    progress_view_active = false;
}

void nfc_share_scene_receive_on_enter(void* context) {
    NfcShareApp* app = context;

    // Create the shared-state lock BEFORE starting the worker thread and the
    // NFC stack, so both threads see a valid mutex from their first tick.
    nsh_lock_ensure();
    ...
    // Receiver role: act as the reader and drive the exchange loop.
    nfc_transport_init(NfcTransportModePoller);
}
...
static void update_timer_callback(void* context) {
    ...
    if(!nsh_try_lock_ms(20)) return; // skip this tick on contention
    if(complete) {
        is_success = g.r_is_success;
        // Actual receive throughput + elapsed time (r_start_ms..r_finnsh_ms).
        uint32_t st = g.r_start_ms, fin = g.r_finnsh_ms, fsz = g.r_file_size;
        uint32_t el_ms = (fin > st) ? (fin - st) : 0;
        uint32_t bps = (el_ms > 0) ? (uint32_t)((uint64_t)fsz * 1000u / el_ms) : 0;
        char tbuf[16];
        nsh_fmt_duration(el_ms / 1000u, tbuf, sizeof(tbuf));
        snprintf(
            progress_text,
    ...
    if(complete) {
        ...
        if(!progress_view_active) {
            view_dispatcher_switch_to_view(app->view_dispatcher, NfcShareViewIdProgress);
...
bool nfc_share_scene_receive_on_event(void* context, SceneManagerEvent event) {
    NfcShareApp* app = context;
    bool consumed = false;
...
void nfc_share_scene_receive_on_exit(void* context) {
    NfcShareApp* app = context;
