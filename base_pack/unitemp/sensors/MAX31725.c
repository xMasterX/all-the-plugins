...
bool unitemp_MAX31725_alloc(Sensor* sensor, char* args) {
    ...
    i2c_sensor->min_i2c_adress = (0x48 << 1);
    i2c_sensor->max_i2c_adress = (0x4F << 1);
    return true;
}
...
bool unitemp_MAX31725_init(Sensor* sensor) {
    ...
    return unitemp_i2c_write_reg(i2c_sensor, MAX31725_REG_CONFIG, cfg);
}
...
SensorStatus unitemp_MAX31725_update(Sensor* sensor) {
    I2CSensor* i2c_sensor = (I2CSensor*)sensor->instance;
    uint8_t buff[2];
    if(!unitemp_i2c_read_reg_array(i2c_sensor, MAX31725_REG_TEMP, 2, buff))
        return UT_SENSORSTATUS_TIMEOUT;
    int16_t raw = ((int16_t)buff[0] << 8) | buff[1];
    // Q8.8 format: LSB = 0.00390625°C
    sensor->temperature = raw * 0.00390625;
