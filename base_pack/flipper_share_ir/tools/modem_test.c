...
static size_t build_events(
    const uint32_t* t, size_t tc, IrModemEvent* ev, int bias, int jitter) {
    // start_from_mark=false layout: index0 = space, then alternating mark/space.
    size_t n = 0;
    for(size_t i = 0; i < tc; ++i) {
        bool level = (i % 2 == 1); // odd index = mark
        int d = (int)t[i];
        if(level) d += bias; else d -= bias;
...
static int test_one(size_t len, int bias, int jitter, const char* label) {
    ...
    for(size_t i = 0; i < len; ++i) in[i] = (uint8_t)(rng() & 0xFF);
    ...
    if(tc == 0) { printf("  [%s len=%zu] ENCODE FAILED\n", label, len); return 1; }
    ...
    if(!ok) { printf("  [%s len=%zu] DECODE FAILED\n", label, len); return 1; }
    if(out_len != len) {
    ...
    if(memcmp(in, out, len) != 0) {
        ...
        for(size_t i = 0; i < len; ++i)
            if(in[i] != out[i]) { printf("    byte %zu: %02X != %02X\n", i, in[i], out[i]); }
        return 1;
    }
    return 0;
}
...
int main(void) {
    ...
    printf("bits/sym=%u levels=%u mark=%u base=%u step=%u maxspace=%u rxto=%u\n",
        IR_MODEM_BITS_PER_SYMBOL, IR_MODEM_LEVELS, IR_MODEM_MARK_US, IR_MODEM_SPACE_BASE_US,
