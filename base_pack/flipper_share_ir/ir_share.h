...
// ===== Behaviour tunables ====================================================
#define ISH_PAYLOAD_THROUGHPUT_BPS 130                      // nominal IR payload throughput for ETA, bytes/sec (measured ~131 on 8 KiB after Tier-1 tuning)
...
typedef struct {
    ...
    uint32_t (*cb_read_block)(uint32_t block_number, uint8_t out52[ISH_DATA_LENGTH]);        // Reader callback from real storage
    ...
    void (*cb_write_block)(uint32_t block_number, const uint8_t in52[ISH_DATA_LENGTH], uint32_t valid_len);
} ish_ctx_t;
...
typedef struct {
    ...
    void (*r_write_block)(uint32_t block_number, const uint8_t in52[ISH_DATA_LENGTH], uint32_t valid_len);
} ish_init_params_t;
